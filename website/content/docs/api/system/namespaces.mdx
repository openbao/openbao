...
## Migrate a namespace barrier

This endpoint allows migration of a namespace between a normal (unsealable) and sealable namespace by creating or removing a namespace barrier. Sudo permissions in the namespace are required for this. This action also affects normal (unsealable) child namespaces of the given namespace. Sealable child namespaces remain unaffected.

| Method | Path                    |
| :----- | :---------------------- |
| `POST`  | `/sys/namespaces/:path/migrate-barrier` |
...
$ curl \
    ...
    --request POST \
    --data @payload.json \
    http://127.0.0.1:8200/v1/sys/namespaces/ns1/migrate-barrier
```
