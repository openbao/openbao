...
func (m *MySQLBackend) ListPage(ctx context.Context, prefix string, after string, limit int) ([]string, error) {
	...
	// For MySQL backend, we implement ListPage using the basic List operation
	// and then filter the results. This is because the original Vault MySQL
	// backend doesn't support pagination natively.
	allKeys, err := m.List(ctx, prefix)
	if err != nil {
		return nil, err
	}

	// Filter keys after the given key
	var keys []string
	foundAfter := after == ""
...
func NewMySQLLock(in *MySQLBackend, l log.Logger, key, value string) (*MySQLLock, error) {
	...
	statements := map[string]string{
		"put": "INSERT INTO " + in.dbLockTable + " VALUES( ?, ? ) ON DUPLICATE KEY UPDATE current_leader=VALUES(current_leader)",
	}
