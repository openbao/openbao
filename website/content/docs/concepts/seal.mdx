...
description: >-
  A OpenBao must be unsealed before it can access its data. Likewise, it can be
  sealed to lock it down.
---

# Seal/Unseal

When a OpenBao server is started, it starts in a _sealed_ state. In this
...
The unseal process is done by running `openbao operator unseal` or via the API.
...
Once a OpenBao node is unsealed, it remains unsealed until one of these things happens:
...
-> **Note:** Unsealing makes the process of automating a OpenBao install
...
It is still possible to seal a OpenBao node using the API. In this case OpenBao
...
OpenBao's unseal key can be rekeyed using a normal `openbao operator rekey`
...
this is performed by using the `-target=recovery` flag to `openbao operator rekey`.
...
`$ openbao operator unseal -migrate`
