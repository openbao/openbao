...
Note: Starting with OpenBao v2.4, `mode` values that are either
[irregular](https://pkg.go.dev/io/fs#FileMode.IsRegular) or contain executable
bits (such as `"0777"`) will be rejected for security purposes.
