...
When a OpenBao client reads the ACME config (`/config/acme`) on a
...
```
$ openbao write pki/config/cluster path=https://cluster-b.openbao.example.com/v1/pki
```
...
Using a OpenBao token, [fetch a new external account
...
```
$ openbao write -f pki/roles/my-role-name/acme/new-eab
...
