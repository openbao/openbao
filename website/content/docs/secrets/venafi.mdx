...
1. Register the `venafi-pki-backend` plugin in the OpenBao
   ...
   ```shell-session
   $ bao write sys/plugins/catalog/secret/venafi-pki-backend \
...
1. Enable the Venafi secrets engine:
   ...
   ```shell-session
   $ bao secrets enable -path=venafi-pki -plugin-name=venafi-pki-backend plugin
   ```
...
1. Configure a Venafi secret that maps a name in OpenBao to connection and authentication
   ...
   `bao path-help venafi-pki/venafi/:name` after creating the secret.

   **Trust Protection Platform**:

   ```shell-session
   $ bao write venafi-pki/venafi/tpp \
   ...
   **Venafi Cloud**:
   ...
   ```shell-session
   $ bao write venafi-pki/venafi/cloud \
...
1. Lastly, configure a [role](/vault/docs/secrets/pki)
   ...
   (for validity), use `bao path-help venafi-pki/roles/:name` after
   creating the role.

   **Trust Protection Platform**:

   ```shell-session
   $ bao write venafi-pki/roles/tpp \
       venafi_secret=tpp \
   ...
   **Venafi Cloud**:
   ...
   ```shell-session
   $ bao write venafi-pki/roles/cloud \
       venafi_secret=cloud \
       store_by=serial store_pkey=true \
       allowed_domains=example.com \
       allow_subdomains=true
...
`bao path-help venafi-pki/issue/:role-name` and
`bao path-help venafi-pki/sign/:role-name`.
...
1. Generate a certificate by writing to the `/issue` endpoint with the name of
   ...
   **Trust Protection Platform**:
   ...
   ```shell-session
   $ bao write venafi-pki/issue/tpp common_name="common-name.example.com" \
   ...
   **Venafi Cloud**:
   ...
   ```shell-session
   $ bao write venafi-pki/issue/cloud common_name="common-name.example.com" \
       alt_names="dns-san-1.example.com,dns-san-2.example.com"
   ```
...
1. Or sign a CSR from a file by writing to the `/sign` endpoint with the name of
   ...
   **Trust Protection Platform**:
   ...
   ```shell-session
   $ bao write venafi-pki/sign/tpp csr=@example.req
   ```
   ...
   **Venafi Cloud**:
   ...
   ```shell-session
   $ bao write venafi-pki/sign/cloud csr=@example.req
   ```
