...
Anything that connects and authenticates to OpenBao to accomplish a task is a
...
![OpenBao Client Workflows](https://www.datocms-assets.com/2885/1617325020-valult-client-workflows.png)

While there are many different potential clients, the most common are:

1. **Human users** interacting directly with OpenBao.
...
1. **OpenBao agents and proxies** that act on behalf of an application or
   microservice.

## Identity and entity assignment

Authorized clients can connect to OpenBao with a variety of authentication methods.

Authorization source       | AuthN method
-------------------------- | ---------------------------------
Externally managed or SSO  | Active Directory, LDAP, OIDC, JWT, GitHub, username+password
Platform- or server-based  | Kubernetes, Cert
Self                       | AppRole, tokens with no associated authN path or role

![OpenBao client types](https://www.datocms-assets.com/2885/1617325030-vault-clients.png)

When a client authenticates, OpenBao assigns a unique identifier
(**client entity**) in the [OpenBao identity system](/docs/secrets/identity)
...
OpenBao treats all ACME connections that authenticate under the same certificate
...
A namespace represents a isolated, logical space within a single OpenBao
cluster and is typically used for administrative purposes.

When a client authenticates **within a given namespace**, OpenBao assigns the same
client entity to activities within any child namespaces because the namespaces
exist within the same larger scope.

When a client authenticates **across namespace boundaries**, OpenBao treats the
...
OpenBao uses tokens as the core method for authentication. You can use tokens to
authenticate directly, or use token [auth methods](/docs/concepts/auth)
to dynamically generate tokens based on external identities.

When clients authenticate with the [token auth method](/docs/auth/token)
...
In **rare** cases, tokens may be created outside of the OpenBao identity system
**without** an associated entity or identity. OpenBao treats every unaffiliated
...
:::note[Behavior of non-entity tokens]
...
<Tabs>
<TabItem value="Related concepts" heading="Related concepts">

<ul>
  <li>
    <a href="/docs/concepts/client-count/counting">Client count calculation</a>
  </li>
  <li>
    <a href="/docs/concepts/client-count/faq">Client count FAQ</a>
  </li>
</ul>
</TabItem>

<TabItem value="Related tutorials" heading="Related tutorials">
<ul>
  <li>
    <a href="/tutorials/auth-methods/identity">Identity: Entities and Groups</a>
  </li>
</ul>
</TabItem>

<TabItem value="Other resources" heading="Other resources">
<ul>
  <li>
    Article: 
    <a href="https://www.hashicorp.com/identity-based-security-and-low-trust-networks">Identity-based Security and Low-trust Networks</a>
  </li>
</ul>
</TabItem>
</Tabs>
