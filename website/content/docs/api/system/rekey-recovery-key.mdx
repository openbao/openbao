...
Note: As of v2.4.0 usage of these endpoints is [deprecated](/community/deprecation/unauthed-rekey)
...
see the documentation around the `disable_unauthed_rekey_endpoints` [listener parameter](../../configuration/listener/unix.mdx).
...
operation, not the entire rekey attempt. The return value is the same as `GET`
