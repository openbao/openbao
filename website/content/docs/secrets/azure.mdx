...
description: |-
  The Azure OpenBao secrets engine dynamically generates Azure
...
```shell-session
$ openbao write azure/roles/my-role \
...
```shell-session
$ openbao write azure/roles/my-role ttl=1h azure_roles=-<<EOF
    [
        {
...
```shell-session
$ openbao read azure/creds/my-role
...
```shell-session
$ openbao write azure/roles/my-role permanently_delete=true ttl=1h azure_roles=-<<EOF
    [
        {
            "role_name": "Contributor",
            "scope":  "/subscriptions/<uuid>/resourceGroups/Website"
...
To enable the Azure debug logs, set the `AZURE_GO_SDK_LOG_LEVEL`  environment variable to `DEBUG` on your OpenBao
server:
...
The Azure secrets engine is written as an external OpenBao plugin and
