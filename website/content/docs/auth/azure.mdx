...
```shell-session
$ openbao write auth/azure/login \
    role="dev-role" \
...
```shell-session
$ openbao write auth/azure/login role="dev-role" \
...
1. Enable Azure authentication in OpenBao:
   ...
   ```shell-session
   $ openbao auth enable azure
   ```

1. Configure the Azure auth method:

   ```shell-session
   $ openbao write auth/azure/config \
...
1. Create a role:
   ...
   ```shell-session
   $ openbao write auth/azure/role/dev-role \
       policies="prod,dev" \
...
import (
	...
	auth "github.com/lf-edge/openbao/api/auth/azure"
)

// Fetches a key-value secret (kv-v2) after authenticating to OpenBao via Azure authentication.
