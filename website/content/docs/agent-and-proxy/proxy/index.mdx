...
Valid values include: `auto-auth`, and `proxying`. Can also be configured by setting the `BAO_PROXY_DISABLE_IDLE_CONNECTIONS`
...
Valid values include: `auto-auth`, and `proxying`. Can also be configured by setting the `BAO_PROXY_DISABLE_KEEP_ALIVES`
environment variable as a comma separated string. This environment variable will override any values found in a configuration file.
...
This value can be overridden by setting the `BAO_ADDR` environment variable.
...
be overridden by setting the `BAO_CACERT` environment variable.
...
This value can be overridden by setting the `BAO_CAPATH` environment
variable.
...
This value can be overridden by setting the `BAO_CLIENT_CERT` environment
variable.
...
This value can be overridden by setting the `BAO_CLIENT_KEY` environment
variable.
...
be overridden by setting the `BAO_SKIP_VERIFY` environment variable.
...
`BAO_TLS_SERVER_NAME` environment variable.
...
A value of `-1` disables retries. The environment variable `BAO_MAX_RETRIES`
...
Alternatively, you can specify the configuration path using the `BAO_PROXY_CONFIG_PATH` environment variable. If the `-config` flag is provided, the environment variable is ignored.

**Example using environment variable:**

```shell-session
$ export BAO_PROXY_CONFIG_PATH=/etc/openbao/proxy-config.hcl
$ bao proxy
```
