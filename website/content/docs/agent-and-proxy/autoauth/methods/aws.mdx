---
layout: docs
page_title: OpenBao Auto-Auth AWS Method
description: AWS Method for OpenBao Auto-Auth
---

# OpenBao Auto-Auth AWS method
...
OpenBao will use the AWS SDK's normal credential chain behavior, which means it
...
- `credential_poll_interval` `(integer: optional)` - In seconds, how frequently the OpenBao agent should check for new credentials if using the iam type.
...
- `region` `(string: "us-east-1")` - The region to use for signing the authentication request. The
  ...
  if a custom endpoint has been configured on the OpenBao server.
...
AWS](/vault/tutorials/vault-agent/agent-aws) tutorial to learn how to integrate OpenBao Agent with IAM AWS' native authentication.
