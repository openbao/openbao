...
func TestLoadConfigFile_AgentCache_NoListeners(t *testing.T) {
	...
	expected := &Config{
		...
		Templates: []*ctconfig.TemplateConfig{
			{
				Source:      new("/path/on/disk/to/template.ctmpl"),
			...
			expectedTemplates: []*ctconfig.TemplateConfig{
				{
					...
					CreateDestDirs: new(true),
					Command:        []string{"restart service foo"},
					Perms:          new(os.FileMode(0o600)),
				},
				{
					Source:      new("/path/on/disk/to/template2.ctmpl"),
					Destination: new("/path/on/disk/where/template/will/render2.txt"),
					Backup:      new(true),
