...
func TestPassthroughBackend_RootPaths(t *testing.T) {
	...
	test(b)
	b = testPassthroughLeasedBackend(t)
	test(b)
}
...
func TestPassthroughBackend_Revoke(t *testing.T) {
	...
	t.Run("passthrough", func(t *testing.T) {
		test(t, testPassthroughBackend(t))
	})
	t.Run("passthrough-leased", func(t *testing.T) {
		test(t, testPassthroughLeasedBackend(t))
	})
}

func testPassthroughBackend(t *testing.T) logical.Backend {
	t.Helper()
	b, _ := PassthroughBackendFactory(t.Context(), &logical.BackendConfig{
		Logger: nil,
...
func testPassthroughLeasedBackend(t *testing.T) logical.Backend {
	t.Helper()
	b, _ := LeasedPassthroughBackendFactory(t.Context(), &logical.BackendConfig{
		Logger: nil,
		System: logical.StaticSystemView{
			DefaultLeaseTTLVal: time.Hour * 24,
			MaxLeaseTTLVal:     time.Hour * 24 * 32,
