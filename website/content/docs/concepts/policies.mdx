...
![OpenBao Auth Workflow](/img/openbao-policy-workflow.png)
...
![OpenBao Auth Workflow](/img/openbao-auth-workflow.png)
...
2. OpenBao establishes a connection to LDAP and asks the LDAP server to verify the
...
3. OpenBao maps the result from the LDAP server to policies inside OpenBao using the
...
4. OpenBao returns the token to the user. This token has the correct policies
...
Here is a very simple policy which grants read capabilities to the [KVv1](../api/secret/kv/kv-v1.mdx) path
`"secret/foo"`:
...
Ensure that any `path` segments refer to an engine's canonicalized variants
(usually lower-case if the backend is case-insensitive). While inbound requests
will have their paths canonicalized before being checked against ACLs, any paths
specified in ACLs themselves will not be and will mismatch if not in canonical
form.
...
- `sudo` <a name="sudo"></a> - Allows access to paths that are _root-protected_. Tokens are not
...
See the [API Specification](../api/secret/kv/kv-v2.mdx) for more information.
...
conforming to the [Paginated Lists RFC](/community/rfcs/paginated-lists). This
...
$ curl \
  ...
  --header "X-Vault-Token: ..." \
  --data '{"policy":"path \"...\" {...} "}' \
  http://127.0.0.1:8200/v1/sys/policy/my-existing-policy
```
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| [auth/token/accessors](../api/auth/token.mdx#list-accessors)                                                                                      | LIST              | List token accessors for all current OpenBao service tokens                                                                                           |
| [auth/token/create](../api/auth/token.mdx#create-token)                                                                                           | POST              | Create a periodic or an orphan token (`period` or `no_parent`) option                                                                               |
| [pki/root](../api/secret/pki.mdx#delete-all-issuers-and-keys)                                                                                     | DELETE            | Delete the current CA key ([pki secrets engine](../secrets/pki/index.mdx))                                                                           |
| [pki/root/sign-self-issued](../api/secret/pki.mdx#sign-self-issued)                                                                               | POST              | Use the configured CA certificate to sign a self-issued certificate ([pki secrets engine](../secrets/pki/index.mdx))                                 |
| [sys/audit](../api/system/audit.mdx)                                                                                                              | GET               | List enabled audit devices                                                                                                                          |
| [sys/audit/:path](../api/system/audit.mdx)                                                                                                        | POST, DELETE      | Enable or remove an audit device                                                                                                                    |
| [sys/auth/:path](../api/system/auth.mdx)                                                                                                          | GET, POST, DELETE | Manage the auth methods (enable, read, and delete)                                                                                                  |
| [sys/auth/:path/tune](../api/system/auth.mdx#tune-auth-method)                                                                                    | GET, POST         | Manage the auth methods (enable, read, delete, and tune)                                                                                            |
| [sys/config/auditing/request-headers](../api/system/config-auditing.mdx)                                                                          | GET               | List the request headers that are configured to be audited                                                                                          |
| [sys/config/auditing/request-headers/:name](../api/system/config-auditing.mdx)                                                                    | GET, POST, DELETE | Manage the auditing headers (create, update, read and delete)                                                                                       |
| [sys/config/cors](../api/system/config-cors.mdx)                                                                                                  | GET, POST, DELETE | Configure CORS setting                                                                                                                              |
| [sys/config/ui/headers](../api/system/config-ui.mdx)                                                                                              | GET, LIST         | Configure the UI settings                                                                                                                           |
| [sys/config/ui/headers/:name](../api/system/config-ui.mdx#name)                                                                                   | POST, DELETE      | Configure custom HTTP headers to be served with the UI                                                                                              |
| [sys/internal/inspect/router/:tag](../api/system/inspect/router.mdx)                                                                              | GET               | Inspect the internal components of OpenBao's router. `tag` must be one of root, uuid, accessor, or storage                                            |
| [sys/leases/lookup/:prefix](../api/system/leases.mdx#list-leases)                                                                                 | LIST              | List lease IDs                                                                                                                                      |
| [sys/leases/revoke-force/:prefix](../api/system/leases.mdx#revoke-force)                                                                          | POST              | Revoke all secrets or tokens ignoring backend errors                                                                                                |
| [sys/leases/revoke-prefix/:prefix](../api/system/leases.mdx#revoke-prefix)                                                                        | POST              | Revoke all secrets generated under a given prefix                                                                                                   |
| [sys/plugins/catalog/:type/:name](../api/system/plugins-catalog.mdx#register-plugin)                                                              | GET, POST, DELETE | Register a new plugin, or read/remove an existing plugin                                                                                            |
| [sys/raw:path](../api/system/raw.mdx)                                                                                                             | GET, POST, DELETE | Used to access the raw underlying store in OpenBao                                                                                                    |
| [sys/raw:prefix](../api/system/raw.mdx#list-raw)                                                                                                  | GET, LIST         | Returns a list keys for a given path prefix                                                                                                         |
| [sys/remount](../api/system/remount.mdx)                                                                                                          | POST              | Moves an already-mounted backend to a new mount point                                                                                               |
| [sys/rotate](../api/system/rotate/index.mdx)                                                                                                            | POST              | Trigger a rotation of the backend encryption key                                                                                                    |
| [sys/seal](../api/system/seal.mdx)                                                                                                                | POST              | Seals the OpenBao                                                                                                                                     |
| [sys/step-down](../api/system/step-down.mdx)                                                                                                      | POST              | Forces a node to give up active status                                                                                                              |
