...
```shell-session
$ openbao read sys/mounts/pki/tune
Key                             Value
---                             -----
...
```shell-session
$ openbao secrets tune -default-lease-ttl=18h -audit-non-hmac-request-keys=common_name -audit-non-hmac-response-keys=serial_number pki/
...
```shell-session
$ openbao secrets tune -audit-non-hmac-request-keys=common_name -audit-non-hmac-request-keys=ttl pki/
```
