...
- `path` `(string: "")` <a name="path"></a> – The file system path where all the OpenBao data gets
  stored.
  This value can be overridden by setting the `BAO_RAFT_PATH` environment variable.

- `node_id` `(string: "")` - The identifier for the node in the Raft cluster.
  This value can be overridden by setting the `BAO_RAFT_NODE_ID` environment variable.
...
- `retry_join_as_non_voter` `(boolean: false)` <a name="retry_join_as_non_voter"></a> - If set, causes any `retry_join`
  ...
  [`-non-voter`](../../commands/operator/raft.mdx#_non_voter) flag for the
  ...
  `BAO_RAFT_RETRY_JOIN_AS_NON_VOTER` environment variable to any non-empty value.
  Only valid if there is at least one `retry_join` stanza.

- `max_entry_size` `(integer: 1048576)` <a name="max_entry_size"></a> - This configures the maximum number of
...
- `max_transaction_size` `(integer: 8388608)` - This configures the maximum
  number of bytes for a Raft entry containing a [transaction](/community/rfcs/transactions).
...
- `leader_tls_servername` `(string: "")` <a name="leader_tls_servername"></a> - The TLS server name to use when
