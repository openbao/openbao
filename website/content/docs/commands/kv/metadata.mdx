...
```text
Usage: openbao kv metadata <subcommand> [options] [args]
...
```shell-session
$ openbao kv metadata delete -mount=secret creds
...
```shell-session
$ openbao kv metadata put -mount=secret creds
...
```shell-session
$ openbao kv metadata put -mount=secret -max-versions=5 creds
Success! Data written to: secret/metadata/creds
```
...
```shell-session
$ openbao kv metadata put -mount=secret -cas-required creds
```
...
```shell-session
$ openbao kv metadata put -mount=secret -delete-version-after="3h25m19s" creds
```
