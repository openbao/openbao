...
endpoint](../../api/secret/pki.mdx#generate-intermediate-csr). Here, when creating
...
endpoint](../../api/secret/pki.mdx#sign-intermediate)) with `cert A`'s reference
...
[using the `/issuers/import/cert` endpoint](../../api/secret/pki.mdx#import-ca-certificates-and-keys).
...
will have a `ca_chain` response field [during read](../../api/secret/pki.mdx#read-issuer)
...
To fix this, update the `manual_chain` field on the [issuers](../../api/secret/pki.mdx#update-issuer)
...
endpoint](../../api/secret/pki.mdx#generate-root). This allows the generation of a new
root certificate with the existing key material (via the `key_ref` request parameter).
If this process succeeded, when [reading the issuer](../../api/secret/pki.mdx#read-issuer)
...
process:
 ...
 1. Use the [`/issuers/generate/intermediate/existing`
    endpoint](../../api/secret/pki.mdx#generate-intermediate-csr)
 ...
 3. Finally, use the [`/intermediate/set-signed` endpoint](../../api/secret/pki.mdx#import-ca-certificates-and-keys)
    to import the signed certificate from step 2.

If the process to reissue an intermediate certificate succeeded, when
[reading the issuer](../../api/secret/pki.mdx#read-issuer) (via
...
1. [Generate](../../api/secret/pki.mdx#generate-root) the new root
