...
plugin processes may be terminated by OpenBao. These cases include, but are not
limited to:
...
```shell-session
$ openbao plugin register -sha256=<SHA256 Hex value of the plugin binary> \
    secret \                  # type
...
```shell-session
$ openbao write sys/plugins/catalog/database/myplugin-database-plugin \
