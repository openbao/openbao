...
- `token_label` `(string: <optional>)`: The token label to use. May also be
  specified by the `BAO_HSM_TOKEN_LABEL` environment variable.
...
- `pin` `(string: <optional>)`: The PIN for `CKU_USER` login. May also
  be specified by the `BAO_HSM_PIN` environment variable. _If set via the
  environment variable, it will need to be re-set if OpenBao is restarted._

- `key_label` `(string: <required>)`: The label (`CKA_LABEL`) of the key or
  key pair to use. May also be specified by the `BAO_HSM_KEY_LABEL` environment
  variable.

- `key_id` `(string: "")`: The ID (`CKA_ID`) of the key or key pair to use. The
  value should be a hexadecimal string (e.g., `"0x33333435363434373537"`). May
  also be specified by the `BAO_HSM_KEY_ID` environment variable.
...
| Thales       | Thales Luna 7 HSM   | Yes         | [Installation Guide](../../guides/unseal/pkcs11/luna7.mdx)    |
