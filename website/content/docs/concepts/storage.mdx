...
As described on our [Architecture](/vault/docs/internals/architecture) page, OpenBao's
...
Many other options for storage are available with community support for OpenBao - see our
...
OpenBao cluster can extend across multiple availability zones within a region.

Vault Enterprise supports replicated clusters and disaster recovery for data
center failure. When using OpenBao in [HA
...
To perform a backup or restore of OpenBao's encrypted data when using a
...
backend is encrypted, some of your configuration may be sensitive (a OpenBao token
