...
1. Enable and configure JWT auth in OpenBao.
  ...
  1. If OpenBao is running in Kubernetes:
     ...
     ```bash
     kubectl exec openbao-0 -- openbao auth enable jwt
     kubectl exec openbao-0 -- openbao write auth/jwt/config \
  ...
  1. Alternatively, if OpenBao is _not_ running in Kubernetes:
     ...
     openbao write auth/jwt/config oidc_discovery_url="${ISSUER}"
     ```
...
1. Create a role for JWT auth that the `default` service account from the
   ...
   ```bash
   openbao write auth/jwt/role/my-role \
      role_type="jwt" \
