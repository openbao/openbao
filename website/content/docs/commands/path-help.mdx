...
Paths are the parameters used for `openbao read`, `openbao write`, etc. An example
...
To discover what paths are supported, use `openbao path-help PATH`. For example,
if you enabled the AWS secrets engine, you can use `openbao path-help aws` to find
...
Once you've found a path you like, you can learn more about it by using `openbao path-help <path>` where "path" is a path that matches one of the regular
expressions from the backend help.

```shell-session
$ openbao path-help secret/password
Request:        password
Matching Route: ^.*$
