...
please see the [LDAP secrets engine docs](../../secrets/ldap.mdx).
...
- `password_policy` `(string: <optional>)` - The name of the [password policy](../../concepts/password-policies.mdx)
...
- If `password_policy` is set, the password will be generated from the associated
  [password policy](../../concepts/password-policies.mdx). The policy is not exercised prior to saving the configuration.
...
See [LDAP secrets engine docs](../../secrets/ldap.mdx) for additional information.
...
- `rotation_period` `(string: <required>)` - How often OpenBao should rotate the password of the user entry. Accepts
  [duration format strings](../../concepts/duration-format.mdx). The minimum rotation period is 5 seconds.<br />
  **Example:** `"3600", "5s", "1h"`
...
- `default_ttl` `(string/int)` - Specifies the TTL for the leases associated with this role. Accepts
  [duration format strings](../../concepts/duration-format.mdx). Defaults to system/engine default TTL time.
- `max_ttl` `(string/int)` - Specifies the maximum TTL for the leases associated with this role. Accepts
  [duration format strings](../../concepts/duration-format.mdx). Defaults to system/mount default TTL time;
...
[Username Templating](../../concepts/username-templating.mdx) for details on how to use templating. Also see
...
[password policies](../../concepts/password-policies.mdx))
