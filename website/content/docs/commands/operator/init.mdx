...
description: |-
  The "operator init" command initializes a OpenBao server. Initialization is the
...
```shell-session
$ openbao operator init -root-token-pgp-key="keybase:hashicorp"
```
...
- `-consul-auto` `(bool: false)` - Perform automatic service discovery using
  Consul in HA mode. When all nodes in a OpenBao HA cluster are registered with
  Consul, enabling this option will trigger automatic service discovery based on
  the provided -consul-service value. When Consul is OpenBao's HA backend, this
