...
description: This is the API documentation for the OpenBao Active Directory secrets engine.
---
...
This is the API documentation for the OpenBao AD secrets engine. For general
information about the usage and operation of the AD secrets engine, please see
the [OpenBao Active Directory documentation](/vault/docs/secrets/ad).
...
- `binddn` (string, required) - Distinguished name of object to bind when performing user and group search. Example: `cn=openbao,ou=Users,dc=example,dc=com`
...
- `last_rotation_tolerance` (string, optional) - Tolerance duration to use when checking the last rotation time.
  Active Directory often shows a "pwdLastSet" time after OpenBao's because it takes
  ...
  within 5 seconds of OpenBao's, OpenBao considers itself to have been the last entity that rotated the password. However, if it's been
  more than 5 seconds, OpenBao thinks that something rotated the password out-of-band, and re-rotates it so it will "know" it and be
...
The `roles` endpoint configures how OpenBao will manage the passwords for individual service accounts.
...
When adding a role, OpenBao verifies its associated service account exists.
...
Performing a `LIST` on the `/ad/roles` endpoint will list the names of all the roles OpenBao contains.
...
```shell-session
$ openbao read ad/creds/my-application
```
...
When adding a service account to the library, OpenBao verifies it already exists in Active Directory.
...
Performing a `LIST` on the `/ad/library` endpoint will list the names of all the sets of service accounts OpenBao contains.
...
Rotate the `bindpass` to a new one known only to OpenBao.

### Risks

1. When the `bindpass` is rotated, it successfully gets rotated in Active Directory but OpenBao can't store it so it becomes unknown.
...
2. Ensure the `binddn` in use only applies to one entity by including all distinguished name parameters possible. For example, use `"CN=openbao-ad-test,CN=Users,DC=example,DC=com"` instead of `"CN=openbao-ad-test"`.
