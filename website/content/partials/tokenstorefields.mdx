...
- `token_strictly_bind_ip` `(bool: false)` - If set, the token will be
  restricted to the source IP address making the initial login request. This
  conflicts with `token_bound_cidrs`.
...
- `token_period` `(integer: 0 or string: "")` - The maximum allowed [period](/docs/concepts/tokens#token-time-to-live-periodic-tokens-and-explicit-max-ttls) value when a periodic token is requested from this role.
