...
func (b *FileBackend) DeleteInternal(ctx context.Context, path string) error {
	...
	if nsPath, err := namespace.WithNS(ctx, path); err != nil {
		return err
	} else {
		path = nsPath
	}
...
func (b *FileBackend) GetInternal(ctx context.Context, k string) (*physical.Entry, error) {
	if nsPath, err := namespace.WithNS(ctx, k); err != nil {
		return nil, err
	} else {
		k = nsPath
	}
...
func (b *FileBackend) PutInternal(ctx context.Context, entry *physical.Entry) error {
	nsPath, err := namespace.WithNS(ctx, entry.Key)
	if err != nil {
		return err
	}

	b.logger.Trace("FileBackend insert", "entry key", entry.Key, "namespace", nsPath)
	if err := b.validatePath(nsPath); err != nil {
		return err
	}
	path, key := b.expandPath(nsPath)
...
func (b *FileBackend) ListPageInternal(ctx context.Context, prefix string, after string, limit int) ([]string, error) {
	if nsPath, err := namespace.WithNS(ctx, prefix); err != nil {
		return nil, err
	} else {
		prefix = nsPath
	}
