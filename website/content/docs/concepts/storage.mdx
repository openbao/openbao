...
There is currently no built-in support for automated snapshots. However, OpenBao snapshots can be automated in various ways such as using a cron job.

We maintain [a repository](https://github.com/openbao/openbao-snapshot-agent) containing a collection of external tools and instructions for automating snapshots. 

Currently for [VMs, we rely on systemd unit files](https://github.com/openbao/openbao-snapshot-agent/blob/main/docs/vm-configuration.md) and cronjob while for [Kubernetes we provide a container image and cronjob](https://github.com/openbao/openbao-snapshot-agent/blob/main/docs/k8s-configuration.md). The Kubernetes cronjob can also be configured and deployed via the [OpenBao Helm chart](https://github.com/openbao/openbao-helm/blob/main/charts/openbao/values.yaml#L1414).
