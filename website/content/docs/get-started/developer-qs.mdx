...
!> **Warning**: This in-memory “dev” server is useful for practicing with OpenBao locally for the first time, but is insecure and **should never be used in production**. For developers who need to manage their own production OpenBao installations, this [page](/tutorials/operations/production-hardening) provides some guidance on how to make your setup more production-friendly.
