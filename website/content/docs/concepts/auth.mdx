...
```shell-session
$ openbao write sys/auth/my-auth type=userpass
```
...
```shell-session
$ openbao path-help auth/my-auth
# ...
```
...
To authenticate with the CLI, `openbao login` is used. This supports many
of the built-in auth methods. For example, with GitHub:

```shell-session
$ openbao login -method=github token=<token>
...
```
...
auth method implements its own login endpoint. Use the `openbao path-help`
mechanism to find the proper endpoint.

For example, the GitHub login endpoint is located at `auth/github/login`.
And to determine the arguments needed, `openbao path-help auth/github/login` can
be used.
...
completely reauthenticate. Just use `openbao token renew <token>` with the
...
//
// Additionally, enterprise OpenBao users should be aware that due to eventual
// consistency, the API may return unexpected errors when running OpenBao with
