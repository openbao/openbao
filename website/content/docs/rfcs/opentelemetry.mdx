...
It is probably save to say that OpenTelemetry won the format war on telemetry
...
We should first add the OpenTelemetry based metrics and deprecated the
[`go-metrics`][] based metrics in one release and in a later release remove the
old metrics.
...
One could use [Vector][] with an [OpenTelemetry
Source][vector-otel-source] and a [Statsd Sink][vector-statsd-sink]
(untested).
