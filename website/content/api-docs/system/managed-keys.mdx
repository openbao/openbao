...
description: The `/sys/managed-keys` endpoint is used to manage the managed keys in OpenBao.
---
...
The `/sys/managed-keys` endpoint is used to manage the Managed Key configuration within OpenBao.
...
- `allow_generate_key` `(string: "false")` - If no existing key can be found in the referenced backend, instructs
  OpenBao to generate a key within the backend.

- `allow_replace_key` `(string: "false")` - Controls the ability for OpenBao to replace through generation or importing
  a key into the configured backend even if a key is present, if set to false those operations are forbidden
  if a key exists.

- `allow_store_key` `(string: "false")` - Controls the ability for OpenBao to import a key
...
- `library` `(string: <required>)` - The name of the `kms_library` stanza to use from OpenBao's config to
...
- `endpoint` `(string: "")`: Used to specify a custom AWS endpoint. If not set, OpenBao will use
...
- `kms_key` `(string: <required>)`: An identifier for the key. If `allow_generate_key` is set to
  ...
  alias does not already exist, OpenBao will generate a key and create an alias for it using this parameter.
...
- `crypto_key` `(string: <required>)`: The name of the GCP Cloud KMS key. If there is no existing key
  and `allow_generate_key` is `true`, OpenBao will generate a key with this name.
