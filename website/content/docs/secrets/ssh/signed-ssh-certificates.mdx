...
on every path, use `openbao path-help` after mounting the secrets engine.

## Client key signing

Before a client can request their SSH key be signed, the OpenBao SSH secrets engine must
be configured. Usually a OpenBao administrator or security team performs these
...
The following steps are performed in advance by a OpenBao administrator, security
...
1.  Mount the secrets engine. Like all secrets engines in OpenBao, the SSH secrets engine
    ...
    ```text
    $ openbao secrets enable -path=ssh-client-signer ssh
...
1.  Configure OpenBao with a CA for signing client keys using the `/config/ca`
    ...
    ```text
    $ openbao write ssh-client-signer/config/ca generate_signing_key=true
    Key             Value
    ---             -----
    ...
    ```text
    $ openbao write ssh-client-signer/config/ca \
        private_key="..." \
        public_key="..."
    ```
...
1.  Add the public key to all target host's SSH configuration. This process can
    ...
    ```text
    $ openbao read -field=public_key ssh-client-signer/config/ca > /etc/ssh/trusted-user-ca-keys.pem
    ```
...
1.  Create a named OpenBao role for signing client keys.
    ...
    ```text
    $ openbao write ssh-client-signer/roles/my-role -<<"EOH"
    {
...
1.  Ask OpenBao to sign your **public key**. This file usually ends in `.pub` and
    ...
    ```text
    $ openbao write ssh-client-signer/sign/my-role \
    ...
    ```text
    $ openbao write ssh-client-signer/sign/my-role -<<"EOH"
    {
...
1.  Save the resulting signed, public key to disk. Limit permissions as needed.
    ...
    ```text
    $ openbao write -field=signed_key ssh-client-signer/sign/my-role \
...
1.  Mount the secrets engine. For the most security, mount at a different path from the
    ...
    ```text
    $ openbao secrets enable -path=ssh-host-signer ssh
...
1.  Configure OpenBao with a CA for signing host keys using the `/config/ca`
    ...
    ```text
    $ openbao write ssh-host-signer/config/ca generate_signing_key=true
    Key             Value
    ---             -----
    public_key      ssh-rsa AAAAB3NzaC1yc2EA...
    ```
    ...
    ```text
    $ openbao write ssh-host-signer/config/ca \
        private_key="..." \
        public_key="..."
    ```
...
1.  Extend host key certificate TTLs.
    ...
    ```text
    $ openbao secrets tune -max-lease-ttl=87600h ssh-host-signer
    ```
...
1.  Create a role for signing host keys. Be sure to fill in the list of allowed
    ...
    ```text
    $ openbao write ssh-host-signer/roles/hostrole \
        key_type=ca \
        algorithm_signer=rsa-sha2-256 \
        ttl=87600h \
...
1.  Sign the host's SSH public key.
    ...
    ```text
    $ openbao write ssh-host-signer/sign/hostrole \
        cert_type=host \
...
1.  Set the resulting signed certificate as `HostCertificate` in the SSH
    ...
    ```text
    $ openbao write -field=signed_key ssh-host-signer/sign/hostrole \
        cert_type=host \
...
1.  Retrieve the host signing CA public key to validate the host signature of
    ...
    ```text
    $ openbao read -field=public_key ssh-host-signer/config/ca
    ```
...
1.  Set `default_user` in the role. If you are always authenticating as the same
    ...
    ```text
    $ openbao write ssh/roles/my-role -<<"EOH"
    {
...
1.  Set `valid_principals` during signing. In situations where multiple users may
    be authenticating to SSH via OpenBao, set the list of valid principles during key
...
```shell-extension
# Using CLI:
openbao secrets enable -path=hosts-ca ssh
