...
$ bao write auth/cf/roles/my-role \
...
```shell-session
$ bao login -method=cf role=test-role
```
...
$ bao write auth/vault-plugin-auth-cf/config identity_ca_certificates="$CURRENT" identity_ca_certificates="$FUTURE"
```
