...
The Vault Secrets Operator may also be installed in OpenShift using the Helm chart. (See [Installation](/docs/platform/k8s/vso/installation) for an overview of installation using the [Helm chart](/docs/platform/k8s/vso/helm).) The examples below show example [values.yaml files](https://helm.sh/docs/chart_template_guide/values_files/) for each configuration, which would be used with `helm install` as below:
...
These values would use the [default operator image](/docs/platform/k8s/vso/helm#v-controller-manager-image) from HashiCorp's [Docker Hub repository][dockerhub].
