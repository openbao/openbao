...
description: The Zookeeper storage backend is used to persist OpenBao's data in Zookeeper.
---
...
- `path` `(string: "openbao/")` – Specifies the path in Zookeeper where data will
  be stored.
...
all nodes. In this scenario, anyone connected to Zookeeper could change OpenBao’s
znodes and, potentially, take OpenBao out of service.
...
- `znode_owner` `(string: "")` – If specified, OpenBao will always set all
...
This example shows configuring OpenBao to communicate with a Zookeeper
...
### zNode OpenBao user only

This example instructs OpenBao to set an ACL on all of its zNodes which permit
access only to the user "openbaoUser". As per Zookeeper's ACL model, the digest
value in `znode_owner` must match the user in `znode_owner`.

```hcl
storage "zookeeper" {
  znode_owner = "digest:openbaoUser:raxgVAfnDRljZDAcJFxznkZsExs="
  auth_info   = "digest:openbaoUser:abc"
}
```

### zNode localhost only

This example instructs OpenBao to only allow access from localhost. As this is the
...
This example instructs OpenBao to connect to Zookeeper using the provided TLS configuration. The host verification will happen with the presented certificate using the servers IP because 'tls_verify_ip' is set to true.

```hcl
storage "zookeeper" {
  address = "host1.com:5200,host2.com:5200,host3.com:5200"
  path = "openbao_path_on_zk/"
  znode_owner = "digest:openbao_user:digestvalueforpassword="
  auth_info = "digest:openbao_user:thisisthepassword"
