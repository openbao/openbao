...
<Note>
  The information below relates to the MongoDB Altas <b>database plugin</b> for the OpenBao database secrets engine.
  Refer to the <a href="vault/docs/secrets/mongodbatlas">MongoDB Atlas secrets engine</a>
  for information about using the MongoDB Atlas secrets engine for the OpenBao.
</Note>
## Capabilities
...
1.  Configure OpenBao with the proper plugin and connection information:
    ...
    ```shell-session
    $ openbao write database/config/my-mongodbatlas-database \
...
1.  Configure a role that maps a name in OpenBao to a MongoDB Atlas command that executes and
    creates the database user credential:

    ```shell-session
    $ openbao write database/roles/my-password-role \
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```shell-session
    $ openbao read database/creds/my-password-role
        Key                Value
        ---                -----
...
1.  Configure a role that maps a name in OpenBao to a MongoDB Atlas command that executes and
    ...
    ```shell-session
    $ openbao write database/roles/my-dynamic-certificate-role \
      db_name=my-mongodbatlas-database \
...
of the role:
    ...
    ```shell-session
    $ openbao read database/creds/my-dynamic-certificate-role
      Key                 Value
      ---                 -----
