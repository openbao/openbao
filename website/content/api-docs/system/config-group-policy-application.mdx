...
<Warning>
  ...
  affects OpenBao sentinel role governing policies (RGPs) for the following OpenBao
  versions:
...
mode of policy application for identity groups in OpenBao. This setting dictates
the behavior across all groups in all namespaces in OpenBao.

OpenBao allows you to add entities and groups from any namespace into an identity group.
However, historically, any policies attached to that group would only apply when the
OpenBao token authorizing a request was created in the same namespace as that group,
