...
This auth plugin is available as an external auth plugin in the
...
3. While logged into your client machine, verify you can reach OpenBao
   through the following command: `$ curl $BAO_ADDR/v1/sys/health`.
...
[Kerberos auth method API](../api/auth/kerberos.mdx) for more
details.
