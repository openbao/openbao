...
OpenBao supports Multi-factor Authentication (MFA) for authenticating to
...
Login MFA is built on top of the Identity system of OpenBao.
...
- `Time-based One-time Password (TOTP)` - If configured and enabled on a login path,
  this would require a TOTP passcode along with a OpenBao token to be presented
...
```shell-session
$ openbao write sys/mfa/validate -format=json @payload.json
```
...
```shell-session
$ openbao write -non-interactive sys/mfa/validate -format=json @payload.json
```
