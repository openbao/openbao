---
layout: docs
page_title: OpenBao Auto-Auth OCI (Oracle Cloud Infrastructure) Method
description: OCI (Oracle Cloud Infrastructure) Method for OpenBao Auto-Auth
---

# OpenBao Auto-Auth OCI (Oracle Cloud infrastructure) method
...
If `apikey` is used, the OpenBao agent will use the first credential it can successfully obtain in the following order:
...
- `credential_poll_interval` `(duration: "60s", optional)` - In seconds, how frequently the OpenBao auto-auth method should check for new credentials.
