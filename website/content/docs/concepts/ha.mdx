...
the bottleneck of OpenBao is the data store itself, not OpenBao core. For example:
to increase the scalability of OpenBao with Consul, you would generally scale
Consul instead of OpenBao.
...
with different backends. For instance, a OpenBao cluster can be set up to use
...
[`api_addr`](/vault/docs/configuration#api_addr) at `https://a.openbao.mycompany.com`,
and vice-versa.
...
Each [`listener`](/vault/docs/configuration/listener) block in OpenBao's configuration
...
implementing the
[`physical.HABackend`](https://pkg.go.dev/github.com/lf-edge/openbao/sdk/physical#HABackend)
