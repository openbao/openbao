...
When using Auto Unseal, the recovery key share(s) can be provided to rotate
the root key since no unseal keys are available. Upon successful rotation, no
unseal key share(s) are returned.
...
auto-unseal mechanism](../../../configuration/self-init.mdx), no recovery
