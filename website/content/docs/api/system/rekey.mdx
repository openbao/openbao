...
When using Auto Unseal, the recovery key share(s) can be provided to rekey
the root key since no unseal keys are available. Upon successful rekey, no unseal
key share(s) are returned.
