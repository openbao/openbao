...
func TestVersionedKV_Metadata_Put(t *testing.T) {
	...
	if resp.Data["current_metadata_version"] != uint64(0) {
		t.Fatalf("Expected initial current_metadata_version to be 0, got: %v", resp.Data["current_metadata_version"])
	}
	...
	if resp.Data["current_metadata_version"] != uint64(0) {
		t.Fatalf("Expected current_metadata_version to still be 0, got: %v", resp.Data["current_metadata_version"])
	}
	...
	data = map[string]interface{}{
		...
		// This will not match the current metadata version which is 1
		"metadata_cas": 0,
	}
...
func TestVersionedKV_Metadata_Patch_MetadataCasRequiredWarning(t *testing.T) {
	...
	req = &logical.Request{
		...
		Data: map[string]interface{}{
			"max_versions": 5,
			"metadata_cas": 0, // Required because global setting
		},
	}
