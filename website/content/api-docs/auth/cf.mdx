...
description: This is the API documentation for the OpenBao Cloud Foundry auth method.
---
...
This is the API documentation for the OpenBao CF auth method. For
general information about the usage and operation of the CF method, please
see the [OpenBao CF method documentation](/vault/docs/auth/cf).
...
on how to obtain these values, please see the [OpenBao CF method
...
- `cf_api_trusted_certificates` `(array: [])`: The certificate that's presented by the
  CF API. This configures OpenBao to trust this certificate when making API calls, resolving
...
Create a role in OpenBao granting a particular level of access to a particular group
...
OpenBao provides both an agent and a CLI tool for logging in that
