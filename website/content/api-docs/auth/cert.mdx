...
Certificate method, please see the [OpenBao TLS Certificate method documentation](/vault/docs/auth/cert).
...
- `ocsp_query_all_servers` `(bool: false)` - If set to true, rather than accepting
  ...
  ~> **Note**: When using OpenBao's PKI engine with Performance Replication clusters
