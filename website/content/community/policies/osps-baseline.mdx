...
OpenBao uses GitHub repository rulesets to protect all important project repositories. The rulsets in place do not allow direct pushes to to the default as well as `release/*` branches.
...
OpenBao uses GitHub repository rulesets to protect all important project repositories. The rulesets in place do not allow force pushes to the default as well as `release/*` branches. In addition, GitHub does not allow deleting the default branch.
...
OpenBao relies on GitHub for git and website hosting. GitHub repositories are only accessible via HTTPS and SSH. GitHub pages are delivered using HTTPS.
...
There are three main platforms that can be used to get in touch and participate in a public discussion.

1. GitHub issues and pull requests can be created and commented on by anyone with a GitHub account.
2. Zulip is a free to use chat hosted by the Linux Foundation and used by the OpenBao community for general discussions. Check out the [contributing](https://openbao.org/community/contributing/) page of the documentation.
3. General feedback to the maintainers (and everyone else subscribed) can be provided via openbao@lists.openssf.org mailing list.
...
OpenBao is released under Mozilla Public License 2.0, which is [compatible to GPLv3](https://www.gnu.org/licenses/license-list.html#apache2) and considered an ["Approved License" by the open source initiative (OSI)](https://opensource.org/license/apache-2.0).
...
Every [OpenBao release on GitHub](https://github.com/openbao/openbao/releases) contains a source code ZIP file archive. This archive contains a snapshot of the source code at release time and therefore the corresponding LICENSE file. Binary release artifact tarballs include the license, too.
...
Additional repositories are listed in the [security-insights.yml](https://github.com/openbao/openbao/blob/main/.github/security-insights.yml) of the main project repository.
