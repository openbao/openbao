...
description: >-
  The Redis ElastiCache plugin for OpenBao's database secrets engine generates new passwords for ElastiCache users.
---
...
- `access_key_id` `(string)` – Specifies the IAM `access_key_id` for OpenBao to use. If omitted, authentication falls back on
