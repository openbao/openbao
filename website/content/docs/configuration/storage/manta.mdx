...
description: >-
  The Manta storage backend is used to persist OpenBao's data in Triton's Manta
  Object
...
The Manta storage backend is used to persist OpenBao's data in [Triton's Manta Object
...
```hcl
storage "manta" {
  directory    = "openbao-storage-directory"
  max_parallel = 512
}
```
