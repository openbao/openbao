---
layout: docs
page_title: OpenBao Agent Template
description: >-
...
or environment variables (via the [Process Supervisor Mode](/docs/agent-and-proxy/agent/process-supervisor))
...
In another example `template_config` with [`error_on_missing_key` parameter in the template stanza](/docs/agent-and-proxy/agent/template#error_on_missing_key)
...
The parameter
[`error_on_missing_key`](/docs/agent-and-proxy/agent/template#error_on_missing_key) can be
...
- `error_on_missing_key` `(bool: false)` - Exit with an error when accessing
  ...
  to "true". Also see [`exit_on_retry_failure` in global OpenBao Agent Template Config](/docs/agent-and-proxy/agent/template#interaction-between-exit_on_retry_failure-and-error_on_missing_key).
...
Unlike [OpenBao Agent caching](/docs/agent-and-proxy/agent/caching), the behavior of how OpenBao Agent
...
This can be configured using Template config [static_secret_render_interval](/docs/agent-and-proxy/agent/template#static_secret_render_interval).
...
[Process Supervisor Mode](/docs/agent-and-proxy/agent/process-supervisor)
