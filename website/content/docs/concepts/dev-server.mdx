...
You can start OpenBao as a server in "dev" mode like so: `openbao server -dev`.
This dev-mode server requires no further setup, and your local `openbao` CLI will
...
- **Initialized and unsealed** - The server will be automatically initialized
  and unsealed. You don't need to use `openbao operator unseal`. It is ready
  for use immediately.
...
- **Automatically Authenticated** - The server stores your root access
  token so `openbao` CLI access is ready to go. If you are accessing OpenBao
