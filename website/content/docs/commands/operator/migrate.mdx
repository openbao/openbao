...
```shell-session
$ openbao operator migrate -config migrate.hcl
...
```shell-session
$ openbao operator migrate -config migrate.hcl -start "data/logical/fd"
```
...
to that used to [configure OpenBao](/vault/docs/configuration/storage),
