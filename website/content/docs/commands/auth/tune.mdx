...
```shell-session
$ openbao read sys/auth/github/tune
Key                  Value
---                  -----
...
```shell-session
$ openbao auth tune -default-lease-ttl=72h github/
...
```shell-session
$ openbao auth tune -default-lease-ttl=-1 github/
Success! Tuned the auth method at: github/
```
...
```shell-session
$ openbao auth tune -audit-non-hmac-request-keys=value1 -audit-non-hmac-request-keys=value2 github/
Success! Tuned the auth method at: github/
```
...
```shell-session
$ openbao auth tune -user-lockout-threshold=10  -user-lockout-duration=10m userpass/
...
```shell-session
$ openbao read sys/auth/userpass/tune
