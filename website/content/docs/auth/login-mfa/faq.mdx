...
| [Okta Auth MFA](/docs/auth/okta#mfa)           | This is MFA as part of [Okta Auth method](/docs/auth/okta) in OpenBao, where MFA is enforced by Okta on login. MFA must be satisfied for authentication to be successful. This is different from the Okta MFA method used with Login MFA. CLI/API login are supported. | MFA is managed externally by Okta | 
...
| [Okta Auth MFA](/docs/auth/okta)               | N/A       | N/A                                                                                                                                       | MFA is not managed by OpenBao | MFA is not managed by OpenBao |
