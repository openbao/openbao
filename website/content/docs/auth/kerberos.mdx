...
a OpenBao token for Kerberos entities.
...
```shell-session
$ openbao write auth/kerberos/config \
...
```shell-session
$ openbao write auth/kerberos/config/ldap \
...
```shell-session
$ openbao write auth/kerberos/groups/engineering-team \
    policies=engineers
```
