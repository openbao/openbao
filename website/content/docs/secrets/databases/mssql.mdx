...
```shell-session
$ bao write azuresql/config/testopenbao \
    plugin_name=mssql-database-plugin \
...
When we no longer need the backend, we can unmount it with `bao unmount azuresql`. Now, you can use the MSSQL Database Plugin with your Azure SQL databases.
...
```shell
bao write database/roles/my-role revocation_statements="\
   USE my_database \
   IF EXISTS \
     (SELECT name \
