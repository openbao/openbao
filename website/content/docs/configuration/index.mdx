...
- `storage` <code>([StorageBackend][storage-backend]: &lt;required&gt;)</code> <a name="storage"></a> –
...
- `ha_storage` <code>([StorageBackend][storage-backend]: nil)</code> <a name="ha_storage"></a> – Configures
...
- `plugin_file_uid` `(integer: 0)` – Uid of the plugin directories and plugin binaries if they
  ...
  file permissions check is enabled via the environment variable `BAO_ENABLE_FILE_PERMISSIONS_CHECK`.
...
- `plugin_file_permissions` `(string: "")` – Octal permission string of the plugin
  ...
  `BAO_ENABLE_FILE_PERMISSIONS_CHECK`.
...
- `plugin_auto_register` `(bool: true)` - Whether automatic plugin registration is enabled. See
  [OCI plugin documentation][oci-plugins] for more information.
...
- `default_lease_ttl` `(string: "768h")` <a name="default_lease_ttl"></a> – Specifies the default lease duration
...
- `max_lease_ttl` `(string: "768h")` <a name="max_lease_ttl"></a> – Specifies the maximum possible lease
  ...
  [auth](../commands/auth/tune.mdx#_max_lease_ttl) or
  [secret](../commands/secrets/tune.mdx#_max_lease_ttl) commands.

- `default_max_request_duration` `(string: "90s")` <a name="default_max_request_duration"></a> – Specifies the default
...
- `ui` `(bool: false)` <a name="ui"></a> – Enables the built-in web UI, which is available on all
  ...
  via the environment variable `BAO_UI`. For more information, please see the
...
- `allow_unauthenticated_workflows` `(bool: false)` - Whether to allow execution
  of unauthenticated workflows.

- `disable_ssct_tokens` `(bool: true)` - Whether to disable SSCT token
  generation. This is disabled by default as they have limited utility: while
  they enforce that the token is present locally on the standby node (returning
  a 412 Precondition Failed otherwise), they do not help with subsequent
  requests. Use of index headers should be preferred.

### High Availability parameters

The following parameters are used on backends that support [High Availability][high-availability].

- `api_addr` `(string: "")` <a name="api_addr"></a> – Specifies the address (full URL) to advertise to
  ...
  environment variable `BAO_API_ADDR`. In general this should be set as a full
...
- `cluster_addr` `(string: "")` <a name="cluster_addr"></a> – Specifies the address to advertise to other
  OpenBao servers in the cluster for request forwarding. This can also be provided
  via the environment variable `BAO_CLUSTER_ADDR`. This is a full URL, like
