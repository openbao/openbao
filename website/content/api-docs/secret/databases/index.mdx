...
please see the
[OpenBao database secrets engine documentation](/vault/docs/secrets/databases).
...
~> We highly recommended that you use a OpenBao-specific user rather than the admin user
...
```shell-session
$ openbao write database/config/mssql \
...
1-to-1 mapping of a OpenBao Role to a user in a database which are automatically
