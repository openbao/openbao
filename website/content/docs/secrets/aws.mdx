...
description: |-
  The AWS secrets engine for OpenBao generates access keys dynamically based on
  IAM policies.
---
...
and are automatically revoked when the OpenBao lease expires.

OpenBao supports three different types of credentials to retrieve from AWS:

1. `iam_user`: OpenBao will create an IAM user for each lease, attach the managed
   ...
   OpenBao will then generate an access key and secret key for the IAM user and
   return them to the caller. IAM users have no session tokens and so no
   session token will be returned. OpenBao will delete the IAM user upon reaching the TTL expiration.
2. `assumed_role`: OpenBao will call
   [sts:AssumeRole](https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html)
   and return the access key, secret key, and session token to the caller.
3. `federation_token`: OpenBao will call
...
a 1-to-1 mapping of OpenBao Roles to IAM users. The current password
for the user is stored and automatically rotated by OpenBao on a
...
When credentials are requested for the Role, OpenBao returns the current
Access Key ID and Secret Access Key for the configured user, allowing anyone with the proper
OpenBao policies to have access to the IAM credentials.
...
1.  Configure the credentials that OpenBao uses to communicate with AWS to generate
    the IAM credentials:

    ```text
    $ openbao write aws/config/root \
        access_key=AKIAJWVN5Z4FOFT7NLNA \
        secret_key=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 \
        region=us-east-1
    ```

    Internally, OpenBao will connect to AWS using these credentials. As such,
    these credentials must be a superset of any policies which might be granted
    on IAM credentials. Since OpenBao uses the official AWS SDK, it will use the
    ...
    credentials. (Note that you can't authorize OpenBao with IAM role credentials if you plan
...
1.  Configure a OpenBao role that maps to a set of permissions in AWS as well as an
    AWS credential type. When users generate credentials, they are generated
    against this role. An example:

    ```text
    $ openbao write aws/roles/my-role \
    ...
    this role, OpenBao will create an IAM user and attach the specified policy
    document to the IAM user. OpenBao will then create an access key and secret
    ...
    ```text
    $ openbao write aws/roles/my-other-role \
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```text
    $ openbao read aws/creds/my-role
    Key                Value
    ---                -----
...
1.  Rotate the credentials that OpenBao uses to communicate with AWS:

    ```test
    $ openbao write -f aws/config/rotate-root
    Key           Value
    ---           -----
    ...
    `aws/config/rotate-root` endpoint, subsequent calls from OpenBao to
...
## Example IAM policy for OpenBao
  ...
  "Statement": [
    {
      ...
      "Resource": ["arn:aws:iam::ACCOUNT-ID-WITHOUT-HYPHENS:user/openbao-*"]
    }
  ]
}
```

OpenBao also supports AWS Permissions Boundaries when creating IAM users. If you
wish to enforce that OpenBao always attaches a permissions boundary to an IAM
...
boundary policies that you wish to ensure that OpenBao uses. This policy will
ensure that OpenBao uses one of the permissions boundaries specified (not all of
them).

## STS credentials

The above demonstrated usage with `iam_user` credential types. As mentioned,
OpenBao also supports `assumed_role` and `federation_token` credential types.

### STS federation tokens

~> **Notice:** Due to limitations in AWS, in order to use the `federation_token`
credential type, OpenBao **must** be configured with IAM user credentials. AWS
...
2. The user inline policy configured in the OpenBao role
3. The managed policy ARNs configured in the OpenBao role
...
the `policy_document`, `policy_arns`, and `iam_groups` parameters in the OpenBao
role.
...
```shell-session
$ openbao write aws/roles/ec2_admin \
...
```shell-session
$ openbao write aws/sts/ec2_admin ttl=60m
Key            	Value
...
`assumed_role` credential type, you must configure outside of OpenBao:

1. An IAM role
2. IAM inline policies and/or managed policies attached to the IAM role
3. IAM trust policy attached to the IAM role to grant privileges for OpenBao to
   assume the role
...
3. Temporary credentials (such as those granted by running OpenBao on an EC2
      ...
      "Statement": [
        {
          "Effect": "Allow",
          "Principal": {
            "AWS": "arn:aws:iam::ACCOUNT-ID-WITHOUT-HYPHENS:user/OPENBAO-AWS-ROOT-CONFIG-USER-NAME"
          },
          "Action": "sts:AssumeRole"
        }
      ]
...
When specifying a OpenBao role with a `credential_type` of `assumed_role`, you can
specify more than one IAM role ARN. If you do so, OpenBao clients can select which
...
assumed, the `policy_document` specified on the OpenBao role (if specified), and
...
can specify any of the role ARNs that are defined on the OpenBao role in order to
...
```shell-session
$ openbao write aws/roles/deploy \
...
```shell-session
$ openbao write aws/sts/deploy ttl=60m
Key            	Value
...
```shell-session
$ openbao read aws/creds/deploy
* Error creating IAM user: User: arn:aws:iam::000000000000:user/hashicorp is not authorized to perform: iam:CreateUser on resource: arn:aws:iam::000000000000:user/openbao-root-1432735386-4059

$ openbao revoke aws/creds/deploy/774cfb27-c22d-6e78-0077-254879d1af3c
...
If you get stuck at any time, simply run `openbao path-help aws` or with a subpath for
interactive help output.

### STS federated token errors

OpenBao generates STS tokens using the IAM credentials passed to `aws/config`.
...
<Note title="AWS character limit includes path">
  ...
  The AWS character limit for token names **includes** the full path to
   the token. For example, `aws/sts/dev005_openbao-test_testtest` (34
   characters) exceeds the limit , but `aws/roles/dev005_openbao-test` (29
   characters) does not.
