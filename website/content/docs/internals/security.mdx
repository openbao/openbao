...
- Protecting against flaws in clients or systems that access OpenBao. If an attacker
  can compromise a OpenBao client (e.g., system, browser) and obtain this client’s OpenBao
...
- Protecting against OpenBao administrators supplying vulnerable or malicious configuration
  ...
  (e.g. [secret engines](/vault/docs/secrets) configurations), or OpenBao's
...
All server-to-server traffic between OpenBao instances within a cluster (i.e,
...
consuming. Opening the bank openbao requires two-factors: the key and the
