...
1. Create a Google Cloud KMS key:
   ...
   ```text
   $ openbao write gcpkms/keys/my-key \
...
1.  Encrypt plaintext data using the `/encrypt` endpoint with a named key:
    ...
    ```text
    $ openbao write gcpkms/encrypt/my-key plaintext="hello world"
    Key            Value
    ---            -----
...
1.  Decrypt ciphertext using the `/decrypt` endpoint with a named key:
    ...
    ```text
    $ openbao write gcpkms/decrypt/my-key ciphertext=CiQAuMv0lTiKjrF43Lgr4...
    Key          Value
    ---          -----
    ...
    ```text
    $ openbao write -field=plaintext gcpkms/decrypt/my-key ciphertext=CiQAuMv0lTiKjrF43Lgr4...
    hello world
    ```
...
1.  Rotate the underlying encryption key. This will generate a new crypto key
    ...
    ```text
    $ openbao write -f gcpkms/keys/rotate/my-key
...
1.  Re-encrypt already-encrypted ciphertext to be encrypted with a new version of
    ...
    ```text
    $ openbao write gcpkms/reencrypt/my-key ciphertext=CiQAuMv0lTiKjrF43Lgr4...
    Key            Value
    ---            -----
...
1.  Trim old key versions by deleting Cloud KMS crypto key versions that are
    ...
    ```text
    $ openbao write gcpkms/keys/config/my-key min_version=10
    ```
    ...
    ```text
    $ openbao write -f gcpkms/keys/trim/my-key
    ```

1.  Delete the key to delete all key versions and OpenBao's record of the key.

    ```text
    $ openbao delete gcpkms/keys/my-key
    ```
...
1.  Decrypt ciphertext using the `/decrypt` endpoint with a named key:
    ...
    ```text
    $ openbao write gcpkms/decrypt/my-key key_version=1 ciphertext=CiQAuMv0lTiKjrF43Lgr4...
    Key          Value
    ---          -----
    plaintext    hello world
    ```
...
1.  Calculate the base64-encoded binary digest. Use the hashing algorithm that
    ...
    ```text
    $ openbao write gcpkms/sign/my-key key_version=1 digest=$DIGEST
    Key          Value
    ---          -----
...
1.  Verify the signature of the digest:
    ...
    ```text
    $ openbao write gcpkms/verify/my-key key_version=1 digest=$DIGEST signature=$SIGNATURE
    Key      Value
    ---      -----
    valid    true
    ```
...
The Google Cloud KMS OpenBao secrets engine is written as an external OpenBao
plugin. The code lives outside the main OpenBao repository. It is automatically
bundled with OpenBao releases, but the code is managed separately.
...
[openbao-transit]: /vault/docs/secrets/transit
