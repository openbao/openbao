...
description: >-
  The OCI Auth method for OpenBao enables authentication and authorization using
  OCI Identity credentials.
---

# OCI auth method

The OCI Auth method for OpenBao enables authentication and authorization using [OCI Identity](https://docs.cloud.oracle.com/iaas/Content/Identity/Concepts/overview.htm) credentials.
...
- A role defined in OpenBao can be mapped to many groups and dynamic groups.
- A single role can be mapped to both groups and dynamic groups.
- A OpenBao policy can be mapped from different roles.
...
### Configure the OCI tenancy to run OpenBao

The OCI Auth method requires [instance principal](https://blogs.oracle.com/cloud-infrastructure/announcing-instance-principals-for-identity-and-access-management) credentials to call OCI Identity APIs, and therefore the OpenBao server needs to run inside an OCI compute instance.

Follow the steps below to add policies to your tenancy that allow the OCI compute instance in which the OpenBao server is running to call certain OCI Identity APIs.

1. In your tenancy, [launch the compute instance(s)](https://docs.cloud.oracle.com/iaas/Content/Compute/Tasks/launchinginstance.htm) that will run the OpenBao server. The [VCN](https://docs.cloud.oracle.com/iaas/Content/Network/Tasks/managingVCNs.htm) in which you launch the Compute Instance should have a [Service Gateway](https://docs.cloud.oracle.com/iaas/Content/Network/Tasks/servicegateway.htm) added to it .
1. Make a note of the Oracle Cloud Identifier (OCID) of the compute instance(s) running OpenBao.
1. In your tenancy, [create a dynamic group](https://docs.cloud.oracle.com/iaas/Content/Identity/Tasks/managingdynamicgroups.htm) with the name OpenBaoDynamicGroup to contain the computer instance(s).
...
1. Add the following policies to the root compartment of your tenancy that allow the dynamic group to call specific Identity APIs.
   ...
   ```plaintext
   allow dynamic-group OpenBaoDynamicGroup to {AUTHENTICATION_INSPECT} in tenancy
   allow dynamic-group OpenBaoDynamicGroup to {GROUP_MEMBERSHIP_INSPECT} in tenancy
   ```
...
```shell-session
$ openbao auth enable oci
```

Then, configure your home tenancy in the OpenBao, so that only users or instances from your tenancy will be allowed to log into OpenBao through the OCI Auth method. 
...
1. Configure the `home_tenancy_id` parameter in the OpenBao.
...
Continue by creating a OpenBao administrator role in the OCI Auth method. The `openbaoadminrole` allows the administrator of OpenBao to log into OpenBao and grants them the permissions allowed in the policy.

1. Create a file named `openbaoadminrole.json` with the below contents. Replace the `ocid_list` with the
Group or Dynamic Group OCIDs in your tenancy that has users or instances that you want to take the OpenBao admin role.

   - For testing in dev mode, you can add the OCID of the dynamic group previously created.
   - In production, add only the OCID of groups and dynamic groups that can take the admin role in OpenBao.

      ```json
      {
         "token_policies": "openbaoadminpolicy",
         "token_ttl": "1800",
...
1. Create the OpenBao admin role:

   ```shell-session
   $ curl --header "X-Vault-Token: $roottoken" --request POST \
      --data @openbaoadminrole.json \
      http://127.0.0.1:8200/v1/auth/oci/role/openbaoadminrole (127.0.0.1:8200/v1/auth/oci/role/openbaoadminrole)
   ```

### Log in to OpenBao using OCI auth
...
- The VAULT_ADDR export has to be specified, as shown earlier in this page; when testing in dev mode in the same compute instance that the OpenBao is running on, this is [http://127.0.0.1:8200](http://127.0.0.1:8200/).

#### Log in with instance principals

```shell-session
$ openbao login -method=oci auth_type=instance role=openbaoadminrole
```

This assumes that the compute instance that you are logging in from should be a part of a dynamic group that was added to the OpenBao admin role. If logging on from a different compute instance than the one on which OpenBao is running on, the compute should have connectivity to the endpoint specified in VAULT_ADDR. 

#### Log in with an API key

```shell-session
$ openbao login -method=oci auth_type=apikey role=openbaoadminrole
```
...
1. [Add an API Key](https://docs.cloud.oracle.com/iaas/Content/API/Concepts/apisigningkey.htm) for a user in the console. This user should be part of a group that has previously been added to the OpenBao admin role.
...
1. Ensure that the region in the config matches the region of the compute instance that is running OpenBao.


### Manage roles in the OCI auth method

1.  Similar to creating the OpenBao administrator role, create other roles mapped to other policies. Create a file named devrole.json with the following contents. Replace ocid_list with Groups or Dynamic Groups in your tenancy.
...
3.  Login to OpenBao assuming the devrole.

   ```shell-session
   $ openbao login -method=oci auth_type=instance role=devrole
   ```

## Authentication

You can authenticate with the OpenBao CLI or by communicating with the API directly.
...
```shell-session
$ openbao login -method=oci auth_type=apikey role=devrole
```

### Via the API

1.  Sign the following request with your OCI credentials and obtain the signing string and the authorization header. Replace the endpoint, scheme (http or https) & role of the URL corresponding to your OpenBao configuration. For more information on signing, see [signing the request](https://docs.cloud.oracle.com/iaas/Content/API/Concepts/signingrequests.htm).
...
1.  Add the signed headers to the "request_headers" field and make the actual request to OpenBao. For example:
