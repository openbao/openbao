...
```shell-session
$ openbao secrets enable -version=1 kv
```
...
1. Write arbitrary data:
   ...
   ```shell-session
   $ openbao kv put kv/my-secret my-value=s3cr3t
...
1. Delete a key:
   ...
   ```shell-session
   $ openbao kv delete kv/my-secret
...
1. Write data using the `example` policy:
   ...
   ```shell-session
   $ openbao kv put kv/my-generated-secret \
...
1. Read the generated data:
   ...
   ```shell-session
   $ openbao kv get kv/my-generated-secret
   ====== Data ======
   Key         Value
   ---         -----
...
```shell-session
$ openbao kv put kv/my-secret ttl=30m my-value=s3cr3t
Success! Data written to: kv/my-secret
```
