...
!> **Warning**: This in-memory “dev” server is useful for practicing with OpenBao locally for the first time, but is insecure and **should never be used in production**. For developers who need to manage their own production OpenBao installations, this [page](/vault/tutorials/operations/production-hardening) provides some guidance on how to make your setup more production-friendly.
...
```go
import openbao "github.com/openbao/openbao/api"
```
...
A variety of [authentication methods](/vault/docs/auth) can be used to prove
...
A common way of storing secrets is as key-value pairs using the [KV secrets engine (v2)](/vault/docs/secrets/kv/kv-v2). In the code we've just added, `password` is the key in the key-value pair, and `OpenBao123` is the value.
...
To learn how to integrate applications with OpenBao without needing to always change your application code, see the [OpenBao Agent](/vault/docs/agent-and-proxy/agent) documentation.
