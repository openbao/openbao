...
description: >-
  The AliCloud secrets engine for OpenBao generates access tokens or STS
  credentials
...
The AliCloud access tokens are time-based and are automatically revoked when the OpenBao
...
1.  Enable the AliCloud secrets engine:
    ...
    ```text
    $ openbao secrets enable alicloud
...
1.  [Create a custom policy](https://www.alibabacloud.com/help/doc-detail/28640.htm)
    in AliCloud that will be used for the access key you will give OpenBao. See "Example
    RAM Policy for OpenBao".

1.  [Create a user](https://www.alibabacloud.com/help/faq-detail/28637.htm) in AliCloud
    with a name like "openbao", and directly apply the new custom policy to that user
...
1.  Configure that access key as the credentials that OpenBao will use to communicate with
    AliCloud to generate credentials:

    ```text
    $ openbao write alicloud/config \
    ...
    changes in credentials will be picked up almost immediately without a OpenBao restart.

    If available, we recommend using instance metadata for these credentials as they are the most
    secure option. To do so, simply ensure that the instance upon which OpenBao is running has sufficient
...
1.  Configure a role describing how credentials will be granted.
    ...
    ```text
    $ openbao write alicloud/role/policy-based \
    ...
    ```text
    $ openbao write alibaba/role/role-based \
...
### Example RAM policy for OpenBao
...
1.  Generate a new access key by reading from the `/creds` endpoint with the name
    ...
    ```text
    $ openbao read alicloud/creds/policy-based
    Key                Value
    ---                -----
    ...
    ```text
    $ openbao read alicloud/creds/role-based
    Key                Value
    ---                -----
