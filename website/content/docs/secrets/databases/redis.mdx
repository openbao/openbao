...
1.  Configure OpenBao with the proper plugin and connection configuration:
    ...
    ```shell-session
    $ openbao write database/config/my-redis-database \
...
1.  You should consider rotating the admin password. Note that if you do, the
    ...
    create a OpenBao-specific database admin user for this.

    ```shell-session
    openbao write -force database/rotate-root/my-redis-database
    ```
...
1.  Configure a dynamic role that maps a name in OpenBao to a JSON string
    ...
    ```shell-session
    $ openbao write database/roles/my-dynamic-role \
...
1.  Generate a new set of credentials by reading from the `/creds` endpoint with the name
    ...
    ```shell-session
    $ openbao read database/creds/my-dynamic-role
    Key                Value
    ---                -----
...
1.  Configure a static role that maps a name in OpenBao to an existing Redis
    user.

    ```shell-session
    $ openbao write database/static-roles/my-static-role \
        db_name="my-redis-database" \
...
1.  Retrieve the credentials from the `/static-creds` endpoint:
    ...
    ```shell-session
    $ openbao read database/static-creds/my-static-role
    Key                    Value
    ---                    -----
    last_openbao_rotation    2020-12-20T10:39:49.647822-06:00
