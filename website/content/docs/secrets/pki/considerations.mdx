...
For backwards compatibility, [the default issuer](../../api/secret/pki.mdx#read-issuers-configuration)
...
some considerations to be aware of:
 ...
 - Storage is an important factor: with [BYOC Revocation](../../api/secret/pki.mdx#revoke-certificate),
...
Roles](../../api/secret/pki.mdx#allowed_domains) and the [`permitted_dns_domains`
parameter to set the Name Constraints extension](../../api/secret/pki.mdx#permitted_dns_domains)
...
URLs](../../api/secret/pki.mdx#enable_aia_url_templating)to allow ACME
...
option](../../api/secret/pki.mdx#tidy). Orders, authorizations, and
challenges are [cleaned up based on the
`safety_buffer`](../../api/secret/pki.mdx#safety_buffer)
parameter, but accounts can live longer past their last issued certificate
by controlling the [`acme_account_safety_buffer`
parameter](../../api/secret/pki.mdx#acme_account_safety_buffer).
...
To solve this issue, there are two possible approaches:
 ...
 1. Use a restrictive [`allowed_roles`, `allowed_issuers`, and
    ...
    configuration](../../api/secret/pki.mdx#set-acme-configuration)
 ...
 2. Use a more permissive [configuration with
    `eab_policy=always-required`](../../api/secret/pki.mdx#eab_policy)
...
configuration](../../api/secret/pki.mdx#eab_policy).  System administrators of
OpenBao instances can enforce this by [setting the
`BAO_DISABLE_PUBLIC_ACME=true` environment
variable](../../api/secret/pki.mdx#acme-external-account-bindings).
...
issuers](../../api/secret/pki.mdx#leaf_not_after_behavior).  This allows
...
However, once these certificates are expired, a [tidy operation](../../api/secret/pki.mdx#tidy)
...
Note: The PKI Secret Engine's [Bring-Your-Own-Cert
   (BYOC)](../../api/secret/pki.mdx#revoke-certificate) functionality allows
...
a [custom CRL can be built](../../api/secret/pki.mdx#combine-crls-from-the-same-issuer)
...
[Roles](../../api/secret/pki.mdx#createupdate-role).
