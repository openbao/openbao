...
Note: As of v2.4.0 usage of these endpoints will be disabled by default [deprecation notice](/docs/deprecation/unauthed-rekey).
To view new authenticated version of the rekey endpoints, please visit [rotation page](/api-docs/system/rotate).
To re-enable, see the documentation around the `disable_unauthed_rekey_endpoints` [listener parameter](/docs/configuration/listener/unix/).
