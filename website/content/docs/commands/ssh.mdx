...
- `-strict-host-key-checking` `(string: "")` - Value to use for the SSH
  ...
  also be specified via the `BAO_SSH_STRICT_HOST_KEY_CHECKING` environment
  variable.
...
- `-user-known-hosts-file` `(string: "~/.ssh/known_hosts")` - Value to use for
  ...
  via the `BAO_SSH_USER_KNOWN_HOSTS_FILE` environment variable.
...
- `-host-key-hostnames` `(string: "*")` - List of hostnames to delegate for the
  ...
  `BAO_SSH_HOST_KEY_HOSTNAMES` environment variable.
...
- `-host-key-mount-point` `(string: "")` - Mount point to the SSH
  ...
  `BAO_SSH_HOST_KEY_MOUNT_POINT` environment variable.
