...
description: |-
  The MongoDB Atlas Secrets Engine for OpenBao generates MongoDB Atlas
...
time-based and are automatically revoked when the OpenBao lease expires, unless renewed.

OpenBao will create a Programmatic API key for each lease that provide appropriate access to the defined MongoDB Atlas
...
[Programmatic API Keys Doc](https://docs.atlas.mongodb.com/reference/api-docs/apiKeys/).
  ...
  for information about using the MongoDB Atlas database plugin for the OpenBao
...
1. Enable the MongoDB Atlas Secrets Engine:
   ...
   ```bash
   $ openbao secrets enable mongodbatlas
...
1. It's necessary to generate and configure a MongoDB Atlas Programmatic API Key for your organization
   or project that has sufficient permissions to allow OpenBao to create other Programmatic API Keys.

   In order to grant OpenBao programmatic access to an organization or project using only the
...
1. Once you have a MongoDB Atlas Programmatic Key pair, as created in the previous step, OpenBao can now
   be configured to use it with MongoDB Atlas:

   ```bash
   $ openbao write mongodbatlas/config \
       public_key=yhltsvan \
       private_key=2c130c23-e6b6-4da8-a93f-a8bf33218830
   ```

   Internally, OpenBao will connect to MongoDB Atlas using these credentials. As such,
...
Programmatic API Key credential types use a OpenBao role to generate a Programmatic API Key at
...
Create a OpenBao role for a MongoDB Atlas Programmatic API Key by mapping appropriate arguments to the
...
```shell-session
$ openbao write mongodbatlas/roles/test \
...
block and IP address are added to the IP Network Access list for Keys generated with this OpenBao role:

```bash
  $ openbao write atlas/roles/test \
      project_id=5cf5a45a9ccf6400e60981b6 \
...
Verify the created Programmatic API Key OpenBao role has the added CIDR block and IP address by running:

```bash
  $ openbao read atlas/roles/test
...
Programmatic API Keys OpenBao have a time-to-live (TTL) and maximum time-to-live (Max TTL).
...
The following creates a OpenBao role "test" for a Project level Programmatic API key with a 2 hour time-to-live and a
...
```shell-session
$ openbao read mongodbatlas/roles/test
...
After a user has authenticated to OpenBao has has sufficient permissions, a read request to the
`creds` endpoint for the role will generate and return new Programmatic API Keys:

```shell-session
$ openbao read mongodbatlas/creds/test
    ...
    description        openbao-test-1563980947-1318
