...
see the [RabbitMQ documentation](../../secrets/rabbitmq.mdx).
...
- `password_policy` `(string: "")` - Specifies a [password policy](../../concepts/password-policies.mdx) to
  use when creating dynamic credentials. Defaults to generating an alphanumeric password if not set.

- `username_template` `(string)` - [Template](../../concepts/username-templating.mdx) describing how
