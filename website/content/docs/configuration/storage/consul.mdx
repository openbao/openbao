...
description: |-
  The Consul storage backend is used to persist OpenBao's data in Consul's
  key-value store. In addition to providing durable storage, inclusion of this
  backend will also register OpenBao as a service in Consul with a default health
  check.
---

# Consul storage backend

The Consul storage backend is used to persist OpenBao's data in [Consul's][consul]
key-value store. In addition to providing durable storage, inclusion of this
backend will also register OpenBao as a service in Consul with a default health
check.
...
Note that if you have configured multiple listeners for OpenBao, you must specify
...
- `path` `(string: "openbao/")` – Specifies the path in Consul's key-value store
...
- `token` `(string: "")` – Specifies the [Consul ACL token][consul-acl] with
  ...
  This is **not** a OpenBao token. This can also be provided via the environment
...
`openbao` and the prefix being used is `openbao/`:
