...
once unmarshaled, consume much more memory than in JSON form (circumventing the
`max_request_size` limit). This limit should not affect valid requests, as even
large requests use a small number of tokens (a large string sill counts as a
single token).
