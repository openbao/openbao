...
```shell-session
$ openbao read identity/entity/id/2f09126d-d161-abb8-2241-555886491d97
```
...
```shell-session
$ openbao read secret/data/customers
```
...
```shell-session
$ openbao kv get -mount=secret customers
```

-> **Comparison:** All three commands retrieve the same data, but display the
output in a different format. By default, `openbao read` prints output in
