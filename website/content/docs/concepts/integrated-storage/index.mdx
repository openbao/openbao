...
```shell-session
$ openbao operator raft join https://node1.openbao.local:8200
```
...
```shell-session
$ openbao operator raft join -non-voter https://node1.openbao.local:8200
```
...
OpenBao exposes two TCP ports: [the API port](/vault/docs/configuration#api_addr) and
...
this problem, a OpenBao node must speak to another OpenBao node using the API port
instead of the cluster port. This is currently the only situation in which
OpenBao does this (Vault Enterprise also does something similar when setting
up replication.)
...
path](/vault/docs/configuration/storage/raft/#path) of each OpenBao server. Inside that
