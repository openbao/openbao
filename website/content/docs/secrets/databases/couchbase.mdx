...
1.  Configure OpenBao with the proper plugin and connection configuration:
    ...
    ```bash
    $ openbao write database/config/my-couchbase-database \
...
1.  You should consider rotating the admin password. Note that if you do, the
    ...
    ```bash
    openbao write -force database/rotate-root/my-couchbase-database
    ```
...
1.  Configure a static role that maps a name in OpenBao to an existing couchbase
    user.
