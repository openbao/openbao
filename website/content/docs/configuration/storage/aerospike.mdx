...
description: |-
  The Aerospike storage backend is used to persist OpenBao's data in an Aerospike
  cluster.
---

# Aerospike storage backend

The Aerospike storage backend is used to persist OpenBao's data in an
...
- **64-bit only** - the Aerospike storage backend is not supported when OpenBao runs
