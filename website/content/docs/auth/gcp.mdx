...
description: |-
  The "gcp" auth method allows users and machines to authenticate to OpenBao using
...
but is automatically bundled in OpenBao releases. Please file all feature
...
$ openbao login -method=gcp role="my-role"
```

For more usage information, run `openbao auth help gcp`.
...
```shell-session
$ openbao write -field=token auth/gcp/login \
    role="my-role" \
...
1. Create a named role:
   ...
   ```shell-session
   $ openbao write auth/gcp/role/my-iam-role \
       type="iam" \
       policies="dev,prod" \
   ...
   ```shell-session
   $ openbao write auth/gcp/role/my-gce-role \
       type="gce" \
       policies="dev,prod" \
...
**For `iam`-type OpenBao roles**, the service account [`credentials`](/vault/api-docs/auth/gcp#credentials)
...
**For `gce`-type OpenBao roles**, the service account [`credentials`](/vault/api-docs/auth/gcp#credentials)
given to OpenBao can have the following role:
...
[![OpenBao Google Cloud IAM Login Workflow](/img/vault-gcp-iam-auth-workflow.svg)](/img/vault-gcp-iam-auth-workflow.svg)
...
[![OpenBao Google Cloud GCE Login Workflow](/img/vault-gcp-gce-auth-workflow.svg)](/img/vault-gcp-gce-auth-workflow.svg)
...
import (
	...
	auth "github.com/lf-edge/openbao/api/auth/gcp"
)

// Fetches a key-value secret (kv-v2) after authenticating to OpenBao
...
func getSecretWithGCPAuthIAM() (string, error) {
	...
	// OpenBao will fall back to Google's default instance credentials.
	...
	if pathToCreds := os.Getenv("GOOGLE_APPLICATION_CREDENTIALS"); pathToCreds == "" {
		fmt.Printf("WARNING: Environment variable GOOGLE_APPLICATION_CREDENTIALS was not set. IAM client for JWT signing and OpenBao server IAM client will both fall back to default instance credentials.\n")
	}
