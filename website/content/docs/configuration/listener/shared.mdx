...
- `consistency_fallback_behavior` `(string: "fail")` - specifies the fallback
  behavior when a client only specifies `X-Vault-Inconsistent: await-state`.
  ...
  (`forward-active-node`). `await-state` is not supported here.
...
- `consistency_max_index_wait` `(string: "0s")` - maximum time the standby
