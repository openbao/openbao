...
- `-openbao-tls-ca-cert` `(string: "")` - (v1.1.0+) Path on disk to a single
  PEM-encoded CA certificate to trust for OpenBao. Takes precendence over
...
- `objects` `(array)` - An array of secrets to retrieve from OpenBao.
 ...
 `secretArgs` are sent as part of the HTTP request body. Therefore, they are only effective for HTTP PUT/POST requests, for instance,
        the [request used to generate a new certificate](/api-docs/secret/pki#generate-certificate).
