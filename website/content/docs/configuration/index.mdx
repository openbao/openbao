...
After the configuration is written, use the `-config` flag with `openbao server`
...
- `cluster_name` `(string: <generated>)` – Specifies the identifier for the
  OpenBao cluster. If omitted, OpenBao will generate a value. When connecting to
...
- `disable_mlock` `(bool: false)` – Disables the server from executing the
  ...
  ```shell
  sudo setcap cap_ipc_lock=+ep $(readlink -f $(which openbao))
  ```
...
- `log_level` `(string: "info")` - Log verbosity level.
  ...
  ~> Note: On SIGHUP (`sudo kill -s HUP` _pid of openbao_), if a valid value is specified, OpenBao will update the existing log level,
