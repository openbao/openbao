...
and managing when an entry is committed. The leader node is also the active OpenBao node and followers are standby nodes. Refer to the [High Availability](/docs/internals/high-availability#design-overview) document for more information.
...
An [Autopilot feature](/docs/concepts/integrated-storage/autopilot) 
...
The [OpenBao Reference Architecture](/tutorials/day-one-raft/raft-reference-architecture#recommended-architecture) 
