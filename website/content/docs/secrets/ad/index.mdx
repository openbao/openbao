...
description: >-
  The Active Directory secrets engine allowing OpenBao to generate dynamic credentials.
---
...
be checked out by a person or by machines. OpenBao will automatically rotate the password
each time a service account is checked in. Service accounts can be voluntarily checked in, or OpenBao
...
2.  Configure the credentials that OpenBao uses to communicate with Active Directory
    to generate passwords:

    ```text
    $ openbao write ad/config \
        binddn=$USERNAME \
        bindpass=$PASSWORD \
    ...
    passwords for these accounts to the one OpenBao is in control of, and this is
...
3.  Configure a role that maps a name in OpenBao to an account in Active Directory.
    When applications request passwords, password rotation settings will be managed by
    this role.

    ```text
    $ openbao write ad/roles/my-application \
...
#### What if someone directly rotates an active directory password that OpenBao is managing?

If an administrator at your company rotates a password that OpenBao is managing,
the next time an application asks _OpenBao_ for that password, OpenBao won't know
it.

To maintain that application's up-time, OpenBao will need to return to a state of
knowing the password. OpenBao will generate a new password, update it, and return
...
for accounts that OpenBao is managing. This may lead to behavior the
...
responsibility of updating passwords can be left to OpenBao, rather than
requiring manual administrator updates.

#### Why does OpenBao return the last password in addition to the current one?
...
this, OpenBao returns the current password with the last password if it's known.
That way, if a new password isn't fully operational, the last password can also
be used.

## Service account Check-Out

OpenBao offers the ability to check service accounts in and out. This is a separate,
...
```shell-session
$ openbao write ad/library/accounting-team \
...
```shell-session
$ openbao read ad/library/accounting-team/status
Key                 Value
---                 -----
...
```shell-session
$ openbao write -f ad/library/accounting-team/check-out
Key                     Value
---                     -----
...
```shell-session
$ openbao write ad/library/accounting-team/check-out ttl=30m
Key                     Value
---                     -----
...
```shell-session
$ openbao lease renew ad/library/accounting-team/check-out/0C2wmeaDmsToVFc0zDiX9cMq
Key                Value
---                -----
...
```shell-session
$ openbao write -f ad/library/accounting-team/check-in
Key          Value
---          -----
...
```shell-session
$ openbao write ad/library/accounting-team/check-in service_account_names=fizz@example.com
Key          Value
---          -----
check_ins    [fizz@example.com]
```
...
To do this, OpenBao looks for either the same [entity ID](/vault/tutorials/auth-methods/identity)
...
```shell-session
$ openbao write -f ad/library/manage/accounting-team/check-in
Key          Value
---          -----
check_ins    [fizz@example.com]
```
...
```shell-session
$ openbao lease revoke ad/library/accounting-team/check-out/PvBVG0m7pEg2940Cb3Jw3KpJ
...
suppose our "accounting-team" service accounts are the ones being requested. When OpenBao
receives a check-out call but none are available, OpenBao will log at debug level:
"'accounting-team' had no check-outs available". OpenBao will also increment a metric
...
#### Sometimes OpenBao gives me a password but then AD says it's not valid.
