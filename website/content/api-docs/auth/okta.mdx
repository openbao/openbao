...
description: This is the API documentation for the OpenBao Okta auth method.
---

# Okta auth method (API)

This is the API documentation for the OpenBao Okta auth method. For
general information about the usage and operation of the Okta method, please
see the [OpenBao Okta method documentation](/vault/docs/auth/okta).
...
- `bypass_okta_mfa` `(bool: false)` - Whether to bypass an Okta MFA request.
  Useful if using one of OpenBao's built-in MFA mechanisms, but this will also
...
requires number verification challenge. Logins through the openbao login CLI
