...
```shell-session
$ openbao login -method=github token="MY_TOKEN"
```
...
1. Enable the GitHub auth method:
   ...
   ```text
   $ openbao auth enable github
   ```

1. Use the `/config` endpoint to configure OpenBao to talk to GitHub.

   ```text
   $ openbao write auth/github/config organization=hashicorp
   ```
...
1. Map the users/teams of that GitHub organization to policies in OpenBao. Team
   ...
   ```text
   $ openbao write auth/github/map/teams/dev value=dev-policy
   ```
   ...
   ***
   ...
   ```text
   $ openbao write auth/github/map/users/sethvargo value=sethvargo-policy
   ```
