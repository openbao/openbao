...
- `allowed_parameters` - A list of keys and values that are
  ...
  - Setting a parameter with a value of the empty list allows the parameter to
    ...
    -> **Usage example:** The [ACL Policy Path
     ...
     method](/vault/docs/auth/userpass) to log in with OpenBao.
...
```shell-session
$ openbao write secret/foo no_store=false value=bar
```
...
$ openbao write secret/foo value=bar
```
...
```shell-session
$ openbao read sys/policy/default
```

To disable attachment of the default policy:

```shell-session
$ openbao token create -no-default-policy
```
...
When a OpenBao server is first initialized, there always exists one root user.
...
~> **Note:** You may also see the CLI command `openbao policies`. This is a convenience
...
```shell-session
$ openbao policy write policy-name policy-file.hcl
```
...
```shell-session
$ openbao write sys/policy/my-existing-policy policy=@updated-policy.json
```
...
```shell-session
$ openbao delete sys/policy/policy-name
```
...
A OpenBao administrator or someone from the security team would create the user in
OpenBao with a list of associated policies:

```shell-session
$ openbao write auth/userpass/users/sethvargo \
    password="s3cr3t!" \
...
```shell-session
$ openbao login -method="userpass" username="sethvargo"
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| [auth/token/accessors](/vault/api-docs/auth/token#list-accessors)                                                                                      | LIST              | List token accessors for all current OpenBao service tokens                                                                                           |
...
| [sys/internal/inspect/router/:tag](/vault/api-docs/system/inspect/router)                                                                              | GET               | Inspect the internal components of OpenBao's router. `tag` must be one of root, uuid, accessor, or storage                                            |
...
| [sys/raw:path](/vault/api-docs/system/raw)                                                                                                             | GET, POST, DELETE | Used to access the raw underlying store in OpenBao                                                                                                    |
...
| [sys/seal](/vault/api-docs/system/seal)                                                                                                                | POST              | Seals the OpenBao                                                                                                                                     |
...
```shell-session
$ openbao token create -policy=dev-readonly -policy=logs
```
...
- [OpenBao Policies](/vault/tutorials/policies/policies)
