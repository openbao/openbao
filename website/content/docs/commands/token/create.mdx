...
```shell-session
$ openbao token create -policy=my-policy -policy=other-policy
Key                Value
---                -----
...
```shell-session
$ openbao token create -period=30m
Key                Value
---                -----
...
- `-wrap-ttl` `(duration: "")` -  Wraps the response in a cubbyhole token with the
  requested TTL. The response is available via the "openbao unwrap" command. The TTL
