...
description: |-
  The MySQL storage backend is used to persist OpenBao's data in a MySQL server or
  cluster.
---

# MySQL storage backend

The MySQL storage backend is used to persist OpenBao's data in a [MySQL][mysql]
server or cluster.
...
- `database` `(string: "openbao")` – Specifies the name of the database. If the
  database does not exist, OpenBao will attempt to create it.

- `table` `(string: "openbao")` – Specifies the name of the table. If the table
  does not exist, OpenBao will attempt to create it.
...
Additionally, OpenBao requires the following authentication information.
...
- `lock_table` `(string: "openbao_lock")` – Specifies the name of the table to
  use for storing high availability information. By default, this is the name
  of the `table` suffixed with `_lock`. If the table does not exist, OpenBao will
