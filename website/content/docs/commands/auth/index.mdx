...
To authenticate to OpenBao as a user or machine, use the [`openbao login`](/vault/docs/commands/login) command instead. This command is for
...
```shell-session
$ openbao auth help userpass/
Usage: openbao login -method=userpass [CONFIG K=V...]
# ...
```
...
```shell-session
$ openbao auth tune -max-lease-ttl=30m userpass/
...
```text
Usage: openbao auth <subcommand> [options] [args]
