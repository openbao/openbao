...
func (c *Core) loadCredentials(ctx context.Context) error {
	...
	// Update entries in memory: we don't sync the root namespace ID to save
	// storage space, but we do want to validate all referenced namespaces
	// exist.
...
func (c *Core) loadLegacyCredentials(ctx context.Context, barrier logical.Storage) (bool, error) {
	...
	if rawLocal != nil {
		...
		if err != nil {
			c.logger.Error("failed to decompress and/or decode the local auth mount table", "error", err)
			return false, err
		}
...
func (c *Core) persistAuth(ctx context.Context, barrier logical.Storage, table *MountTable, local *bool, mount string) error {
	...
	// transactions.
	needTxnCommit := false
	if _, ok := barrier.(logical.TransactionalStorage); ok {
		var err error
		barrier, err = barrier.(logical.TransactionalStorage).BeginTx(ctx)
		if err != nil {
