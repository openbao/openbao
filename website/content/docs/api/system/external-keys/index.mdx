...
- `plugin` `(string: <required>)` - Specifies the name of the KMS plugin to use,
  e.g., `"transit"` or `"pkcs11"`. This corresponds to the name the plugin is
  declared under within the server configuration, or some constant identifier
  if the plugin is built-in. See the [plugin overview](./plugins/index.mdx) for
  more information on this parameter's intersection with OpenBao's plugin system
...
Note that like ACL policies, grants are tied to paths, not to mount identities.
A grant can be given to a mount path that does not exist; neither is a grant
automatically removed when a matching mount is disabled.
