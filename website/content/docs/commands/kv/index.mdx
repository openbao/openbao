...
The path to where the secrets engine is mounted can be indicated with the `-mount` flag, such as `openbao kv get -mount=secret creds`.

The deprecated path-like syntax can also be used (e.g. `openbao kv get secret/creds`), but this should be avoided
...
```shell-session
$ openbao kv put -mount=secret creds passcode=my-long-passcode
== Secret Path ==
secret/data/creds
...
```shell-session
$ openbao kv get -mount=secret creds
== Secret Path ==
secret/data/creds
...
```shell-session
$ openbao kv metadata get -mount=secret creds
=== Metadata Path ===
...
```shell-session
$ openbao kv get -mount=secret -version=1 creds
== Secret Path ==
secret/data/creds
...
```text
Usage: openbao kv <subcommand> [options] [args]
