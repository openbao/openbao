...
description: The Venafi integrated secrets engine for OpenBao.
---

# Venafi secrets engine for OpenBao
...
integrations and policy controls, combined with OpenBao's built-in authentication
...
The Venafi secrets engine makes use of OpenBao's
...
#### Trust between OpenBao and trust protection platform
...
1. Create the [directory](/vault/docs/plugins/plugin-architecture#plugin-directory)
   where your OpenBao server will look for plugins (e.g. /etc/openbao/openbao_plugins).
   ...
   as /private/etc/openbao/openbao_plugins.
...
1. Download the latest `vault-pki-backend-venafi`
   ...
   $ mv venafi-pki-backend /etc/openbao/openbao_plugins
   ```

1. Update the OpenBao [server configuration](/vault/docs/configuration/)
   to specify the plugin directory:

   ```shell-session
   $ plugin_directory = "/etc/openbao/openbao_plugins"
   ```

1. Start your OpenBao using the [server command](/vault/docs/commands/server).

1. Get the SHA-256 checksum of the `venafi-pki-backend` plugin binary:

   ```shell-session
   $ SHA256=$(sha256sum /etc/openbao/openbao_plugins/venafi-pki-backend| cut -d' ' -f1)
   ```

1. Register the `venafi-pki-backend` plugin in the OpenBao
   [system catalog](/vault/docs/plugins/plugin-architecture#plugin-catalog):

   ```shell-session
   $ openbao write sys/plugins/catalog/secret/venafi-pki-backend \
...
1. Enable the Venafi secrets engine:
   ...
   ```shell-session
   $ openbao secrets enable -path=venafi-pki -plugin-name=venafi-pki-backend plugin
   ```

1. Configure a Venafi secret that maps a name in OpenBao to connection and authentication
   ...
   `openbao path-help venafi-pki/venafi/:name` after creating the secret.

   **Trust Protection Platform**:

   ```shell-session
   $ openbao write venafi-pki/venafi/tpp \
   ...
   **Venafi Cloud**:
   ...
   ```shell-session
   $ openbao write venafi-pki/venafi/cloud \
       apikey="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" \
       zone="zzzzzzzz-zzzz-zzzz-zzzz-zzzzzzzzzzzz"
   ```

1. Lastly, configure a [role](/vault/docs/secrets/pki)
   that maps a name in OpenBao to a Venafi secret for enrollment. To see all
   options available for roles, including `ttl`, `max_ttl` and `issuer_hint`
   (for validity), use `openbao path-help venafi-pki/roles/:name` after
   creating the role.

   **Trust Protection Platform**:

   ```shell-session
   $ openbao write venafi-pki/roles/tpp \
       venafi_secret=tpp \
   ...
   **Venafi Cloud**:
   ...
   ```shell-session
   $ openbao write venafi-pki/roles/cloud \
       venafi_secret=cloud \
       store_by=serial store_pkey=true \
       allowed_domains=example.com \
       allow_subdomains=true
...
After the Venafi secrets engine is configured and a user/machine has a OpenBao
...
`openbao path-help venafi-pki/issue/:role-name` and
`openbao path-help venafi-pki/sign/:role-name`.
...
1. Generate a certificate by writing to the `/issue` endpoint with the name of
   ...
   **Trust Protection Platform**:
   ...
   ```shell-session
   $ openbao write venafi-pki/issue/tpp common_name="common-name.example.com" \
   ...
   **Venafi Cloud**:
   ...
   ```shell-session
   $ openbao write venafi-pki/issue/cloud common_name="common-name.example.com" \
       alt_names="dns-san-1.example.com,dns-san-2.example.com"
   ```
...
1. Or sign a CSR from a file by writing to the `/sign` endpoint with the name of
   ...
   **Trust Protection Platform**:
   ...
   ```shell-session
   $ openbao write venafi-pki/sign/tpp csr=@example.req
   ```
   ...
   **Venafi Cloud**:
   ...
   ```shell-session
   $ openbao write venafi-pki/sign/cloud csr=@example.req
   ```
...
[OpenBao API](/vault/api-docs/secret/pki)
