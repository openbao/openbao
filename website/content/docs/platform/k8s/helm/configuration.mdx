...
- `injector` - Values that configure running a Vault Agent Injector Admission Webhook Controller within Kubernetes.
  ...
  - `agentDefaults` - Values that configure the injected Vault Agent containers default values.
    ...
    - `templateConfig` - Default values within Agent's [`template_config` stanza](/docs/agent-and-proxy/agent/template).
      ...
      - `staticSecretRenderInterval` (`string: ""`) - Configures how often Vault Agent Template should render non-leased secrets such as KV v2. See the [Vault Agent Templates documentation](/docs/agent-and-proxy/agent/template#non-renewable-secrets) for more details.
...
- `server` - Values that configure running a Vault server within Kubernetes.
  ...
  - `ingress` - Values that configure Ingress services for Vault.
    ...
    If [`ha`](#ha) is enabled the Ingress will point to the active vault server via the `active` Service. This requires vault 1.4+ and [service_registration](/docs/configuration/service-registration/kubernetes) to be set in the vault config.
  ...
  - `route` - Values that configure Route services for Vault in OpenShift
    ...
    ~> If [`ha`](#ha) is enabled the Route will point to the active vault server via the `active` Service (requires vault 1.4+ and [service_registration](/docs/configuration/service-registration/kubernetes) to be set in the vault config).
  ...
  - `serviceAccount` - Values that configure the Kubernetes service account created for Vault.
    ...
    - `serviceDiscovery` - Values that configure permissions required for Vault Server to automatically discover and join a Vault cluster using pod metadata.
      ...
      - `enabled` (`boolean: true`) - Enable or disable a service account role binding with the permissions required for Vault's Kubernetes [`service_registration`](/docs/configuration/service-registration/kubernetes) config option.
  ...
  - `ha` - This configures `ha` mode for the Vault server.
    ...
    - `clusterAddr` (`string: null`) - Set the [`cluster_addr`](/docs/configuration#cluster_addr) configuration for Vault HA.
...
- `csi` - Values that configure running the Vault CSI Provider.
  ...
  - `extraArgs` (`array: []`) - The extra arguments to be applied to the CSI pod startup command. See [here](/docs/platform/k8s/csi/configurations#command-line-arguments) for available flags.
