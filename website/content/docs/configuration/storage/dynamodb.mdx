...
description: |-
  The DynamoDB storage backend is used to persist OpenBao's data in DynamoDB
  table.
---
...
- **High Availability** – the DynamoDB storage backend supports high
  availability. Because DynamoDB uses the time on the OpenBao node to implement
  the session lifetimes on its locks, significant clock skew across OpenBao nodes
...
- `ha_enabled` `(string: "false")` – Specifies whether this backend should be used
  to run OpenBao in high availability mode. Valid values are "true" or "false". This
...
- `read_capacity` `(int: 5)` – Specifies the maximum number of reads consumed
  per second on the table, for use if OpenBao creates the DynamoDB table. This has
  no effect if the `table` already exists. This can also be provided via the
  environment variable `AWS_DYNAMODB_READ_CAPACITY`.

- `table` `(string: "openbao-dynamodb-backend")` – Specifies the name of the
  DynamoDB table in which to store OpenBao data. If the specified table does not
...
- `write_capacity` `(int: 5)` – Specifies the maximum number of writes performed
  per second on the table, for use if OpenBao creates the DynamoDB table. This value
...
running your OpenBao server on an EC2 instance, you can also make use of the EC2
instance profile service to provide the credentials OpenBao will use to make
DynamoDB API calls. Leaving the `access_key` and `secret_key` fields empty will
cause OpenBao to attempt to retrieve credentials from the AWS metadata service.
...
The governing policy for the IAM user or EC2 instance profile that OpenBao uses
to access DynamoDB must contain the following permissions for OpenBao to perform
...
initialization of OpenBao, you will need to create a table with these attributes:
...
resource "aws_dynamodb_table" "dynamodb-table" {
  ...
  tags = {
    Name        = "openbao-dynamodb-table"
    Environment = "prod"
  }
}
```

If a table with the configured name already exists, OpenBao will not modify it -
and the OpenBao configuration values of `read_capacity` and `write_capacity` have
no effect.

If the table does not already exist, OpenBao will try to create it, with read and
...
## DynamoDB examples of OpenBao configuration
