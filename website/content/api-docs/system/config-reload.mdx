...
description: The '/sys/config/reload' endpoint is used to reload specific parts of OpenBao's configuration.
---
...
The `sys/config/reload` endpoint allows reloading specific parts of OpenBao's configuration.
...
- `subsystem` `(string: <required>)` - Specifies the subsystem for OpenBao to reload. This is part of the request URL.
