...
Alternatively, you can specify the configuration path using the `BAO_OP_MIGRATE_CONFIG_PATH` environment variable. If the `-config` flag is provided, the environment variable is ignored.

**Example using environment variable:**

```shell-session
$ export BAO_OP_MIGRATE_CONFIG_PATH=/etc/openbao/migrate.hcl
