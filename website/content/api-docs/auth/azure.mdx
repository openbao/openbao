...
description: |-
  This is the API documentation for the OpenBao Azure authentication
  method plugin.
---

# Azure auth method (API)

This is the API documentation for the OpenBao Azure auth method
plugin. To learn more about the usage and operation, see the
[OpenBao Azure method documentation](/vault/docs/auth/azure).

This documentation assumes the plugin method is mounted at the
`/auth/azure` path in OpenBao. Since it is possible to enable auth methods
...
```json
{
  ...
  "max_ttl": 1800000,
  "max_jwt_exp": 10000,
  "bound_resource_groups": ["openbao-dev", "openbao-staging", "openbao-prod"]
}
```
