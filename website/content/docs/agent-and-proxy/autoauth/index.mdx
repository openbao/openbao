...
[`token_num_uses`](../../api/auth/approle.mdx#parameters-1).
...
- `namespace` `(string: optional)` - Namespace in which the mount lives.
  ...
  environment variable `BAO_NAMESPACE`, and then the highest precedence
...
- `wrap_ttl` `(string or integer: optional)` <a name="wrap_ttl"></a> - If specified, the written token
