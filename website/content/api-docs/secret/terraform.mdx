...
description: This is the API documentation for the OpenBao Terraform Cloud secret backend.
---

# Terraform Cloud secret backend HTTP API

This is the API documentation for the OpenBao Terraform Cloud secret backend. For general
information about the usage and operation of the Terraform Cloud backend, please see the
[OpenBao Terraform Cloud backend documentation](/vault/docs/secrets/terraform).

This documentation assumes the Terraform Cloud backend is mounted at the `/terraform` path
in OpenBao. Since it is possible to mount secret backends at any location, please
update your API calls accordingly.

## Configure access

This endpoint configures the access information for Terraform Cloud. This access
information is used so that OpenBao can communicate with Terraform Cloud and generate
...
This endpoint creates or updates the Terraform Cloud role definition in OpenBao.
...
access: Organizations, Teams, and Users. A OpenBao Role can manage a single type
...
originally created by OpenBao. As such, requesting an Organization or Team token
from OpenBao will return the same token indefinitely until that token is rotated
...
- `name` `(string: <required>)` – Specifies the name for the OpenBao role. This is
...
- `ttl` `(duration: "")` – Specifies the TTL for this role. If not
  provided, the default OpenBao TTL is used. Only applies to User API tokens.
...
- `max_ttl` `(duration: "")` – Specifies the max TTL for this role. If not
  provided, the default OpenBao Max TTL is used. Only applies to User API tokens.
  Uses [duration format strings](/vault/docs/concepts/duration-format).

### Sample payload

To create a OpenBao role to manage a Terraform Cloud User tokens
