...
### Restart the OpenBao server in recovery mode

To start OpenBao in recovery mode, add the `-recovery` flag to your `bao` startup command:
```shell-session
$ bao server -config /path/to/config.hcl -recovery
```

When OpenBao is started in recovery mode, `Recovery Mode: true` will be visible in the logs:
```shell-session
...
In recovery mode, every request to OpenBao needs to be authenticated with a recovery token. Such
tokens can be generated similarly to a root token.
...
```shell-session
$ bao operator generate-root -nonce <nonce> -recovery-token <recovery/unseal-key>
```

Once a threshold of keys has been submitted, OpenBao will return an encoded token:
```shell-session
Nonce            9e2de75b-e1aa-b358-eaaf-c97dbd02fc31
Started          true
Progress         1/1
