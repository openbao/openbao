...
func TestStress_HighThroughputSustained(t *testing.T) {
	...
	d := newDispatcher("stress-throughput", numWorkers, 0, newTestLogger("stress-throughput"))
	d.start()
...
func TestStress_BurstLoadThenStop(t *testing.T) {
	numWorkers := 100
	d := newDispatcher("stress-burst-stop", numWorkers, 0, newTestLogger("stress-burst-stop"))
	d.start()
...
func TestStress_ConcurrentShutdownUnderLoad(t *testing.T) {
	numWorkers := 50
	d := newDispatcher("stress-shutdown-load", numWorkers, 0, newTestLogger("stress-shutdown-load"))
	d.start()
