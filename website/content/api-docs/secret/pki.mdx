...
- `allow_commas_in_substitutions` `(bool: false)` - If set and templating is
  enabled, the values substituted in for `allowed_uri_sans_template` and
  `allowed_domains` template expression might contain a wildcard characters ('*'
  and '+'). This can enable injection attacks. Only use this, if the data used
  by the templates is trusted. Defaults to false.
