---
description: |-
  Technical overview of client count calculations in OpenBao 
---
...
OpenBao provides usage telemetry for the number of clients based on the number of
...
- Constructed entity assignments for active non-entity tokens.
...
:::note[Certificate entities currently reported as non-entity clients]

  OpenBao currently rolls certificate entities into the non-entity client count
  in the UI and API query requests. For more detailed information on certificate
  entities, use the
  [internal counter endpoint](/api-docs/system/internal-counters) to query
  monthly data for the PKI mount path or export historic data and look for
  `client_type=pki-acme`.

:::

## How OpenBao tracks clients

Each time a client authenticates, OpenBao checks whether the corresponding entity
ID has already been recorded in the client log as active for the current month:

- **If no record exists**, OpenBao adds an entry for the entity ID.
...
At the **end of each month**, OpenBao pre-computes reports for each cluster on the
...
## How OpenBao approximates current-month client count

OpenBao approximates client count for the current month using a
...
<TabItem value="Single-month tests" heading="Single-month tests">
...
<TabItem value="Multi-month / multi-segment tests" heading="Multi-month / multi-segment tests">
...
<ul>
  <li>
    <a href="/docs/concepts/client-count/">Clients and entities</a>
  </li>
  <li>
...
<TabItem value="Related API docs" heading="Related API docs">
<ul>
  <li>
    <a href="/api-docs/system/internal-counters#client-count">Client Count API</a>
  </li>
  <li>
    <a href="/api-docs/system/internal-counters">Internal counters API</a>
  </li>
</ul>
</TabItem>
...
<ul>
  <li>
    <a href="/tutorials/monitoring/usage-metrics">
      OpenBao Usage Metrics in OpenBao UI
    </a>
  </li>
  <li>
    <a href="/tutorials/monitoring/usage-metrics">KMIP Client metrics</a>
  </li>
</ul>
</TabItem>
...
<ul>
  ...
  <li>
    Blog post: 
    <a href="https://www.hashicorp.com/blog/onboarding-applications-to-vault-using-terraform-a-practical-guide">Onboarding Applications to Vault Using Terraform: A Practical Guide</a>
  </li>
</ul>
</TabItem>
