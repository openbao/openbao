...
`openbao server -dev` (or `openbao operator init` for a non-dev server) outputs an
...
1. The initial root token generated at `openbao operator init` time -- this token has no
   expiration
1. By using another root token; a root token with an expiration cannot create a
   root token that never expires
1. By using `openbao operator generate-root` ([example](/vault/tutorials/operations/generate-root))
...
period using `openbao token renew` or the appropriate renewal endpoint. At this
