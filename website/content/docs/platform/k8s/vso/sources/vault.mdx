...
- All Vault [secret engines](/docs/secrets) supported.
...
| Backend                                   | Description                                                                                                 |
|-------------------------------------------|-------------------------------------------------------------------------------------------------------------|
| [Kubernetes](/docs/auth/kubernetes) | Relies on short-lived Kubernetes ServiceAccount tokens for Vault authentication                             |
| [JWT](/docs/auth/jwt)               | Relies on either static JWT tokens or short-lived Kubernetes ServiceAccount tokens for Vault authentication |
| [AppRole](/docs/auth/approle)       | Relies on static AppRole credentials for Vault authentication                                               |
| [AWS](/docs/auth/aws)               | Relies on AWS credentials for Vault authentication                                                          |
...
Supported secrets engines: [kv-v2](/docs/secrets/kv/kv-v2), [kv-v1](/docs/secrets/kv/kv-v1)

##### KV version 1 secret example

The KV secrets engine's `kvv1` mount path is specified under `spec.mount` of `VaultStaticSecret` custom resource. Please consult [KV Secrets Engine - Version 1 - Setup](/docs/secrets/kv/kv-v1#setup) for configuring KV secrets engine version 1. The following results in a request to `http://127.0.0.1:8200/v1/kvv1/eng/apikey/google` to retrieve the secret.
...
[KV Secrets Engine - Version 2 - Setup](/docs/secrets/kv/kv-v2#setup)
guide.
...
Supported secrets engines: [pki](/docs/secrets/pki)

The PKI secrets engine's mount path is specified under `spec.mount` of `VaultPKISecret` custom resource. Please consult [PKI Secrets Engine - Setup and Usage](/docs/secrets/pki/setup) for configuring PKI secrets engine. The following results in a request to `http://127.0.0.1:8200/v1/pki/issue/default` to generate TLS certificates.
...
Supported secrets engines *non-exhaustive*: [databases](/docs/secrets/databases), [aws](/docs/secrets/aws),
[azure](/docs/secrets/azure), [gcp](/docs/secrets/gcp), ...
...
[Database Secrets Engine - Setup](/docs/secrets/databases#setup) guide.
...
configuration options, consult the
[AWS Secrets Engine - Setup](/docs/secrets/aws#setup) guide.
...
Kubernetes](/tutorials/kubernetes/vault-secrets-operator) tutorial to
