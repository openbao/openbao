...
Want an automated alternative to Shamir's stored in Kubernetes secrets,
OpenStack Barbican, or similar static secret stores? Check out [static key
auto-unseal](../configuration/seal/static.mdx) to avoid the discouraged pattern of automating
Shamir's manual unseal.
...
Recovery key generation using auto-unseal [can be delayed](../api/system/init.mdx)
until after initialization by setting `recovery_shares=0` on the call to
`sys/init`. Generating them later requires a root or policy with `sudo` access
to the [`sys/rotate/recovery` endpoints](../api/system/rotate/init.mdx).

**Just don't forget to generate them well before you need them!**
...
and their API equivalents in the [/sys/init](../api/system/init.mdx) endpoint:
...
[`/sys/rotate/recovery` endpoint](../api/system/rotate/index.mdx).
