...
description: This is the API documentation for the OpenBao AWS secrets engine.
---

# AWS secrets engine (API)

This is the API documentation for the OpenBao AWS secrets engine. For general
information about the usage and operation of the AWS secrets engine, please see
the [OpenBao AWS documentation](/vault/docs/secrets/aws).
...
are multiple ways to pass root IAM credentials to the OpenBao server, specified
...
- `username_template` `(string: <optional>)` - [Template](/vault/docs/concepts/username-templating) describing how
  ...
  is provided the field defaults to the template below. It is to be noted that, DisplayName is the name of the OpenBao instance
  ...
  ```
  {{ if (eq .Type "STS") }}
      {{ printf "openbao-%s-%s" (unix_time) (random 20) | truncate 32 }}
  {{ else }}
      {{ printf "openbao-%s-%s-%s" (printf "%s-%s" (.DisplayName) (.PolicyName) | truncate 42) (unix_time) (random 20) | truncate 64 }}
  {{ end }}
  ```
...
- `role_arns` `(list: [])` – Specifies the ARNs of the AWS roles this OpenBao role
...
- `iam_groups` `(list: [])` - A list of IAM group names. IAM users generated
  against this OpenBao role will be added to these IAM Groups. For a credential
...
<Tabs>
  ...
  ```bash
    openbao write aws/roles/example-role \
    credential_type=iam_user \
...
If invalid role data was supplied to the role from an earlier version of OpenBao,
...
- `role_arn` `(string)` – The ARN of the role to assume if `credential_type` on
  the OpenBao role is `assumed_role`. Must match one of the allowed role ARNs in
  the OpenBao role. Optional if the OpenBao role only allows a single AWS role ARN;
  required otherwise.
...
with an AWS IAM User, which will be adopted and managed by OpenBao, including rotating it according
to the configured `rotation_period`.

<Note>

OpenBao will create a new credential upon configuration, and if the maximum number of access keys already exist, OpenBao will rotate the oldest one. OpenBao must do this to know the credential.

At each rotation, OpenBao will rotate the oldest existing credential.
...
OpenBao should wait before rotating the password. The minimum is 1 minute. Can be
