...
```shell-session
$ openbao secrets move database/ db-prod/
...
```shell-session
$ openbao secrets tune -max-lease-ttl=30m db-prod/
...
```shell-session
$ openbao secrets disable db-prod/
...
```text
Usage: openbao secrets <subcommand> [options] [args]
