...
description: |-
  The "debug" command monitors a OpenBao server, probing information about
  it for a certain duration.
---

# debug

The `debug` command starts a process that monitors a OpenBao server, probing
...
```shell-session
$ openbao debug -duration=1m -interval=10s -metrics-interval=5s -compress=false
```

Start debug with specific targets:

```shell-session
$ openbao debug -target=host -target=metrics
```
