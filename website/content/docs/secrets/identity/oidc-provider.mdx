...
OpenBao's source of [identity](/vault/docs/concepts/identity) and wide range of [authentication methods](/vault/docs/auth)
...
Each OpenBao namespace has a default OIDC [provider](/vault/docs/concepts/oidc-provider#providers)
...
1. Enable a OpenBao auth method:
...
2. Create a user:
   ...
   ```text
   $ openbao write auth/userpass/users/end-user password="securepassword"
...
2. Create a client application:
   ...
   ```text
   $ openbao write identity/oidc/client/my-webapp \
   ...
   To allow all OpenBao entities to authenticate, the built-in [allow_all](/vault/docs/concepts/oidc-provider#assignments)
   assignment is provided.

2. Read client credentials:

   ```text
   $ openbao read identity/oidc/client/my-webapp
...
After configuring a OpenBao auth method and client application, the following details can
...
Refer to the [OpenBao as an OIDC Identity Provider](/vault/tutorials/auth-methods/oidc-identity-provider)
