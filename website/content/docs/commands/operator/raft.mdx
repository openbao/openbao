...
```text
Usage: openbao operator raft <subcommand> [options] [args]
...
```text
Usage: openbao operator raft join [options] <leader-api-addr>

  Join the current node as a peer to the Raft cluster by providing the address
  of the Raft leader node.

	  $ openbao operator raft join "http://127.0.0.2:8200"
```
...
```text
Usage: openbao operator raft join [options] <auto-join-configuration>
  Join the current node as a peer to the Raft cluster by providing cloud auto-join
  metadata configuration.
    $ openbao operator raft join "provider=aws region=eu-west-1 ..."
```
...
```text
Usage: openbao operator raft list-peers

  Provides the details of all the peers in the Raft cluster.

	  $ openbao operator raft list-peers
```
...
```text
Usage: openbao operator raft remove-peer <server_id>

  Removes a node from the Raft cluster.

	  $ openbao operator raft remove-peer node1
```
...
```text
Usage: openbao operator raft snapshot <subcommand> [options] [args]
...
```text
Usage: openbao operator raft snapshot save <snapshot_file>

  Saves a snapshot of the current state of the Raft cluster into a file.

	  $ openbao operator raft snapshot save raft.snap
```
...
Restores a snapshot of OpenBao data taken with `openbao operator raft snapshot save`.

```text
Usage: openbao operator raft snapshot restore <snapshot_file>

  Installs the provided snapshot, returning the cluster to the state defined in it.

	  $ openbao operator raft snapshot restore raft.snap
```
...
```text
Usage: openbao operator raft autopilot <subcommand> [options] [args]
...
```text
Usage: openbao operator raft autopilot get-config

  Returns the configuration of the autopilot subsystem under integrated storage.

    $ openbao operator raft autopilot get-config
```
...
```text
Usage: openbao operator raft autopilot set-config [options]

  Modify the configuration of the autopilot subsystem under integrated storage.

	  $ openbao operator raft autopilot set-config -server-stabilization-time 10s
