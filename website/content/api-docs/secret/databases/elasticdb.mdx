...
- `username` `(string: <required>)` - The username to be used in the connection URL (`openbao`).
...
- `creation_statements` `(string: <required>)` – Using JSON, either defines an
  ...
  in Elasticsearch.  Defining roles in OpenBao is more secure than using pre-existing
