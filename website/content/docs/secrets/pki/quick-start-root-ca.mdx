...
This document provides a brief overview of setting up a OpenBao PKI Secrets
...
```shell-session
$ openbao secrets tune -max-lease-ttl=87600h pki
...
```shell-session
$ openbao write pki/root/generate/internal common_name=myopenbao.com ttl=87600h
Key             Value
---             -----
...
```shell-session
$ openbao write pki/config/urls issuing_certificates="http://openbao.example.com:8200/v1/pki/ca" crl_distribution_points="http://openbao.example.com:8200/v1/pki/crl"
...
```shell-session
$ openbao write pki/roles/example-dot-com \
...
```shell-session
$ openbao write pki/issue/example-dot-com \
...
If you get stuck at any time, simply run `openbao path-help pki` or with a
