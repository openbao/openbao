...
-> If you are running your applications in a Kubernetes cluster, we recommend
  evaluating the [OpenBao Secrets Operator](/vault/docs/platform/k8s/vso) and
  the [OpenBao Agent Sidecar Injector](/vault/docs/platform/k8s/injector).

!> OpenBao Agent's Process Supervisor Mode is in public beta. Please provide your
   feedback by opening a GitHub issue [here](https://github.com/lf-edge/openbao/issues).
...
- `error_on_missing_key` `(bool: false)` - Exit with an error when accessing
  ...
  to "true". Also see
  [`exit_on_retry_failure` in global OpenBao Agent Template Config](/vault/docs/agent-and-proxy/agent/template#interaction-between-exit_on_retry_failure-and-error_on_missing_key).
