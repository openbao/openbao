...
The demonstration below uses the [`KVv1` secrets engine](/api-docs/secret/kv/kv-v1), which is a
...
A [KVv2 example](/api-docs/secret/kv/kv-v2#sample-request-3) for the engine path of `secret` requires that URI is
...
Additionally, with the [OpenAPI generation](/api-docs/system/internal-specs-openapi) in OpenBao, you will get back a small
