...
```shell-session
$ bao login -method=okta username=my-username
```
...
```shell-session
$ bao login -method=okta username=my-username totp=123456
```
...
```shell-session
$ bao login -method=okta username=my-username totp=123456 provider=GOOGLE
```
...
1. Map an Okta group to a OpenBao policy:
  ...
  ```shell-session
  $ bao write auth/okta/groups/scientists policies=nuclear-reactor
  ```
...
1. It is also possible to add users directly:
  ...
  ```shell-session
  $ bao write auth/okta/groups/engineers policies=autopilot
  $ bao write auth/okta/users/tesla groups=engineers
  ```
