...
- `kubernetes_ca_cert` `(string: "")` - PEM encoded CA cert for use by the TLS client used to talk with the Kubernetes API. NOTE: Every line must end with a newline: `\n`
  If unset and depending on the value of `disable_local_ca_jwt` flag, OpenBao will use either local Kubernetes CA certificate or host system trust store.
...
If unset and `disable_local_ca_jwt` is set to `false`, OpenBao uses the local Kubernetes CA certificate. If unset and `disable_local_ca_jwt` is set to `true`, OpenBao uses the host system trust store.
