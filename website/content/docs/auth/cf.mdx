...
The `cf` auth method provides an automated mechanism to retrieve a OpenBao token
...
1. You construct a request to OpenBao including your `CF_INSTANCE_CERT`, signed by your `CF_INSTANCE_KEY`.
2. OpenBao validates that the signature is no more than 300 seconds old, or 60 seconds in the future.
3. OpenBao validates that the cert was issued by the CA certificate you've pre-configured.
4. OpenBao validates that the request was signed by the private key for the `CF_INSTANCE_CERT`.
5. OpenBao validates that the `CF_INSTANCE_CERT` application ID, space ID, and org ID presently exist.
6. If all checks pass, OpenBao issues an appropriately-scoped token.

## Known risks

This authentication engine uses CF's instance identity service to authenticate users to OpenBao. Because
CF makes its CA certificate and private key available to certain users at any time, it's possible for
someone with access to them to self-issue identity certificates that meet the criteria for a OpenBao role,
allowing them to gain unintended access to OpenBao.
...
run `$ cf api` to view the API endpoint that OpenBao will use.

Next, configure a user for OpenBao to use. This plugin was tested with Org Manager level
permissions, but lower level permissions _may_ be usable.

```shell-session
$ cf create-user openbao pa55w0rd
$ cf orgs
$ cf org-users my-example-org
$ cf set-org-role openbao my-example-org OrgManager
```

Specifically, the `openbao` user created here will need to be able to perform the following API calls:
...
After obtaining the information described above, a OpenBao operator will configure the CF auth method
like so:

```shell-session
$ openbao auth enable cf

$ openbao write auth/cf/config \
...
$ openbao write auth/cf/roles/my-role \
...
```shell-session
$ openbao login -method=cf role=test-role
```

For CF, we do also offer an agent that, once configured, can be used to obtain a OpenBao token on
your behalf.
...
$ openbao write auth/vault-plugin-auth-cf/config identity_ca_certificates="$CURRENT" identity_ca_certificates="$FUTURE"
```

If OpenBao receives a `CF_INSTANCE_CERT` matching _any_ of the `identity_ca_certificates`,
...
an expired certificate, an incorrect `ca.crt`, or a OpenBao configuration that doesn't
