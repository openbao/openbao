...
- `check_valid_email(value ref.Val) bool` - returns true if the value is a
syntactically valid e-mail address.

- `decode_json(value ref.Val) dyn|bool` - parses a JSON string into a CEL value
(map/list/string/number/bool/null) or returns false if the input isn’t a string
or the JSON is invalid.

- `encode_json(value ref.Val) string|bool` - returns a JSON string or false if conversion 
or JSON marshaling fails.
