...
If request forwarding is enabled (turned on by default), clients can still force
the older/fallback redirection behavior (see below) if desired by setting the
...
the value can also be specified by the `BAO_API_ADDR` environment variable,
...
[`cluster_address`](../configuration/listener/tcp.mdx#cluster_address) values set in the
...
This value can also be specified by the `BAO_CLUSTER_ADDR` environment
...
[comparison
chart](../configuration/storage/index.mdx#integrated-storage-raft-vs-postgresql)
