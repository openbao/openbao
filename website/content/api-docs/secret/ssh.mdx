...
The default issuer is a reference to an issuer whose operations will be performed
against when no issuer reference is provided in the role creation. The default
reference can be updated at any time by submitting/generating a new issuer and
setting it as default or updating it through the configuration endpoint.
...
The name must be unique accross all issuers and not be the reserved value `default`.
