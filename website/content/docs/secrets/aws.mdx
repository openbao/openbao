...
1.  Configure a OpenBao role that maps to a set of permissions in AWS as well as an
    ...
    ```text
    $ bao write aws/roles/my-other-role \
...
1.  Rotate the credentials that OpenBao uses to communicate with AWS:
    ...
    ```test
    $ bao write -f aws/config/rotate-root
    Key           Value
    ---           -----
...
```shell-session
$ bao write aws/sts/ec2_admin ttl=60m
Key            	Value
...
```shell-session
$ bao write aws/sts/deploy ttl=60m
Key            	Value
...
$ bao revoke aws/creds/deploy/774cfb27-c22d-6e78-0077-254879d1af3c
...
If you get stuck at any time, simply run `bao path-help aws` or with a subpath for
