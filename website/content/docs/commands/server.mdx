...
description: |-
  The "server" command starts a OpenBao server that responds to API requests. By
...
The `server` command starts a OpenBao server that responds to API requests. By
default, OpenBao will start in a "sealed" state. The OpenBao cluster must be
initialized before use, usually by the `openbao operator init` command. Each OpenBao
server must also be unsealed using the `openbao operator unseal` command or the
...
- [`operator init` command](/vault/docs/commands/operator/init) for information
  on initializing a OpenBao server.

- [`operator unseal` command](/vault/docs/commands/operator/unseal) for
  information on providing unseal keys.

- [OpenBao configuration](/vault/docs/configuration) for the syntax and
  various configuration options for a OpenBao server.
...
```shell-session
$ openbao server -config=/etc/openbao/config.hcl
```
