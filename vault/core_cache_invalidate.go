...
func (ij *invalidationJob) Execute() error {
	ij.im.dispacherLogger.Trace("issuing invalidation", "key", ij.key, "ns", ij.nsUUID)
	defer ij.im.dispacherLogger.Trace("finished invalidation", "key", ij.key, "ns", ij.nsUUID)
	...
	case ij.im.core.router.Invalidate(ctx, key):
