...
description: |-
  The One-Time SSH Password (OTP) SSH secrets engine type allows a OpenBao server
...
The One-Time SSH Password (OTP) SSH secrets engine type allows a OpenBao server to
...
documentation on every path, use `openbao path-help` after mounting the secrets
engine.
...
```shell-session
$ openbao write ssh/roles/otp_key_role \
    key_type=otp \
...
```shell-session
$ openbao write ssh/creds/otp_key_role ip=x.x.x.x
Key            	Value
...
```shell-session
$ openbao ssh -role otp_key_role -mode otp username@x.x.x.x
...
```shell-session
$ openbao ssh -role otp_key_role -mode otp -strict-host-key-checking=no username@x.x.x.x
