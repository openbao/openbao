...
- A blocking failure is one where an attempt to write to the audit device never completes. This is unlikely with a local disk device, but could occur with a network-based audit device.
...
docs](../api/system/audit.mdx) for more details.
