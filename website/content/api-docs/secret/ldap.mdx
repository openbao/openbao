...
- `schema` `(string: "openldap")` <a name="schema"></a> - The LDAP schema to use when storing entry passwords.
  Valid schemas include `openldap`, `ad`, and `racf`.
- `userdn` `(string: <optional>)` <a name="userdn"></a> - The base DN under which to perform user search in
  [library set management](ldap.mdx#library-set-management) and [static roles](ldap.mdx#static-roles).
  For example, `ou=Users,dc=openbao,dc=com`.
- `userattr` `(string: <optional>)` <a name="userattr"></a> – The attribute field name used to perform user search
  in [library set management](ldap.mdx#library-set-management) and [static roles](ldap.mdx#static-roles).
