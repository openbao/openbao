...
1.  Configure OpenBao with the proper plugin and connection information:
    ...
    ```shell-session
    $ bao write database/config/my-mongodbatlas-database \
...
1.  Configure a role that maps a name in OpenBao to a MongoDB Atlas command that executes and
    ...
    ```shell-session
    $ bao write database/roles/my-password-role \
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```shell-session
    $ bao read database/creds/my-password-role
        Key                Value
        ---                -----
...
1.  Configure a role that maps a name in OpenBao to a MongoDB Atlas command that executes and
    ...
    ```shell-session
    $ bao write database/roles/my-dynamic-certificate-role \
      db_name=my-mongodbatlas-database \
...
of the role:
    ...
    ```shell-session
    $ bao read database/creds/my-dynamic-certificate-role
      Key                 Value
      ---                 -----
