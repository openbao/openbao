...
func doVerifyEntry(key string, value []byte, expected []byte) error {
	...
	if subtle.ConstantTimeCompare(actual, expected) == 0 {
		return fmt.Errorf("for get %v (actual=%v vs expected=%v - actual value `%v`): %w", key, hex.EncodeToString(actual), hex.EncodeToString(expected), hex.EncodeToString(value), physical.ErrTransactionCommitFailure)
...
func doVerifyList(repr string, items []string, expected []byte) error {
	...
	if subtle.ConstantTimeCompare(actual, expected) == 0 {
		return fmt.Errorf("for list %v: %w", repr, physical.ErrTransactionCommitFailure)
		// return physical.ErrTransactionCommitFailure
	}
