...
deployments. Use of the `BAO_DISABLE_PUBLIC_ACME` environment variable
...
#### Parameters
 ...
 - `challenge_permitted_ip_ranges` `(list: [])` -
   Specifies a list of IP addresses or CIDR ranges that are exclusively allowed
   for challenge validation (HTTP-01, TLS-ALPN-01).
   If empty, all addresses are permitted except those in `challenge_excluded_ip_ranges`.

 - `challenge_excluded_ip_ranges` `(list: [])` -
   Specifies a list of IP addresses or CIDR ranges that are excluded from challenge validation.
   This takes precedence over `challenge_permitted_ip_ranges`.
...
- `type` `(string: <required>)` - Specifies the type of the key to
  ...
  later_. If `kms`, we expect the `external_key_ref` parameter to refer to an
  external key. This parameter is part of the request URL.
...
- `type` `(string: <required>)` - Specifies the type of the root to
  ...
  existing key material to create the root CA; if `kms`, we expect the
  `external_key_ref` parameter to refer to an external key. This parameter is
...
- `key_ref` `(string: "default")` - Specifies the key (either `default`, by
  ...
  for `type=existing` requests. This is different from `external_key_ref`.
...
- `type` `(string: <required>)` - Specifies the type of the intermediate to
  ...
  key material to create the CSR; if `kms`, we expect the `external_key_ref`
  parameter to refer to an external key. This parameter is part of the request
  URL.
...
- `external_key_ref` `(string: "")` - When `type=kms`, this specifies
