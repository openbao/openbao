...
1. [Mount][plugin_management] the plugin. Skip this step if your initial plugin
   is already mounted.
    ...
    ```shell-session
    $ openbao secrets enable my-secret-plugin
    ```
...
1. Tune the existing mount to configure it to use the newly registered version.
   ...
   ```shell-session
   $ openbao secrets tune -plugin-version=v1.0.1 my-secret-plugin
   ```
...
1. Finally, trigger a [plugin reload](/vault/docs/commands/plugin/reload) to reload all
   ...
   with either the `plugin` or `mounts` flag respectively.
    ...
    ```shell-session
    $ openbao plugin reload -plugin my-secret-plugin
    ```
...
now match the "Version" when you run `openbao secrets list -detailed`.
...
1. [Mount][plugin_management] the plugin. Skip this step if your initial plugin
   is already mounted.
    ...
    $ openbao write database/config/my-db \
...
* Existing OpenBao [leases](/vault/docs/concepts/lease) and [tokens](/vault/docs/concepts/tokens)
