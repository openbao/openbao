...
func (d *OutputProfileError) formatHCL(value any) (string, error) {
	switch value.(type) {
	case string:
...
func (d *OutputProfileError) buildProfile() (string, error) {
	...
	profile.WriteString(fmt.Sprintf("   operation = %q\n", d.operation()))

	profile.WriteString(fmt.Sprintf("   path = %q\n", d.path()))

	if d.token() != "" {
		profile.WriteString(fmt.Sprintf("   token = %q\n", d.token()))
	}
	...
	if len(headers) > 0 {
		...
		for header, value := range headers {
			profile.WriteString(fmt.Sprintf("      %q = %v\n", header, value))
		}
		profile.WriteString("   }\n")
	}
