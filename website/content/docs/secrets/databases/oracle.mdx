...
The Oracle database plugin is not bundled in the core OpenBao code tree and can be
...
```sql
GRANT CREATE USER to openbao WITH ADMIN OPTION;
GRANT ALTER USER to openbao WITH ADMIN OPTION;
GRANT DROP USER to openbao WITH ADMIN OPTION;
GRANT CONNECT to openbao WITH ADMIN OPTION;
GRANT CREATE SESSION to openbao WITH ADMIN OPTION;
GRANT SELECT on gv$session to openbao;
GRANT SELECT on v_$sql to openbao;
GRANT ALTER SYSTEM to openbao WITH ADMIN OPTION;
```

~> OpenBao needs `ALTER SYSTEM` to terminate user sessions when revoking users. This may be 
substituted with a stored procedure and granted to the OpenBao administrator user.

If you are running OpenBao with [mlock enabled](/vault/docs/configuration#disable_mlock),
...
1.  Download and register the plugin:
    ...
    ```shell-session
    $ openbao write sys/plugins/catalog/database/oracle-database-plugin \
        sha256="..." \
...
1.  Configure OpenBao with the proper plugin and connection information:
    ...
    ```shell-session
    $ openbao write database/config/my-oracle-database \
        plugin_name=oracle-database-plugin \
        connection_url="{{username}}/{{password}}@localhost:1521/OraDoc.localhost" \
        allowed_roles="my-role" \
        username="OPENBAO_SUPER_USER" \
...
1. It is highly recommended that you immediately rotate the "root" user's password, see
   ...
   recommended that you create a user for OpenBao to use rather than using the actual root user.

1. Configure a role that maps a name in OpenBao to an SQL statement to execute to
   create the database credential:

   ```shell-session
   $ openbao write database/roles/my-role \
   ...
   Note: The `creation_statements` may be specified in a file and interpreted by the OpenBao CLI using the `@` symbol:
...
If the Oracle server OpenBao is trying to connect to uses an SSL listener, the database
plugin will require extra configuration using the `connection_url` parameter:

```shell-session
openbao write database/config/oracle \
  plugin_name=openbao-plugin-database-oracle \
...
~> **Note**: The wallets used when connecting via SSL should be available on every OpenBao
server when using high availability clusters.

The wallet used by OpenBao should be in a well known location with the proper filesystem permissions. For example, if OpenBao is running as the `openbao` user,
the wallet directory may be setup as follows:

```shell-session
mkdir -p /etc/openbao/wallets
cp cwallet.sso /etc/openbao/wallets/cwallet.sso
chown -R openbao:openbao /etc/openbao
chmod 600 /etc/openbao/wallets/cwallet.sso
```

### Using TNS names

~> **Note**: The `tnsnames.ora` file and environment variable used when connecting via SSL should
be available on every OpenBao server when using high availability clusters.

OpenBao can optionally use TNS names in the connection string when connecting to Oracle databases using a `tnsnames.ora` file. An example
...
To configure OpenBao to use TNS names, set the following environment variable on the OpenBao server:

```shell-session
TNS_ADMIN=/path/to/tnsnames/directory
```

~> **Note**: If OpenBao returns a "could not open file" error, double check that
the `TNS_ADMIN` environment variable is available to the OpenBao server.

Use the alias in the `connection_url` parameter on the database configuration:

```
openbao write database/config/oracle-east \
    plugin_name=openbao-plugin-database-oracle \
...
openbao write database/config/oracle-west \
    plugin_name=openbao-plugin-database-oracle \
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```text
    $ openbao read database/creds/my-role
    Key                Value
    ---                -----
    ...
    username           V_OPENBAOUSE_MY_ROLE_SJJUK3Q8W3BKAYAN8S62_1602543009
    ```
