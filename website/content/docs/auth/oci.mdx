...
```shell-session
$ bao login -method=oci auth_type=instance role=openbaoadminrole
```
...
```shell-session
$ bao login -method=oci auth_type=apikey role=openbaoadminrole
```
...
3.  Login to OpenBao assuming the devrole.
   ...
   ```shell-session
   $ bao login -method=oci auth_type=instance role=devrole
   ```
...
```shell-session
$ bao login -method=oci auth_type=apikey role=devrole
```
