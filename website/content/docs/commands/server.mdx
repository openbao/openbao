...
- `-config` `(string: "")` - Path to a configuration file or directory of
  ...
  are appended to the config. If the path is `-`, the configuration is read from
  stdin. Stdin is read only once: on `SIGHUP`, the configuration read at startup
  is reused, while configuration files are re-read from disk.
...
- `-log-format` `(string: "standard")` <a name="_log_format"></a> - Log format. Supported values
  are `standard` and `json`. This can also be specified via the
  `BAO_LOG_FORMAT` environment variable.

- `-log-file` <a name="_log_file"></a> - the absolute path where OpenBao should save log
...
- `-log-rotate-bytes` <a name="_log_rotate_bytes"></a> - to specify the number of
...
- `-log-rotate-duration` <a name="_log_rotate_duration"></a> - to specify the maximum
  duration a log should be written to before it needs to be rotated. Must be a duration
  value such as 30s. Defaults to 24h.

- `-log-rotate-max-files` <a name="_log_rotate_max_files"></a> - to specify the maximum
...
- `BAO_ALLOW_PENDING_REMOVAL_MOUNTS` `(bool: false)` <a name="bao_allow_pending_removal_mounts"></a> - (environment variable)
  ...
  more information, see the [Deprecation FAQ](/community/deprecation/faq#q-what-are-the-phases-of-deprecation).
...
- `-dev-listen-address` `(string: "127.0.0.1:8200")` - Address to bind to in
  "dev" mode. This can also be specified via the `BAO_DEV_LISTEN_ADDRESS`
...
- `-dev-root-token-id` `(string: "")` - Initial root token. This only applies
  ...
  `BAO_DEV_ROOT_TOKEN_ID` environment variable.
