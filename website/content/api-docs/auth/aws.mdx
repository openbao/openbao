...
description: This is the API documentation for the OpenBao AWS auth method.
---
...
This is the API documentation for the OpenBao AWS auth method. For
general information about the usage and operation of the AWS method, please
see the [OpenBao AWS method documentation](/vault/docs/auth/aws).
...
- `iam_server_id_header_value` `(string: "")` - The value to require in the
  ...
  to a production server. Consider setting this to the OpenBao server's DNS name.
...
When you have configured OpenBao with static credentials, you can use this
endpoint to have OpenBao rotate the access key it used. Note that, due to AWS
eventual consistency, after calling this endpoint, subsequent calls from OpenBao
to AWS may fail for a few seconds until AWS becomes consistent again.

In order to call this endpoint, OpenBao's AWS access key MUST be the only access
key on the IAM user; otherwise, generation of a new access key will fail. Once
this method is called, OpenBao will now be the only entity that knows the AWS
...
The new access key OpenBao uses is returned by this operation.

## Configure identity integration

This configures the way that OpenBao interacts with the
[Identity](/vault/docs/secrets/identity) store. The default (as of OpenBao
...
- `iam_alias` `(string: "role_id")` - How to generate the identity alias when
  ...
  **Note**: if you select `full_arn` and then delete and recreate the IAM role,
  OpenBao won't be aware and any identity aliases set up for the role name will
  still be valid.
...
(i.e. those which are not the account in which the OpenBao server is
running.) OpenBao will use credentials obtained by assuming these STS roles
...
- `account_id` `(string: <required>)` - AWS account ID to be associated with
  STS role. If set, OpenBao will use assumed credentials to verify any login
  attempts from EC2 instances in this account.
- `sts_role` `(string: <required>)` - AWS ARN for STS role to be assumed when
  interacting with the account specified. The OpenBao server must have
...
- `role` `(string: <required>)` - Name of the role. OpenBao normalizes all role
  ...
  OpenBao can be provided the role in any casing, and it will internally handle
...
- `bound_iam_principal_arn` `(list: [])` - Defines the list of IAM principals
  ...
  This is a comma-separated string or JSON array.
- `inferred_entity_type` `(string: "")` - When set, instructs OpenBao to turn on
  inferencing. The only current valid value is "ec2_instance" instructing OpenBao
...
- `resolve_aws_unique_ids` `(bool: true)` - When set, resolves the
  ...
  This requires OpenBao to be able to call `iam:GetUser` or `iam:GetRole` on the
  ...
  or roles won't get access to roles in OpenBao that were permissioned to the
  ...
  true on an existing role, OpenBao will attempt to resolve the role's bound IAM
  ...
  then OpenBao has already been able to resolve internal IDs, and it doesn't need
  any further IAM permissions to authenticate users. If a role has been deleted
  and recreated, and OpenBao has cached the old unique ID, you should just call
  this endpoint specifying the same `bound_iam_principal_arn` and, as long as
  OpenBao still has the necessary IAM permissions to resolve the unique ID, OpenBao
...
- `iam_request_headers` `(string: <required-iam>)` - Key/value pairs of headers
  ...
  one). If the `iam_server_id_header_value` is configured in OpenBao for the aws
