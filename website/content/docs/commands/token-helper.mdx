...
This is an example token helper written in Ruby that stores and retrieves tokens in a json file called `~/.openbao_tokens`. The key is the environment variable `BAO_ADDR`, this allows the OpenBao user to easily store and retrieve tokens from a number of different OpenBao servers.
...
unless ENV['BAO_ADDR']
  STDERR.puts "No BAO_ADDR environment variable set. Set it and run me again!"
  exit 100
end
...
case ARGV.first
when 'get'
  print tokens[ENV['BAO_ADDR']] if tokens[ENV['BAO_ADDR']]
  exit 0
when 'store'
  tokens[ENV['BAO_ADDR']] = STDIN.read
when 'erase'
  tokens.delete!(ENV['BAO_ADDR'])
end
