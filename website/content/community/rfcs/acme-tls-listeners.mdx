...
(ignoring the warning about the node not running, since it wasn't started through `devbao`).

Finally, the new address can be used:

```
$ export BAO_ADDR=https://localhost:8200
$ export BAO_CACERT=root.pem
$ bao list pki-int/certs # (should have more certs)
```

Note that changing `BAO_ADDR` to equivalent ones will also work (e.g., `localhost.localdomain`, `127.0.0.1`, &c): the initial connection will be slower but subsequent ones will be fast.
