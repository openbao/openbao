...
func (c *Client) setIndexFromResult(request *Request, response *Response) {
	addr := request.URL.String()
	ns := request.Headers.Get(NamespaceHeaderName)
...
type IndexManager interface {
	...
	Set(addr string, namespace string, index string)
}
...
func (i *simpleIndexManager) Set(_ string, _ string, index string) {
	i.lock.Lock()
	defer i.lock.Unlock()
