...
description: >-
  The OCI Object Storage backend is used to persist OpenBao's data in OCI Object
  Storage.
---

# OCI object storage storage backend

The OCI Object Storage backend is used to persist OpenBao's data in OCI Object Storage.
...
storage "oci" {
   ...
   bucket_name = "<openbao_data_bucket_name>"
...
To use the OCI Object Storage OpenBao storage backend, you must have a OCI account. Either using the API or web interface, create the data bucket and lock bucket if enabling high availability.

The OCI Object Storage backend does not support creating the buckets automatically at this time.

## `oci` authentication

The OCI Object Storage OpenBao storage backend uses the official OCI Golang SDK. This means it supports the common ways of providing credentials to OCI.

For more information on service accounts, please see the [OCI Identity documentation][oci-identity].

## `oci` parameters

- `region` `(string: <optional>)` - Specifies the OCI region where OpenBao should look for object storage buckets. If not specified the OCI Storage Backend will use the region specified in your OCI credentials configuration.
...
- `bucket_name` `(string: <required>)` - Specifies the name of the bucket that will be used to store the OpenBao data.
