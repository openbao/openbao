...
### Mount and namespace limits

Since [OpenBao v2.1.0](/community/release-notes/2-1-0#features), the mount,
auth, and namespace tables have been split into separate storage entries per
mount, effectively removing the storage-related limits on the number of mounts
and namespaces. These are now managed [with transactions](/community/rfcs/split-mount-tables)
and are split by namespace.

Testing has reached over [300k mounts](/community/rfcs/split-mount-tables/#proof-of-concept)
and 20k namespaces. Benchmarking is provided in the [benchmark-openbao
project](https://github.com/openbao/benchmark-openbao/tree/main/docs/examples/sys).
...
The number of entities can be monitored using OpenBao's [telemetry](telemetry/metrics/authn.mdx); see `vault.identity.num_entities` (total) or `vault.identity.entities.count` (by namespace).
...
[`vault.core.fetch_acl_and_token`](telemetry/metrics/core-system.mdx#vault-core-fetch_acl_and_token)
...
The environment variable [`BAO_CLIENT_TIMEOUT`](../commands/index.mdx#bao_client_timeout) sets a client-side maximum duration as well,
...
[maximum TTL per mount point](../api/system/mounts.mdx#max_lease_ttl-1) can be
configured.
...
[`vault.expire.num_leases`](telemetry/metrics/authn.mdx#vault-expire-num_leases) metric.
