...
These endpoints are unauthenticated from a OpenBao authentication model, but
...
ACME protocol. With EAB however, a OpenBao authenticated client will have to
...
- `format` `(string: "pem")` - Specifies the format for returned data. Can be
  ...
  issuing CA is not a OpenBao-derived self-signed root, it will be concatenated
...
This endpoint can be used both when signing a OpenBao-backed intermediate or
...
<EnterpriseAlert product="openbao">
...
| `GET`  | `/pki/unified-crl`                              | `default` | DER [\[1\]](#openbao-cli-with-der-pem-responses "OpenBao CLI With DER/PEM Responses") | Complete | Unified |
| `GET`  | `/pki/unified-crl/pem`                          | `default` | PEM [\[1\]](#openbao-cli-with-der-pem-responses "OpenBao CLI With DER/PEM Responses") | Complete | Unified |
| `GET`  | `/pki/cert/unified-delta-crl`                   | `default` | JSON                                                                              | Delta    | Unified |
| `GET`  | `/pki/unified-crl/delta`                        | `default` | DER [\[1\]](#openbao-cli-with-der-pem-responses "OpenBao CLI With DER/PEM Responses") | Delta    | Unified |
| `GET`  | `/pki/unified-crl/delta/pem`                    | `default` | PEM [\[1\]](#openbao-cli-with-der-pem-responses "OpenBao CLI With DER/PEM Responses") | Delta    | Unified |
| `GET`  | `/pki/issuer/:issuer_ref/unified-crl`           | Selected  | JSON                                                                              | Complete | Unified |
| `GET`  | `/pki/issuer/:issuer_ref/unified-crl/der`       | Selected  | DER [\[1\]](#openbao-cli-with-der-pem-responses "OpenBao CLI With DER/PEM Responses") | Complete | Unified |
| `GET`  | `/pki/issuer/:issuer_ref/unified-crl/pem`       | Selected  | PEM [\[1\]](#openbao-cli-with-der-pem-responses "OpenBao CLI With DER/PEM Responses") | Complete | Unified |
| `GET`  | `/pki/issuer/:issuer_ref/unified-crl/delta`     | Selected  | JSON                                                                              | Delta    | Unified |
| `GET`  | `/pki/issuer/:issuer_ref/unified-crl/delta/der` | Selected  | DER [\[1\]](#openbao-cli-with-der-pem-responses "OpenBao CLI With DER/PEM Responses") | Delta    | Unified |
| `GET`  | `/pki/issuer/:issuer_ref/unified-crl/delta/pem` | Selected  | PEM [\[1\]](#openbao-cli-with-der-pem-responses "OpenBao CLI With DER/PEM Responses") | Delta    | Unified |
...
| `GET`  | `/pki/unified-ocsp/<base 64+URL encoded ocsp DER request>` | DER [\[1\]](#openbao-cli-with-der-pem-responses "OpenBao CLI With DER/PEM Responses") | Unified |
| `POST` | `/pki/unified-ocsp`                                        | DER [\[1\]](#openbao-cli-with-der-pem-responses "OpenBao CLI With DER/PEM Responses") | Unified |
...
~> Note: When multiple Performance Replication clusters are enabled, each
   ...
   single CRL (externally; as OpenBao does not support this functionality)
...
- `path` `(string: "")` - Specifies the path to this performance replication
  ...
  For example, `https://pr-a.openbao.example.com/v1/ns1/pki-root`.

- `aia_path` `(string: "")` - Specifies the path to this performance replication
  cluster's AIA distribution point; may refer to an external, non-OpenBao responder.
...
~> Note: The `unified_crl`, `unified_crl_on_existing_paths`, and
   ...
   functionality. While they appear in the responses on OpenBao Community Edition, they cannot
   be enabled.
...
- `cross_cluster_revocation` `(bool: false)` -
  <EnterpriseAlert product="openbao" inline /> Enables cross-cluster revocation
  request queues. When a serial not issued on this local cluster is presented
  to OpenBao via the [`/revoke` API](#revoke-certificate), it is replicated
...
- `unified_crl` `(bool: false)` - 
  <EnterpriseAlert product="openbao" inline /> Enables unified CRL and OCSP building. This
...
- `unified_crl_on_existing_paths` `(bool: false)` -
  <EnterpriseAlert product="openbao" inline /> Enables serving the
...
CRL of revocations across distinct OpenBao clusters such as primary and performance replica
clusters.
