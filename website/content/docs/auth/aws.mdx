...
The `aws` auth method provides an automated mechanism to retrieve a OpenBao token
for IAM principals and AWS EC2 instances. Unlike most OpenBao auth methods, this
...
this information already provided by AWS which OpenBao can use to authenticate
clients.
...
Based on how you attempt to authenticate, OpenBao will determine if you are
...
the request. However, it means that the OpenBao server does need network-level
...
replay attacks. In addition, OpenBao allows you to require an additional header,
...
dev OpenBao instance and used to authenticate to a prod OpenBao instance). OpenBao
...
authenticate to OpenBao using this method. It does not appear possible to enforce
an IAM principal to be MFA authenticated while authenticating to OpenBao.

### EC2 auth method

Amazon EC2 instances have access to metadata which describes the instance. The
OpenBao EC2 auth method leverages the components of this metadata to authenticate
and distribute an initial OpenBao token to an EC2 instance. The data flow (which
is also represented in the graphic below) is as follows:

[![OpenBao AWS EC2 Authentication Flow](/img/vault-aws-ec2-auth-flow.png)](/img/vault-aws-ec2-auth-flow.png)
...
1. The AWS EC2 instance makes a request to OpenBao with the PKCS#7 signature.
   The PKCS#7 signature contains the Instance Identity Document.

1. OpenBao verifies the signature on the PKCS#7 document, ensuring the information
   is certified accurate by AWS. This process validates both the validity and
   integrity of the document data. As an added security measure, OpenBao verifies
   that the instance is currently running using the public EC2 API endpoint.

1. Provided all steps are successful, OpenBao returns the initial OpenBao token to
...
Clients authenticating to OpenBao must have an ARN that matches one of the ARNs bound to
...
account to login to it. If you wish to specify a wildcard, you must give OpenBao
...
With the iam auth method, normally OpenBao will see the IAM principal that
authenticated, either the IAM user or role. However, when you have an EC2
instance in an IAM instance profile, OpenBao can actually see the instance ID of
the instance and can "infer" that it's an EC2 instance. However, there are
important security caveats to be aware of before configuring OpenBao to make that
inference.
...
latter value which OpenBao actually sees.
...
instance's instance ID. Thus, it is possible for OpenBao to extract the instance
ID out of the value it sees when an EC2 instance in an instance profile
authenticates to OpenBao with the iam auth method. This is known as
"inferencing." OpenBao can be configured, on a role-by-role basis, to infer that a
...
simply pass in your instance's instance ID and spoof your instance to OpenBao.
...
that OpenBao cannot offer an iron-clad guarantee about the inference and it is up
...
OpenBao allows you to configure using either the ec2 auth method or the iam auth
method, but not both auth methods. Further, **assumed roles are not supported**
and OpenBao prevents you from enforcing restrictions that it cannot enforce given
...
3. You configure a role with the iam auth type and further configure
   ...
   ID viewable by OpenBao, and the instance must have come from the bound AMI ID.

## Comparison of the IAM and EC2 methods

The iam and ec2 auth methods serve similar and somewhat overlapping
functionality, in that both authenticate some type of AWS entity to OpenBao.
...
- How the entities are authenticated
  - The ec2 auth method authenticates instances by making use of the EC2
    ...
    infrequently, so OpenBao adds a number of other constructs to mitigate against
  ...
  - The iam auth method authenticates by having clients provide a specially
    ...
    the signature and tell OpenBao who created it. The actual secret (i.e.,
...
(e.g., if you're running OpenBao on an EC2 instance in an IAM instance profile),
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": ["arn:aws:iam::<AccountId>:role/<OpenBaoRole>"]
    },
    {
...
Here are some of the scenarios in which OpenBao would need to use each of these
permissions. This isn't intended to be an exhaustive list of all the scenarios
in which OpenBao might make an AWS API call, but rather illustrative of why these
are needed.
...
- `iam:GetInstanceProfile` is used when you have a `bound_iam_role_arn` in the
  `ec2` auth method. OpenBao needs to determine which IAM role is attached to the
  instance profile.
...
- The `ManageOwnAccessKeys` stanza is necessary when you have configured OpenBao
...
access to an EC2 instance), it can impersonate that instance and fetch a OpenBao
...
other resources provided by or resident in OpenBao.
...
If in a given organization's architecture, a client fetches a long-lived OpenBao
...
certificate, which covers most AWS regions, is already included in OpenBao and
does not need to be added. Instances whose PKCS#7 signatures cannot be
verified by the default public certificate included in OpenBao can register a
...
An EC2 instance, after authenticating itself with the method, gets a OpenBao token.
...
To allow OpenBao to authenticate IAM principals and EC2 instances in other
accounts, OpenBao supports using AWS STS (Security Token Service) to assume AWS
IAM Roles in other accounts. For each target AWS account ID, you configure the
IAM Role for OpenBao to assume using the `auth/aws/config/sts/<account_id>` and
OpenBao will use credentials from assuming that role to validate IAM principals
and EC2 instances in the target account.

The account in which OpenBao is running (i.e. the master account) must be listed as
a trusted entity in the IAM Role being assumed on the remote account. The Role itself
should allow the permissions specified in the [Recommended OpenBao IAM
Policy](#recommended-openbao-iam-policy) except it doesn't need any further
`sts:AssumeRole` permissions.

Furthermore, in the master account, OpenBao must be granted the action `sts:AssumeRole`
...
If not specified, OpenBao will attempt to use standard environment variables
...
#### Enable AWS authentication in OpenBao.
...
The following example demonstrates the AWS (IAM) auth method to authenticate with OpenBao.
...
import (
	...
	openbao "github.com/lf-edge/openbao/api"
	auth "github.com/lf-edge/openbao/api/auth/aws"
)

// Fetches a key-value secret (kv-v2) after authenticating to OpenBao via AWS IAM,
...
func getSecretWithAWSAuthIAM() (string, error) {
	...
	awsAuth, err := auth.NewAWSAuth(
		auth.WithRole("dev-role-iam"), // if not provided, OpenBao will fall back on looking for a role with the IAM role name if you're using the iam auth type, or the EC2 instance's AMI id if using the ec2 auth type
	)
	if err != nil {
