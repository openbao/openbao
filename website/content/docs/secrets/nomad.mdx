...
```shell-session
$ openbao write nomad/config/lease ttl=3600 max_ttl=86400
...
```shell-session
$ openbao write nomad/config/access \
...
```shell-session
$ openbao write nomad/role/monitoring policies=readonly
...
```shell-session
$ openbao read nomad/creds/monitoring
Key              Value
---              -----
