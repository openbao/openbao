...
- `tidy_revoked_certs` `(bool: false)` - Set to true to remove all invalid revoked 
  certs and expired certificates from storage. A revoked storage entry is considered
...
- `tidy_invalid_certs` `(bool: false)` - Set to true to remove all invalid certificates 
  from storage. A certificate is considered invalid if it cannot be parsed by X.509's
  certificate parser.
