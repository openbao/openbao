...
The core of a profile is the request, which contains the following fields:
 ...
 - `for` `([]any: optional)` - when present, yields either a list or map of
   objects to iterate through. `this` is set to the value and `this_index`
   is set to the index in the list or key of the map in the request. `for` is
   computed head of any other field being evaluated (including `when`),
   letting other fields depend on it. If this request's outer block also has a
   `for` block, the outer block's `this` and `this_index` will be moved to
   `outer_this` and `outer_this_index` respectively.
...
Inside an outer block, the following fields are supported:
 ...
 - `for` `([]any: optional)` - when present, yields either a list or map of
   ...
   is set to the index in the list or key of the map in the request. `for` is
   computed head of any other field being evaluated (including `when`),
   letting other fields depend on it. If a contained request also has a `for`
   block, the outer block's `this` and `this_index` will be moved to
   `outer_this` and `outer_this_index` respectively.
...
- A data source evaluation. These take the form:
  ...
  - `eval_type` `(string: <required>)` - the output type after evaluation;
    ...
    `map` (alias for `map[string]any`), and `interface{}` (alias for
    `any`). If the output of the source evaluation is not convertible
...
- `data` `(map[string]any: <optional>)`: additional context for the
  templating engine.
