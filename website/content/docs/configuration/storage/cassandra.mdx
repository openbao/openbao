...
description: |-
  The Cassandra storage backend is used to persist OpenBao's data in an Apache
  Cassandra cluster.
---
...
```cql
CREATE KEYSPACE "openbao" WITH REPLICATION = {
    'class': 'SimpleStrategy',
    'replication_factor': 1
};

CREATE TABLE "openbao"."entries" (
    bucket text,
    key text,
    value blob,
...
- `keyspace` `(string: "openbao")` Cassandra keyspace to use.
