...
the `307` status code. There is a `"/sys/config/ui"` [API endpoint](../../api/system/config-ui.mdx) which allows users
to set `UI` specific custom headers. If a header is configured in a configuration file, it is not allowed
to be reconfigured through the `"/sys/config/ui"` [API endpoint](../../api/system/config-ui.mdx). In cases where a
...
in the `"/sys/config/ui"` [API endpoint](../../api/system/config-ui.mdx), the header in the configuration file takes precedence.
For example, the `"Content-Security-Policy"` header is defined by default in the
`"/sys/config/ui"` [API endpoint](../../api/system/config-ui.mdx). If that header is also defined in the configuration file,
the value in the configuration file is set in the response headers instead of the
default value in the `"/sys/config/ui"` [API endpoint](../../api/system/config-ui.mdx).

## `tcp` listener parameters

- `address` `(string: "127.0.0.1:8200")` <a name="address"></a> – Specifies the address to bind to for
...
- `cluster_address` `(string: "127.0.0.1:8201")` <a name="cluster_address"></a> – Specifies the address to bind
...
- `disable_unauthed_generate_root_endpoints` `(bool: true)` - Whether to
  disable requests to the legacy unauthenticated generate root endpoints
  (under `/sys/generate-root/*`). These are a security risk to leave exposed
  on public listeners: unauthenticated users can cancel any inflight root
  generation requests.

  :::warning

  **As of OpenBao v2.5.3, this parameter defaults to true, forbidding any calls
  to the unauthenticated generate root endpoints.**
...
- `tls_key_exchange_preferences` `([]string: nil)` - Specifies the list of supported
  ...
  "SecP384r1MLKEM1024", "X25519", "X25519MLKEM768", and "MLKEM1024". They follow
  the Go standard library [CurveIDs][curve-id].
...
listener "tcp" {
  ...
  tls_key_exchange_preferences = ["X25519MLKEM768", "SecP256r1MLKEM768", "SecP384r1MLKEM10242", "MLKEM1024"]
}
```
