...
- `vault.hashicorp.com/template-static-secret-render-interval` - If specified,
  ...
  See [Vault Agent Template Config](/docs/agent-and-proxy/agent/template#template-configurations) for more details.
...
- `vault.hashicorp.com/agent-cache-enable` - configures Vault Agent to enable
  [caching](/docs/agent-and-proxy/agent/caching). In Vault 1.7+ this annotation will also enable
...
- `vault.hashicorp.com/agent-enable-quit` - enable the [`/agent/v1/quit` endpoint](/docs/agent-and-proxy/agent#quit) on an injected agent. This option defaults to false, and if true will be set on the existing cache listener, or a new localhost listener with a basic cache stanza configured. The [agent-cache-listener-port annotation](/docs/platform/k8s/injector/annotations#vault-hashicorp-com-agent-cache-listener-port) can be used to change the port.

- `vault.hashicorp.com/agent-telemetry` - specifies the [telemetry](/docs/configuration/telemetry) configuration for the
...
- `vault.hashicorp.com/auth-config` - configures additional parameters for the configured
  ...
  see the Vault Agent [auto-auth documentation](/docs/agent-and-proxy/autoauth/methods).
...
- `vault.hashicorp.com/auth-min-backoff` - set the [min_backoff](/docs/agent-and-proxy/autoauth#min_backoff) option in the auto-auth config. Requires Vault 1.11+.

- `vault.hashicorp.com/auth-max-backoff` - set the [max_backoff](/docs/agent-and-proxy/autoauth#max_backoff) option in the auto-auth config

- `vault.hashicorp.com/agent-auto-auth-exit-on-err` - set the [exit_on_err](/docs/agent-and-proxy/autoauth#exit_on_err) option in the auto-auth config
...
- `vault.hashicorp.com/agent-disable-idle-connections` - Comma-separated [list
  of Vault Agent features](/docs/agent-and-proxy/agent#disable_idle_connections) where idle
...
- `vault.hashicorp.com/agent-disable-keep-alives` - Comma-separated [list of
  Vault Agent features](/docs/agent-and-proxy/agent#disable_keep_alives) where keep-alives
