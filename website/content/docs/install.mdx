...
$ RELEASE="openbao_${VERSION}_${PLATFORM}.tar.gz"
$ wget "https://github.com/openbao/openbao/releases/download/v$VERSION/$RELEASE"
$ wget "https://github.com/openbao/openbao/releases/download/v$VERSION/checksums.txt"
$ wget "https://github.com/openbao/openbao/releases/download/v$VERSION/checksums.txt.gpgsig"
$ wget "https://github.com/openbao/openbao/releases/download/v$VERSION/checksums.txt.sigstore.json"
```
...
bao_2.6.0_freebsd_amd64.tar.gz: OK
```
...
To verify GPG signed artifacts, use `gpg2` from the command line. For example,
to verify `checksums.txt` with the `checksums.gpgsig` stored locally:
...
to verify `checksums.txt` with the `checksums.sigstore.json` stored locally:
...
OpenBao will always publish releases from either the protected `main` branch
or the protected `release/...` set of branches. This is reflected in the
certificate identity checked by cosign.

You can enforce that an artifact was signed by a workflow running on a
particular branch by passing a fixed `--certificate-identity` instead of
`--certificate-identity-regexp`:

If published from the wrong branch, the output will look like this:

```shell-session
$ cosign verify-blob --bundle checksums.txt.sigstore.json \
                     --certificate-identity='https://github.com/openbao/openbao/.github/workflows/release.yml@refs/heads/release/2.6.x' \
                     --certificate-oidc-issuer='https://token.actions.githubusercontent.com' \
                     checksums.txt
Error: failed to verify certificate identity: no matching CertificateIdentity found, last error: expected SAN value "https://github.com/openbao/openbao/.github/workflows/release.yml@refs/heads/release/2.6.x", got "https://github.com/openbao/openbao/.github/workflows/release.yml@refs/heads/bad-branch"
error during command execution: failed to verify certificate identity: no matching CertificateIdentity found, last error: expected SAN value "https://github.com/openbao/openbao/.github/workflows/release.yml@refs/heads/release/2.6.x", got "https://github.com/openbao/openbao/.github/workflows/release.yml@refs/heads/bad-branch"
```
