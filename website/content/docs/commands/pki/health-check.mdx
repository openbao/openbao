...
1. Delete any expired CAs with one of the following options:
  - Run [tidy](../../api/secret/pki.mdx#tidy) manually with `bao write <mount>/tidy tidy_expired_issuers=true`.
  - Use the OpenBao API to call [delete issuer](../../api/secret/pki.mdx#delete-issuer).
...
1. Use [BYOC revocations](../../api/secret/pki.mdx#revoke-certificate) to
...
1. Make sure that you are not renewing certificates too soon. Certificate
   ...
   lifespan.
1. Consider setting the `no_store` field for all roles to `true` and use [BYOC revocations](../../api/secret/pki.mdx#revoke-certificate) to avoid storage.
...
Review the [ACME Certificate Issuance](../../api/secret/pki.mdx#acme-certificate-issuance)
