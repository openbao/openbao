...
description: |-
  The Google Cloud Storage storage backend is used to persist OpenBao's data in
...
- **High Availability** – the Google Cloud Storage storage backend supports high
  ...
  time on the OpenBao node to acquire sessions, clock skew across OpenBao servers
...
To use the Google Cloud Storage OpenBao storage backend, you must have a Google
...
```shell-session
$ gsutil mb gs://mycompany-openbao-data
```
...
a service account that limits OpenBao's interactions with Google Cloud to objects
  ...
  "bindings": [
    {
      ...
      "members": ["serviceAccount:my-openbao@gserviceaccount.com"]
    }
  ]
}
```

Then give OpenBao the service account's credential file as a configuration option.
...
The Google Cloud Storage OpenBao storage backend uses the official Google Cloud
...
- `chunk_size` `(string: "8192")` – Specifies the maximum size (in kilobytes) to
  ...
  objects in OpenBao, it is recommended to set this to a low value (minimum is
  256), since it will reduce the amount of memory OpenBao uses. Alternatively, this parameter
...
data to OpenBao, setting a lower number can reduce OpenBao's memory consumption, but
