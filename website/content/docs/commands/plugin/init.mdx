...
The `plugin init` command downloads OCI-based plugins configured in OpenBao server configuration files. It reads [plugin definitions](/docs/configuration/oci-plugins) from the configuration and downloads the specified OCI images containing plugin binaries.

Downloaded plugins are placed in the plugin directory but are **not** automatically registered in the plugin catalog. Use [`plugin register`](/docs/commands/plugin/register) to register downloaded plugins.
...
The following flags are available in addition to the [standard set of flags](/docs/commands) included on all commands.
...
- Valid credentials for private registries (if configured in `plugin_oci_auth`)
