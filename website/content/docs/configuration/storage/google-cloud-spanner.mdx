...
description: |-
  The Google Cloud Spanner storage backend is used to persist OpenBao's data in
...
To use the Google Cloud Spanner OpenBao storage backend, you must have a Google
...
```sql
CREATE TABLE OpenBao (
  Key       STRING(MAX) NOT NULL,
  Value     BYTES(MAX),
) PRIMARY KEY (Key);

CREATE TABLE OpenBaoHA (
  Key           STRING(MAX) NOT NULL,
  Value         STRING(MAX),
...
The Google Cloud Spanner OpenBao storage backend uses the official Google Cloud
...
- `table` `(string: "OpenBao")` - Specifies the name of the table where
...
- `ha_table` `(string: "OpenBaoHA")` - Specifies the name of the table to use for
...
storage "spanner" {
  database   = "projects/demo/instances/abc123/databases/openbao-data"
  ha_enabled = "true"
}
```
...
Cloud Spanner OpenBao storage backend.

```hcl
storage "spanner" {
  database = "projects/demo/instances/abc123/databases/openbao-data"
  table    = "OpenBaoData"
  ha_table = "OpenBaoLeader"
}
```
