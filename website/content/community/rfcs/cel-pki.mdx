...
It takes in a set of parameters  like `common_name`, `alt_names`, &c similar to [pki/issue/:name](/docs/api/secret/pki/#generate-certificate-and-key).
...
`alt_names`, &c similar to [pki/sign/:name](/docs/api/secret/pki/#sign-certificate)), subject to the
...
Certain policies can be implemented using [roles](/docs/api/secret/pki/#createupdate-role),
