...
1.  Configure OpenBao with the proper plugin and connection information:
    ...
    ```text
    $ openbao write database/config/my-mongodb-database \
...
1.  Configure a role that maps a name in OpenBao to a MongoDB command that executes and
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    username           v-openbaouser-my-role-ItceCZHlp0YGn90Puy9Z-1602542024
    ```
...
from MongoDB with the exception that the OpenBao parameters are the contents of those files, not filenames. As such,
