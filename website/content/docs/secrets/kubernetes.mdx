...
~> **Note:** We do not recommend using tokens created by the Kubernetes Secrets Engine to
   authenticate with the [OpenBao Kubernetes Auth Method](/vault/docs/auth/kubernetes). This will
...
1. Enable the Kubernetes Secrets Engine:
   ...
   ```shell-session
   $ openbao secrets enable kubernetes
...
```shell-session
$ openbao write kubernetes/creds/my-role \
...
```shell-session
$ openbao write kubernetes/roles/auto-managed-sa-role \
...
```shell-session
$ openbao write kubernetes/creds/auto-managed-sa-role \
    kubernetes_namespace=test
Key                          Value
---                          -----
...
```shell-session
$ openbao write kubernetes/roles/auto-managed-sa-and-role \
    allowed_kubernetes_namespaces="test" \
    generated_role_rules='{"rules":[{"apiGroups":[""],"resources":["pods"],"verbs":["list"]}]}'
```
You can then get credentials in the same way as before.
```shell-session
$ openbao write kubernetes/creds/auto-managed-sa-and-role \
    kubernetes_namespace=test
Key                          Value
---                          -----
