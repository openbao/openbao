...
1. **Simple plugin deployment:**
   ...
   is recommended plugin deployment flow.
2. **`kms` plugin deployment:**
   `kms` plugin cannot be registered via the API (since they must be available in a sealed state).
   The declarative method is the only way to define `kms` plugins.

The `plugin` stanza allows the declaration of both OCI images and manually downloaded plugins.
