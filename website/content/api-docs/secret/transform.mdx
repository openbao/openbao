...
`/transform` path in OpenBao. Since it is possible to enable secrets engines at any
...
- `stores` `(list: ["builtin/internal"])` -
  The list of tokenization stores to use for tokenization state. OpenBao's
...
~> **Note**: Transformations whose key material was generated by OpenBao do not support
...
- `connection_string` `(string: <required>)` -
  ...
  OpenBao will use for locating and connecting to a database. Each
...
- `min_decryption_version` `(int: <optional>)` –
  Specifies the minimum key version that openbao can use to decode values for the
