...
OpenBao uses GitHub repository rulesets to protect all important project repositories. The rulsets in place do not allow foce pushes to to the default as well as `release/*` branches. In addition, the GitHub UI does not allow deleting the default branch.
...
1. Github issues can be filed by anyone who has a Github account.
2. Zulip is open for registration. Check out the [contributing](https://openbao.org/community/contributing/) page of the documentation.
2. General feedback to the maintainers (and everyone else subscribed) can be provided via openbao@lists.openssf.org mailing list.
...
OpenBao is released under Mozilla Public License 2.0, which is [compatibile to GPLv3](https://www.gnu.org/licenses/license-list.html#apache2).
