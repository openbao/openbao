...
size by reconfiguring OpenBao or Consul to a larger maximum storage
...
OpenBao shards the entities across 256 storage entries. This creates a
...
The number of entities can be monitored using OpenBao's [telemetry](/vault/docs/internals/telemetry#token-identity-and-lease-metrics); see `vault.identity.num_entities` (total) or `vault.identity.entities.count` (by namespace).
...
The maximum duration of a OpenBao operation is
...
Monitor the active OpenBao node's CPU and network utilization, as well as
