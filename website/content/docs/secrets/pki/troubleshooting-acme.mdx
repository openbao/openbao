...
(EAB)](../../api/secret/pki.mdx#acme-external-account-bindings), the
...
[ACME configuration](../../api/secret/pki.mdx#set-acme-configuration),
new account registration (and reuse of existing accounts will fail).

### Resolution

Using an OpenBao token, [fetch a new external account
binding](../../api/secret/pki.mdx#get-acme-eab-binding-token) for
the [desired directory](../../api/secret/pki.mdx#acme-directories):
...
[challenge type](../../api/secret/pki.mdx#acme-challenge-types) (`dns-01`,
...
including setting [any custom DNS resolver](../../api/secret/pki.mdx#dns_resolver).
...
If you run a [manual tidy](../../api/secret/pki.mdx#tidy_acme) or have
[auto-tidy](../../api/secret/pki.mdx#configure-automatic-tidy) enabled
