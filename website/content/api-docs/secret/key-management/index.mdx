...
`/keymgmt` path in OpenBao. Since it is possible to enable secrets engines at any
...
- `provider` `(string: <required>)` – Specifies the name of a KMS provider that's external to
  OpenBao. Cannot be changed after creation. For more information about each provider, refer to
