...
Revocation can happen manually via the API, via the `openbao lease revoke` cli command,
...
When reading a dynamic secret, such as via `openbao read`, OpenBao always returns a
`lease_id`. This is the ID used with commands such as `openbao lease renew` and `openbao lease revoke` to manage the lease of the secret.
...
example, `openbao lease renew -increment=3600 my-lease-id` would request that the TTL of the lease
...
example, to revoke all AWS access keys, you can do `openbao lease revoke -prefix aws/`.
