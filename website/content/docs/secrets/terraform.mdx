...
on every path, use `openbao path-help` after mounting the backend.
...
1.  Enable the Terraform Cloud secrets engine:
    ...
    ```shell-session
    $ openbao secrets enable terraform
...
2.  Configure OpenBao to connect and authenticate to Terraform Cloud:
    ...
    ```shell-session
    $ openbao write terraform/config \
...
3.  Configure a role that maps a name in OpenBao to a Terraform Cloud User. At
    ...
    ```shell-session
    $ openbao write terraform/role/my-role user_id=user-12345abcde
...
```shell-session
$ openbao read terraform/creds/my-role
Key                Value
---                -----
...
```shell-session
$ openbao write terraform/role/testing organization="${TF_ORGANIZATION}"
Success! Data written to: terraform/role/testing

$ openbao write -f terraform/rotate-role/testing
...
```
$ openbao read terraform/creds/testing
...
```shell-session
$ openbao write terraform/role/user-testing user_id="${TF_USER_ID}"
...
```
$ openbao read terraform/creds/user-testing
