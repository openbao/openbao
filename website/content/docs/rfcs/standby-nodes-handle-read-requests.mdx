...
```
Sealed Node → Unseal → Standby Node → [Waits for Leadership] → Active Node
                       	   ↑                                        ↑
                      No postUnseal                       postUnseal called
                      (minimal state)                    (full initialization)
```
...
Forwarding should first depend on the request method. If it's write request → forward. If read request → check if standby can serve reads and handle locally. All other logic stays the same.

in `http/handler.go`:

forwarding should first depend on request method, if it's write request and not leader - forward. If read request - try to serve.
...
Read Request - doesn't mean, that we are not writing to storage! 

If the logical backend attempts to write to storage, the storage layer, being in read-only state, returns `logical.ErrReadOnly`. On standby nodes, this error should be translated to `logical.ErrStandbyPleaseForward`.
