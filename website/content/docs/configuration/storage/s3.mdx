...
The S3 storage backend is used to persist OpenBao's data in an [Amazon S3][s3]
bucket.
...
- `kms_key_id` `(string: "")` - Specifies the ID or Alias of the KMS key used to
  encrypt data in the S3 backend. OpenBao must have `kms:Encrypt`, `kms:Decrypt`
  and `kms:GenerateDataKey` permissions for this KMS key. You can use 
  `alias/aws/s3` to specify the default key for the account.

- `path` `(string: "")` - Specifies the path in the S3 Bucket where OpenBao
  data will be stored.
