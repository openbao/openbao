...
description: |-
  The MongoDB Atlas Secrets Engine for OpenBao generates MongoDB Atlas Programmatic API Keys dynamically.
---

# MongoDB atlas secrets engine

The MongoDB Atlas Secrets Engine generates Programmatic API keys for MongoDB Atlas. This allows one to manage the lifecycle of these MongoDB Atlas secrets through OpenBao. The created MongoDB Atlas secrets are
time-based and are automatically revoked when the OpenBao lease expires, unless renewed. OpenBao will create a Programmatic API key for each lease scoped to the MongoDB Atlas project or organization denoted with the included role(s). An IP Whitelist may also be configured for the Programmatic API key with desired IPs and/or CIDR blocks.
...
Programmatic API Key credential types create a OpenBao role to generate a Programmatic API Key at
