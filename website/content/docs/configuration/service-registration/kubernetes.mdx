...
- `vault-perf-standby` `(string: "true"/"false")` – Vault performance standby is updated dynamically each
  time OpenBao's leader/standby status changes. **This field is only valuable if the pod is a member of a performance standby cluster**,
  it will simply be set to "false" when it's not applicable. True indicates that this OpenBao pod is currently a performance standby. False indicates
  that this OpenBao pod is currently a performance leader.
...
setting up OpenBao replication, it can be used as the primary address:

```shell-session
$ openbao write -f sys/replication/performance/primary/enable \
    primary_cluster_addr='https://openbao-active-us-east:8201'
```
