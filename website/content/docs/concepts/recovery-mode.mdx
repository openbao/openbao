...
- if using Integrated Storage, run `openbao status` on each node to find the highest-index ones
...
Each node's AppliedIndex value can be obtained by running `openbao status` against
