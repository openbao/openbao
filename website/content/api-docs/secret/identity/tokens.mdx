...
This endpoint can verify the authenticity and active state of an access token. Access tokens can be acquired from the
`identity/oidc/provider/:name/token` endpoint, in the `access_token` field of the response.

### Authentication oidc-client

The endpoint uses `client-credentials` authentication, hence either `client_secret_basic` or `client_secret_post` must
be used. It is required that the oidc-client used for authentication is of type `confidential` and hence the
`client_secret` is required.

### Access-token oidc-client

The access-token provided to the endpoint may have been created by the same, or another oidc-client which client-type
could be either `confidential` or `public`. The oidc-client that generated the access-token will be reflected in the
`aud` (audience) field in the response. To introspect an access-token created by a public oidc-client, one needs to
authenticate with a confidential oidc-client.

| Method  | Path                                     |
| :------ | :--------------------------------------- |
| `POST`  | `/identity/oidc/introspect-access-token` |

### Parameters

- `token` `(string: <required>)` - The `access_token` provided by the Provider-Token endpoint.

- `client_id` `(string: <optional>)` - The Client ID of the oidc-client (`client_type` must be `confidential`). This parameter is required when using the `client_secret_post` client authentication method.

- `client_secret` `(string: <optional>)` - The Client Secret of the oidc-client. This parameter is required when using the `client_secret_post` client authentication method.
...
- `Authorization: Basic` `(string: <optional>)` - An HTTP Basic authentication scheme header
  ...
  authentication method. This header is required when using the `client_secret_basic` client authentication method.

### Sample request (using `client_secret_basic`)
...
$ curl \
    ...
    -d "token=b.AAAAAQJEH5VXjfjUESCwySTKk2MS1MGVNc9oU-N2EyoLKVo9SYa-NnOWAX[...]" \
    http://127.0.0.1:8200/v1/identity/oidc/introspect-access-token
```

### Sample request (using `client_secret_post`)
...
### Sample response (valid access-token)

The `aud` field refers to the oidc-client that created the access-token.

```json
{
  "active": true,
  "aud": "avn23t50atv0q2c5m08q23tv02v8",
  "iat": 1742895855,
  "exp": 1742934752
}
```

### Sample response (invalid access-token)

If the access-token is invalid, expired or otherwise problematic:
