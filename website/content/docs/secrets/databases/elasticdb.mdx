...
used by OpenBao. [Here is an example using OpenSSL](https://stackoverflow.com/questions/15144046/converting-pkcs12-certificate-into-pem-using-openssl)
...
### Create a role for OpenBao

Next, in Elasticsearch, we recommend that you create a user just for OpenBao to use in managing secrets.

To do this, first create a role that will allow OpenBao the minimum privileges needed to administer users and passwords by performing a
POST to Elasticsearch. To do this, we used the `elastic` openbaouser whose password we created in the
...
$ curl \
    ...
    http://openbaouser:$PASSWORD@localhost:9200/_xpack/security/role/openbao
```

Next, create a user for OpenBao associated with that role.
...
$ curl \
    ...
    -H "Content-Type: application/json" \
    -d @data.json \
    http://openbaouser:$PASSWORD@localhost:9200/_xpack/security/user/openbao
```
...
```json
{
 ...
 "full_name" : "OpenBao",
 "metadata" : {
   "plugin_name": "OpenBao Plugin Database Elasticsearch",
 }
}
```

Now, Elasticsearch is configured and ready to be used with OpenBao.
...
1. Configure OpenBao with the proper plugin and connection information:
   ...
   ```shell-session
   $ openbao write database/config/my-elasticsearch-database \
...
1. Configure a role that maps a name in OpenBao to a role definition in Elasticsearch.
   ...
   ```shell-session
   $ openbao write database/roles/internally-defined-role \
...
1. Alternatively, configure a role that maps a name in OpenBao to a pre-existing
   role definition in Elasticsearch:

   ```shell-session
   $ openbao write database/roles/externally-defined-role \
         db_name=my-elasticsearch-database \
...
1. Generate a new credential by reading from the `/creds` endpoint with the name
   ...
   username           v-openbaouser-my-role-AgIViC5TdQHBdeiCxae0-1602541724
   ```
