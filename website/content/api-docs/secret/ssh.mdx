...
- [Managing Issuers](#managing-issuers)
  ...
  - [Read Issuer Public Key (Unauthenticated)](#read-oissuer-public-key-unauthenticated)
...
Authority (CAs) certificates ("issuers") in a single mount, for the purpose of
facilitating rotation. For that, the concept of a default issuer is introduced.
The default issuer is a reference to the issuer that will be used for signing
or issueing new SSH keys and perform issuing operations if no issuer reference
is provided in the role creation. This reference can be updated at any time
by submitting/generating a new issuers and setting it as default or updating
it through the configuration endpoint.

:::warning

**Note**: Some functionality will not work if a default issuer is not
configured. OpenBao automatically selects the pre-configured issuer as
the default issuer from the current issuing certificate on migration
from an older OpenBao version.
...
it will be overriden with the new CA information and any reference to it updated._
...
- `issuer_name` `(string: "")` - Specifies an explicit name to the issuer. This
  name can later by used to refer to the issuer. This is part of the request URL.
...
- `set_default` `(bool: false)` - Specifies whether the submitted or generated CA
  ...
  default issuer for performing operations by any role referngin it. Only one
