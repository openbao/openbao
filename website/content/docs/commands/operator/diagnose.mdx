...
Warn or fail prefixes in nested checks will bubble up to the parent if the prefix supersedes the
parent prefix. Fail supersedes warn, and warn supersedes ok. For example, if the TLS checks under
...
Ensures that one of the `BAO_API_ADDR`, `BAO_REDIRECT_ADDR`, or `BAO_ADVERTISE_ADDR`
environment variables are set, or that the redirect address is specified in the OpenBao
configuration.

#### Check cluster address

Parses the cluster address from the `BAO_CLUSTER_ADDR` environment variable, or from the
