...
OpenBao executes and communicates with over RPC. Each time a OpenBao secret engine,
...
aws                 v1.12.0+builtin.openbao
# ...
```

Here, OpenBao has a dependency on `v0.14.0` of the [hashicorp/vault-plugin-secrets-ad](https://github.com/hashicorp/vault-plugin-secrets-ad)
repo, and the `openbao` metadata identifier for `aws` indicates that plugin's code was
