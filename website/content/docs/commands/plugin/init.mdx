...
The `plugin init` command downloads OCI-based plugins configured in OpenBao server configuration files. It reads [plugin definitions](../../configuration/oci-plugins.mdx) from the configuration and downloads the specified OCI images containing plugin binaries.
