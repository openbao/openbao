...
description: |-
  The Google Cloud secrets engine for OpenBao dynamically generates Google Cloud
...
1.  Configure the secrets engine with account credentials, or leave blank or unwritten
    ...
    ```shell-session
    $ openbao write gcp/config credentials=@my-credentials.json
...
```shell-session
$ openbao write gcp/roleset/my-token-roleset \
...
```shell-session
$ openbao write gcp/roleset/my-key-roleset \
    project="my-project" \
...
```shell-session
$ openbao write gcp/roleset/my-roleset
...
```shell-session
$ openbao write gcp/static-account/my-token-account \
...
```shell-session
$ openbao write gcp/static-account/my-key-account \
    service_account_email="account@my-project.iam.gserviceaccount.com" \
    secret_type="service_account_key"  \
    bindings=-<<EOF
      resource "//cloudresourcemanager.googleapis.com/projects/my-project" {
...
```shell-session
$ openbao write gcp/static-account/my-account
    bindings=@mybindings.hcl
    ...
```
...
```shell-session
$ openbao write gcp/impersonated-account/my-token-impersonate \
...
**Roleset:**
```shell-session
$ openbao read gcp/roleset/my-token-roleset/token
...
**Static account:**
```shell-session
$ openbao read gcp/static-account/my-token-account/token
...
```shell-session
$ openbao read gcp/impersonated-account/my-token-impersonate/token
...
```shell-session
$ openbao read gcp/roleset/my-key-roleset/key
...
The Google Cloud OpenBao secrets engine is written as an external OpenBao plugin and
thus exists outside the main OpenBao repository. It is automatically bundled with
OpenBao releases, but the code is managed separately.
