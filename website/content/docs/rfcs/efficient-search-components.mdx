...
Each query may optionally include a `match` or `prefix` field to control how values are compared. "exact" is the default and can be omitted.
...
The current storage system has the limitation of a one-to-one mapping between keys and nodes. This means that each key can only have one node associated with it, which can be a bottleneck when dealing with large amounts of data. To overcome this limitation, a new storage system that supports multiple nodes per key should be implemented. A page-based storage system can be used to achieve this.
