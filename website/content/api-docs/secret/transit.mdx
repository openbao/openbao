...
This endpoint uses the already existing named base key and the derivation parameters to derive a new named key.

| Method | Path                        |
| :----- | :-------------------------- |
| `POST` | `/transit/derive-key/:name` |

### Parameters

- `name` `(string: <required>)` – Specifies the name of the output derived key.
  This is specified as part of the URL.

- `key_derivation_algorithm` `(string: ecdh)` – Key derivation algorithm to use. The
  currently-supported algorithms are:

  - `ecdh` – Elliptic-curve Diffie–Hellman (ECDH) key agreement using HMAC Key Derivation Function.

- `nonce` `(string: "")` – A base64 encoded nonce used as salt by the underlying HMAC Key Derivation Function.
  When this parameter is used, the both parties have to provide the same nonce for getting the same derived key.

- `peer_public_key` `(string: <required>)` - The pem-encoded other party's ECC public key.
  This API assumes that only trusted public keys are provided by the caller.

- `base_key_name` `(string: <required>)` – Name of the base key to use for derivation (own private key for ECDH).

- `base_key_version` `(int, optional)` - The version of the base key to use for derivation.
 Must be 0 (for latest) or a value greater than or equal
 to the min_derivation_version configured on the key.

- `derived_key_type` `(string: "aes256-gcm96")` – Specifies the type of the output derived key. The
  currently-supported types are:
