...
description: |-
  ...
  roles for the Redis database, and also supports [Static Roles](/docs/secrets/databases#static-roles).
---
...
1.  Configure OpenBao with the proper plugin and connection configuration:
    ...
    ```shell-session
    $ bao write database/config/my-redis-database \
...
1.  You should consider rotating the admin password. Note that if you do, the
    ...
    ```shell-session
    $ bao write -force database/rotate-root/my-redis-database
    ```
