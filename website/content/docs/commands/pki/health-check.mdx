...
```shell-session
$ openbao pki health-check pki-root/
```

Configuration can be specified using the `-health-config` flag:

```shell-session
$ openbao pki health-check -health-config=mycorp-root.json pki-root/
```
...
```shell-session
$ openbao pki health-check -list pki-root/
```
...
1. Delete any expired CAs with one of the following options:
  - Run [tidy](/vault/api-docs/secret/pki#tidy) manually with `openbao write <mount>/tidy tidy_expired_issuers=true`.
  - Use the OpenBao API to call [delete issuer](/vault/api-docs/secret/pki#delete-issuer).
...
Use `openbao write` to enable CRL auto-rebuild:

```shell-session
$ openbao write <mount>/config/crl auto_rebuild=true
```
...
Use `openbao secrets tune` to set the desired audit parameters:

```shell-session
$ openbao secrets tune \
...
1. Update `allowed_response_headers` and `passthrough_request_headers` for all
   policies with `openbao secrets tune`:
...
1. Update ACME-specific headers with `openbao secrets tune` (if you are using ACME):
...
Use `openbao write` to enable auto-tidy with the recommended defaults:

```shell-session
$ openbao write <mount>/config/auto-tidy \
    enabled=true \
...
1. Schedule a manual run of tidy with `openbao write`:

  ```shell-session
  $ openbao write <mount>/tidy \
      tidy_cert_store=true \
      tidy_revoked_certs=true \
      tidy_acme=true \
      tidy_revocation_queue=true \
...
1. Review the tidy status endpoint, `openbao read <mount>/tidy-status` for
...
1. Verify that tidy ran recently with `openbao read`:
    ```shell-session
    $ openbao read <mount>/tidy-status
    ````
1. Schedule a manual run of tidy with `openbao write`:
    ```shell-session
    $ openbao write <mount>/tidy \
...
Use `openbao secrets tune` to add the missing headers to `allowed_response_headers`:
```shell-session
$ openbao secrets tune \
  -allowed-response-headers="Last-Modified" \
  -allowed-response-headers="Replay-Nonce" \
