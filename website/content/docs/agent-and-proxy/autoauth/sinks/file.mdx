---
layout: docs
page_title: OpenBao Agent and OpenBao Proxy Auto-Auth File Sink
...
- `mode` `(int: optional)` - A string containing an octal number representing the bit pattern for the file mode, similar to chmod. Set to `0000` to prevent OpenBao from modifying the file mode. Note: This configuration option is only available in Vault 1.3.0 and above.
