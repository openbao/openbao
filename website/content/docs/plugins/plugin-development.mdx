...
Because OpenBao communicates to plugins over a gRPC interface, you can build
and distribute a plugin for OpenBao without having to rebuild OpenBao itself.
This makes it easy for you to build an OpenBao plugin for your organization's
internal use, for a proprietary API that you don't want to open source, or to
prototype something before contributing it back to the main project.

Developing a plugin is simple. The only knowledge necessary to write a plugin
is basic command-line skills and basic knowledge of the [Go programming
language](http://go.dev).

Your plugin implementation needs to satisfy the interface for the plugin type
you want to build. You can find these definitions in the docs for the backend
running the plugin.
...
Serving a plugin requires calling into the respective plugin entrypoint function
exposed by the appropriate plugin server SDK for type of plugin developed:

- Auth method and secrets engine plugins: Use
  [`ServeMultiplex`](https://pkg.go.dev/github.com/openbao/openbao/sdk/v2/plugin#ServeMultiplex)
  from
  [`github.com/openbao/openbao/sdk/v2/plugin`](https://pkg.go.dev/github.com/openbao/openbao/sdk/v2/plugin).
- Database provider plugins: Use
  [`ServeMultiplex`](https://pkg.go.dev/github.com/openbao/openbao/sdk/v2/database/dbplugin#ServeMultiplex)
  from
  [`github.com/openbao/openbao/sdk/v2/database/dbplugin`](https://pkg.go.dev/github.com/openbao/openbao/sdk/v2/database/dbplugin).
- KMS provider plugins: Use
  [`Serve`](https://pkg.go.dev/github.com/openbao/go-kms-wrapping/plugin/v2#Serve)
  from
  [`github.com/openbao/go-kms-wrapping/plugin/v2`](https://pkg.go.dev/github.com/openbao/go-kms-wrapping/plugin/v2).
...
OpenBao's [plugin multiplexing](plugin-architecture.mdx#plugin-multiplexing)
...
communication](plugin-architecture.mdx#plugin-communication). If you are certain
...
plugin. Finally, [register](plugin-architecture.mdx#plugin-registration) the
