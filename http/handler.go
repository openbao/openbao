...
func wrapGenericHandler(core *vault.Core, h http.Handler, props *vault.HandlerProperties) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		case nsHeader != "" && strings.HasPrefix(r.URL.Path, "/v1/sys/") && restrictedSysAPIsNonLogical.HasPathSegments(r.URL.Path[8:]):
			respondError(nw, http.StatusBadRequest, fmt.Errorf("unavailable operation: %s %s", r.Method, r.URL.Path))
			cancelFunc()
			return
...
func forwardRequest(core *vault.Core, w http.ResponseWriter, r *http.Request) {
	...
	if alwaysRedirectPaths.HasPath(r.URL.Path[4:]) {
		respondStandby(core, w, r.URL)
		return
	}
