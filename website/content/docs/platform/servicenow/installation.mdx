...
* Select your desired auth method from Agent's [supported auth methods](/docs/agent-and-proxy/autoauth/methods)
...
* Install Vault Agent as a service running `vault agent -config=/path/to/agent.hcl`
  * Documentation for Windows service installation [here](/docs/agent-and-proxy/agent/winsvc)

## Uploading JAR file to MID server

:::warning[Use the ServiceNow app store to install Vault Credential Resolver]
