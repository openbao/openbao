...
description: |-
  ...
  data. Instead all the nodes in a OpenBao cluster will have a replicated copy of
...
all the nodes in a OpenBao cluster will have a replicated copy of OpenBao's data.
...
- `retry_join_as_non_voter` `(boolean: false)` - If set, causes any `retry_join`
  ...
  `openbao operator raft join` command, but only affects voting status when joining
...
- `autopilot_upgrade_version` `(string: "")` - This is an optional string that, if
  provided, will be used reported to autopilot as OpenBao's version. This is then used
  by autopilot when it makes decisions regarding
  [automated upgrades](/vault/docs/enterprise/automated-upgrades). If omitted, the
  version of OpenBao currently in use will be used. Note that this string must conform
  to [Semantic Versioning](https://semver.org). Use of this feature requires OpenBao
  Enterprise.

- `autopilot_redundancy_zone` `(string: "")` - This is an optional string that specifies
  OpenBao's [redundancy zone](/vault/docs/enterprise/redundancy-zones). This is reported to autopilot
...
Storage](/vault/tutorials/raft) series of tutorials to learn more about implementing OpenBao using Integrated Storage.
