...
1.  Configure OpenBao with the proper plugin and connection information to access your Redshift database:

    ```text
    $ openbao write database/config/my-redshift-database \
...
1.  Configure a role that maps a name in OpenBao to a SQL statement to execute which
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    username           v-openbaouse-my-role-zi0460sr1hwm98qugxu8-1602542706
    ```
