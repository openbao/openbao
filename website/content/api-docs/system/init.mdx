...
an auto-unseal mechanism, it is possible to initialize OpenBao with 
`recovery_shares` config parameter set as `0`, resulting in the recovery keys not
generated at initialization time, and thus will not returned back to the operator.
The system will function as if it has no recovery keys. To create recovery keys
the operator is required to to manually call `/sys/rotate/recovery/init` endpoint,
which will return working keys immediately. This is not be applicable for Shamir's
based unsealing. Recovery mode is not function until such keys are created.
