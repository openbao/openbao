...
description: |-
  The FoundationDB storage backend is used to persist OpenBao's data in the
  FoundationDB KV store.
---

# FoundationDB storage backend

The FoundationDB storage backend is used to persist OpenBao's data in
[FoundationDB][foundationdb].

The backend needs to be explicitly enabled at build time, and is not available
in the standard OpenBao binary distribution. Please refer to the documentation
accompanying the backend's source in the OpenBao source tree.

- **High Availability** – the FoundationDB storage backend supports high
  availability. The HA implementation relies on the clocks of the OpenBao
...
storage "foundationdb" {
  ...
  path             = "openbao-top-level-directory"
  ha_enabled       = "true"
}
```
...
- `path` `(string: "openbao")` - The path of the top-level FoundationDB directory
  (using the directory layer) under which the OpenBao data will reside.

- `ha_enabled` `(string: "false")` - Whether or not to enable OpenBao
...
by the user OpenBao is running as. You probably want to isolate the cluster
...
When running OpenBao with mlock, the OpenBao binary must have capabilities set to
allow the use of mlock.

```
# setcap cap_ipc_lock=+ep /path/to/bin/openbao
$ getcap /path/to/bin/openbao
/path/to/bin/openbao = cap_ipc_lock+ep
```

The presence of the capabilities will cause the linker to ignore
`LD_LIBRARY_PATH`, for security reasons.

In that case, we have to set an `RPATH` on the OpenBao binary at build time
to replace the use of `LD_LIBRARY_PATH`.

When building OpenBao, pass the `-r /dest/dir/for/primary` option to the Go
linker, for instance:
...
You can verify `RPATH` is set on the OpenBao binary using `readelf`:

```shell-session
$ readelf -d /path/to/bin/openbao | grep RPATH
