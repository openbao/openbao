...
1. Create a named encryption key:
   ...
   ```text
   $ openbao write -f transit/keys/my-key
...
1.  Encrypt some plaintext data using the `/encrypt` endpoint with a named key:
    ...
    **NOTE:** All plaintext data **must be base64-encoded**. The reason for this
    ...
    ```text
    $ openbao write transit/encrypt/my-key plaintext=$(echo "my secret data" | base64)
    ...
    block](/vault/docs/configuration/listener/tcp) in the OpenBao server
    configuration.

1.  Decrypt a piece of data using the `/decrypt` endpoint with a named key:

    ```text
    $ openbao write transit/decrypt/my-key ciphertext=vault:v1:8SDd3WHDOjf7mq69CyCqYjBXAiQQAVZRkFM13ok481zoCmHnSeDX9vyf7w==
    ...
    ```text
    $ openbao write -field=plaintext transit/decrypt/my-key ciphertext=... | base64 --decode
    my secret data
    ```
...
1.  Rotate the underlying encryption key. This will generate a new encryption key
    ...
    ```text
    $ openbao write -f transit/keys/my-key/rotate
...
1.  Upgrade already-encrypted data to a new key. OpenBao will decrypt the value
    ...
    ```text
    $ openbao write transit/rewrap/my-key ciphertext=vault:v1:8SDd3WHDOjf7mq69CyCqYjBXAiQQAVZRkFM13ok481zoCmHnSeDX9vyf7w==
...
```text
$ openbao read transit/wrapping_key
```
