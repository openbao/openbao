...
This causes `openbao write` to read a JSON blob containing all request parameters
...
```shell-session
$ openbao write cubbyhole/git-credentials username="student01" password="p@$$w0rd"
```

Create a new encryption key in the transit secrets engine:

```shell-session
$ openbao write -force transit/keys/my-key
```
...
```shell-session
$ openbao write aws/roles/ops policy=@policy.json
```

Configure access to Consul by providing an access token:

```shell-session
$ echo $MY_TOKEN | openbao write consul/config/access token=-
```
...
```shell-session
$ openbao write auth/token/create policies="admin" policies="secops" ttl=8h num_uses=3
```
...
The `openbao write` command simplifies the API call.
...
the token creation. Use the `openbao create` command with options to set the token
TTL, policies, and use limit.

```shell-session
$ openbao token create -policy=admin -policy=secops -ttl=8h -use-limit=3
```
