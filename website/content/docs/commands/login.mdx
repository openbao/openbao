...
description: |-
  ...
  provided arguments. A successful authentication results in a OpenBao token -
...
arguments. A successful authentication results in a OpenBao token - conceptually
...
auth method, use the "openbao auth help TYPE" command. You can also use "openbao
...
is already stored in the token helper. You do NOT need to run "openbao login"
...
```shell-session
$ openbao login s.3jnbMAKl1i4YS3QoKdbHzGXq
Success! You are now authenticated. The token information displayed below
is already stored in the token helper. You do NOT need to run "openbao login"
again. Future OpenBao requests will automatically use this token.
...
```shell-session
$ openbao login -method=userpass username=my-username
Password (will be hidden):
Success! You are now authenticated. The token information below is already
stored in the token helper. You do NOT need to run "openbao login" again. Future
...
```shell-session
$ openbao login -method=github -path=github-prod
Success! You are now authenticated. The token information below is already
stored in the token helper. You do NOT need to run "openbao login" again. Future
requests will use this token automatically.
