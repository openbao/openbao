...
## Release checklist
 ...
 - [ ] Open PR to version documentation when doing a minor release. Run the following
       commands in the `website` folder and update the versions in the `docusaurus.config.ts` file:
       ```shell
       npm run docusaurus docs:version:api-docs 2.<minor-version>.x 
       npm run docusaurus docs:version 2.<minor-version>.x 
       ```
