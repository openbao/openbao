...
description: >-
    This is a reference for OpenBao’s profile system and declarative self-initialization. It describes the supported source types and the initialize_name mechanism.
---

The Profiles engine processes declarative API requests with dynamic data source evaluation.
...
The `input` source is only available for profiles; not for declarative
self-initialization.
