...
- `only_public_routes` `(bool: false)` - If set to true, the listener is designated as a public route
  listener. It accepts plain HTTP requests, but only for paths that a backend has explicitly designated
  as public and that an operator has enabled access to.
