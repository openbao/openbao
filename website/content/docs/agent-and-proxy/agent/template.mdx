...
- `error_on_missing_key` `(bool: false)` - Exit with an error when accessing
  ...
  to "true". Also see [`exit_on_retry_failure` in global OpenBao Agent Template Config](/vault/docs/agent-and-proxy/agent/template#interaction-between-exit_on_retry_failure-and-error_on_missing_key).
...
Unlike [OpenBao Agent caching](/vault/docs/agent-and-proxy/agent/caching), the behavior of how OpenBao Agent
