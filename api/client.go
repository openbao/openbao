...
type Client struct {
	...
	// indexTracker is shared across all client implementations;
	// it holds the latest seen index from a given cluster.
	indexTracker *simpleStorageIndexTracker
}
...
func NewClient(c *Config) (*Client, error) {
	...
	if c.DefaultStrongConsistency {
		...
		client.setStorageIndexTracker(newSimpleStorageIndexTracker())
	}
...
func (c *Client) clone(cloneHeaders bool) (*Client, error) {
	...
	client.setStorageIndexTracker(c.indexTracker)
...
func (c *Client) NewRequest(method, requestPath string) *Request {
	...
	var index string
	if c.indexTracker != nil {
		index = c.indexTracker.get()
	}
...
START:
	...
	if result != nil {
		...
		if err := c.setIndexFromResult(r, result); err != nil {
			return result, fmt.Errorf("error parsing %v response headers: %w", IndexHeaderName, err)
		}
	}
