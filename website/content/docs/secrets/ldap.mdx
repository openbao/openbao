...
3. Rotate the root password so only OpenBao knows the credentials:
   ...
   ```sh
   $ openbao write -f ldap/rotate-root
   ```
...
1. Configure a static role that maps a name in OpenBao to an entry in LDAP.
   ...
   ```sh
   $ openbao write ldap/static-role/lf-edge\
...
2. Request credentials for the "openbao" role:
   ...
   ```sh
   $ openbao read ldap/static-cred/lf-edge
   ```
...
```bash
$ openbao write ldap/role/dynamic-role \
...
```bash
$ openbao read ldap/creds/dynamic-role
Key                    Value
---                    -----
...
```shell-session
$ openbao write ldap/library/accounting-team \
...
```shell-session
$ openbao read ldap/library/accounting-team/status
Key                 Value
---                 -----
...
```shell-session
$ openbao write -f ldap/library/accounting-team/check-out
Key                     Value
---                     -----
...
```shell-session
$ openbao write ldap/library/accounting-team/check-out ttl=30m
Key                     Value
---                     -----
...
```shell-session
$ openbao lease renew ldap/library/accounting-team/check-out/0C2wmeaDmsToVFc0zDiX9cMq
Key                Value
---                -----
...
```shell-session
$ openbao write -f ldap/library/accounting-team/check-in
Key          Value
---          -----
...
```shell-session
$ openbao write ldap/library/accounting-team/check-in service_account_names=fizz@example.com
Key          Value
---          -----
check_ins    [fizz@example.com]
```
...
```shell-session
$ openbao write -f ldap/library/manage/accounting-team/check-in
Key          Value
---          -----
check_ins    [fizz@example.com]
```
...
```shell-session
$ openbao lease revoke ldap/library/accounting-team/check-out/PvBVG0m7pEg2940Cb3Jw3KpJ
...
with an LDAP password policy (ppolicy, not to be confused with a OpenBao password
