...
In addition to a verbose [HTTP API](../api/index.mdx), OpenBao features a command-line
...
[argument](../api/auth/userpass.mdx#createupdate-user)
...
[arguments](../api/auth/userpass.mdx#createupdate-user) (`password` and
...
**Example:** API help on the [`sys/mounts/`](../api/system/mounts.mdx) path.
...
To authenticate, use the [`login` command](login.mdx).
...
customizable, and you can even [build your own](token-helper.mdx).
...
[OpenBao client API](../api/libraries.mdx#go).
...
Additional HTTP request headers to send with every CLI request. The value must
be a JSON object with string keys and string values. For example:

```shell-session
$ export BAO_HEADERS='{"X-IAP-Token":"example-token"}'
```

Header names beginning with `X-Vault-` are reserved for OpenBao and are not
allowed. Protect this environment variable when header values contain secrets.
Unlike other CLI flags, a `-header` value with the same name is appended to the
environment-provided value instead of replacing it.