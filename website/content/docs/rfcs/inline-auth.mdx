...
description: |-
  An OpenBao RFC for allowing writeless authentication sent inline with a normal operation if it does not create a lease.
---
...
Furthermore, using the standard authentication flow incurs three storage
write operations:
...
pressure significantly, depending on workload.
...
The generated token is not returned to the user.

## Technical Description

This mostly hooks into `core.switchedLockHandleRequest(...)` called from
`core.HandleRequest` and thus has fairly broad support. Before executing the
given request, we check for inline authentication in
...
However, the application does not see the token ever.
...
- Is the header format sufficiently defensive?
- Will header-size limiting become an issue for certain request types?
- Not clear is if this will affect quotas or other subsystems.
- Not clear if this should have a separate `X-Vault-Inline-Auth-Namespace`
  header to support cross-namespace requests/authentication.
- Should `core.LocalUpdateUserFailedLoginInfo(...)` be skipped on inline
  auth requests?
...
TODO:
 ...
 - Handle revocation of leases if generated and return an error.
