...
func (im *invalidationManager) Add(index string, keys ...string) {
	...
	im.pendingLock.Lock()
	if len(im.pending) == 0 {
		// Pre-allocate enough entries if we're empty; this should occur
		// fairly frequently given the dispatch notification logic.
		im.pending = make([]pendingEntry, 0, len(keys))
	}
