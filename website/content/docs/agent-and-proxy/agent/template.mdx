...
- `static_secret_render_interval` `(string or integer: 5m)` <a name="static_secret_render_interval"></a> - If specified, configures
...
In another example `template_config` with [`error_on_missing_key` parameter in the template stanza](#error_on_missing_key)
...
The parameter
[`error_on_missing_key`](#error_on_missing_key) can be
...
- `command_timeout`Δ `(duration: 30s)` - This is the maximum amount of time to
  ...
  `exec` option.
- `error_on_missing_key` `(bool: false)` <a name="error_on_missing_key"></a> - Exit with an error when accessing
