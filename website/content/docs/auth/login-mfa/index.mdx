...
MFA, an MFA method needs to be configured. Please see [Login MFA API](../../api/secret/identity/mfa/index.mdx) for details
...
Please see [Login MFA Enforcement API](../../api/secret/identity/mfa/login-enforcement.mdx)
...
Please see [Duo API](../../api/secret/identity/mfa/duo.mdx) for details
on how to configure the boolean value for Duo.

To validate the MFA restricted login request, the user sends a second request to the [validate](../../api/system/mfa-validate.mdx)
