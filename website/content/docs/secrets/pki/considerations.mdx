...
can be used to measure the performance of a OpenBao PKI instance. In general,
...
was added. In Vault 1.14, with ACME support, the same configuration is used for allowing
...
unauthenticated from a OpenBao perspective, the use of roles with ACME
...
To solve this issue, there are two possible approaches:
 ...
 1. Use a restrictive [`allowed_roles`, `allowed_issuers`, and
    ...
    to a OpenBao token other mechanism for acquiring a OpenBao EAB ACME token.
 2. Use a more permissive [configuration with
    ...
    to a OpenBao token which can be suitably ACL'd to particular sets of
...
requests (which do not generate OpenBao tokens), it would not be counted in
...
This allows modification of the issuance behavior: should OpenBao `err`,
...
certificate's Subjects. Issuers created within OpenBao are fine, but when using
...
We suggest the following personas:
 ...
 - *Unauthed*; any arbitrary user or service that lacks a OpenBao token.
...
<!-- TODO: Remove documentation regarding older Vault versions -->
