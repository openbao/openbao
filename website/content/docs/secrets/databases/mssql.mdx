...
```sql
-- Create Login
CREATE LOGIN openbao_login WITH PASSWORD = '<password>';

-- Create User
CREATE user openbao_user for login openbao_login;

-- Grant Permissions
GRANT ALTER ANY LOGIN TO "openbao_user";
GRANT ALTER ANY USER TO "openbao_user";
GRANT ALTER ANY CONNECTION TO "openbao_login";
GRANT CONTROL ON SCHEMA::<schema_name> TO "openbao_user";
EXEC sp_addrolemember "db_accessadmin", "openbao_user";
```
...
1.  Configure OpenBao with the proper plugin and connection information:
    ...
    ```text
    $ openbao write database/config/my-mssql-database \
    ...
    ~> Note: The example above demonstrates a connection with SQL server user named `openbaouser`, although the user `openbaouser` might be Windows Authentication user part of Active Directory domain, for example: `DOMAIN\openbaouser`.

    In this case, we've configured OpenBao with the user "openbaouser" and password
    "yourStrong(!)Password", connecting to an instance at "localhost" on port 1433. It is
    not necessary that OpenBao has the openbaouser login, but the user must have privileges
    ...
    users and grant permissions in the databases that OpenBao manages. The fixed
...
OpenBao will execute the default revocation procedure.
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    username           v-openbaouser-my-role-r7kCtKGGr3eYQP1OGR6G-1602542258
    ```
...
<Note>
  Azure SQL databases may use different authentication mechanism that are configured on the SQL server. OpenBao only supports SQL authentication. Azure AD authentication is not supported.
</Note>

First, we mount a database backend at the azuresql path with `openbao secrets enable -path=azuresql database`. Then we configure a connection called "testopenbao" to connect to a database called "test-openbao", using "azuresql" at the beginning of our path:

```shell-session
$ openbao write azuresql/config/testopenbao \
    plugin_name=mssql-database-plugin \
    connection_url='server=hashisqlserver.database.windows.net;port=1433;user id=admin;password=pAssw0rd;database=test-openbao;app name=openbao;' \
    allowed_roles="test"
```

Now we add a role called "test" for use with the "testopenbao" connection:

```shell-session
$ openbao write azuresql/roles/test \
    db_name=testopenbao \
...
We can now use this role to dynamically generate credentials for the Azure SQL database, test-openbao:

```shell-session
$ openbao read azuresql/creds/test
Key            	Value
---            	-----
...
When we no longer need the backend, we can unmount it with `openbao unmount azuresql`. Now, you can use the MSSQL Database Plugin with your Azure SQL databases.
...
the "sysadmin" role, which is used by default during OpenBao's revocation process for MSSQL. The workaround
is to add custom revocation statements to roles, for example:

```shell
openbao write database/roles/my-role revocation_statements="\
   USE my_database \
   IF EXISTS \
     (SELECT name \
