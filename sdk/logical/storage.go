...
func ScanViewWithLogger(ctx context.Context, view ClearableView, logger hclog.Logger, cb func(path string)) error {
	...
	if paginated, ok := view.(PaginatedClearableView); ok {
		return ScanViewPaginated(ctx, paginated, logger, DefaultScanViewPageLimit, func(page int, index int, path string) (cont bool, err error) {
			cb(path)
			return true, nil
		})
	}

	return scanViewWithoutPagination(ctx, view, logger, cb)
}

func ScanViewPaginated(ctx context.Context, view PaginatedClearableView, logger hclog.Logger, pageSize int, cb func(page int, index int, path string) (cont bool, err error)) error {
	frontier := []string{""}
	for len(frontier) > 0 {
		n := len(frontier)
...
func scanViewWithoutPagination(ctx context.Context, view ClearableView, logger hclog.Logger, cb func(path string)) error {
	frontier := []string{""}
	for len(frontier) > 0 {
		n := len(frontier)
		current := frontier[n-1]
		frontier = frontier[:n-1]
		logger.Trace("iterating frontier", "n", n, "current", current, "remaining", frontier)
