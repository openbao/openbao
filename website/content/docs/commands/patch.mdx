...
This causes `openbao patch` to read a JSON blob containing all request parameters
...
```shell-session
$ openbao patch pki/roles/example allow_localhost=false
```
...
The `openbao patch` command simplifies the API call.
