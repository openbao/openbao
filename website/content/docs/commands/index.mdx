...
```text
openbao <command> [options] [path] [args]
```
...
```shell-session
$ openbao write -address="http://127.0.0.1:8200" auth/userpass/users/bob password="long-password"
```
...
```shell-session
$ openbao write -address="http://127.0.0.1:8200" -namespace="my-organization" \
...
The four most common operations in OpenBao are [read](/vault/docs/commands/read),
[write](/vault/docs/commands/write), [delete](/vault/docs/commands/delete), and
[list](/vault/docs/commands/list). These operations work on most paths in OpenBao. Some
...
```shell-session
$ openbao write -output-curl-string  auth/userpass/users/bob password="long-password"

curl -X PUT -H "X-Vault-Request: true" -H "X-Vault-Token: $(openbao print token)" -d '{"password":"long-password"}' http://127.0.0.1:8200/v1/auth/userpass/users/bob
```
...
```
$ openbao kv put -output-policy kv/secret value=itsasecret
...
To invoke the OpenBao API paths, you can use the [read](/vault/docs/commands/read) (for
...
```shell-session
$ openbao kv put secret/password value=itsasecret
```
...
```shell-session
$ echo -n '{"value":"itsasecret"}' | openbao kv put secret/password -
```
...
```shell-session
$ echo -n "itsasecret" | openbao kv put secret/password value=-
```
...
```shell-session
$ openbao kv put secret/password @data.json
```

Or specify the contents of a file as a value:

```shell-session
$ openbao kv put secret/password value=@data.txt
```
...
All `kv` commands can alternatively refer to the path to the KV secrets engine using a flag-based syntax like `$ openbao kv get -mount=secret password`
instead of `$ openbao kv get secret/password`. The mount flag syntax was created to mitigate confusion caused by the fact that for KV v2 secrets,
...
The `openbao` command features opt-in autocompletion for flags, subcommands, and
arguments (where supported).

Enable autocompletion by running:

```shell-session
$ openbao -autocomplete-install
```

~> Be sure to **restart your shell** after installing autocompletion!

When you start typing a OpenBao command, press the `<tab>` character to show a
...
This environment variable will limit the rate at which the `openbao` command
...
_Note:_ The rate is limited for each invocation of the `openbao` CLI. Since
each invocation of the `openbao` CLI typically only makes a few requests,
this environment variable is most useful when using the Go
[OpenBao client API](/vault/api-docs/libraries#go).
...
~> **Note:** Disabling redirect following behavior could cause issues with commands such as 'openbao operator raft snapshot' as this command redirects the request to the cluster's primary node.
