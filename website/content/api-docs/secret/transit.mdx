...
- `type` `(string: "aes256-gcm96")` – Specifies the type of key to create. The
  ...
  ~> **Note**: In FIPS 140-2 mode, the following algorithms are not certified
     and thus should not be used: `chacha20-poly1305`, `xchacha20-poly1305`,
     and `ed25519`.
...
!> OpenBao HTTP API imposes a maximum request size of 32MB to prevent a denial of service attack. This can be tuned per [`listener` block](/vault/docs/configuration/listener/tcp) in the OpenBao server configuration.
