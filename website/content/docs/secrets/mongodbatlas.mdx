...
1. Enable the MongoDB Atlas Secrets Engine:
   ...
   ```bash
   $ bao secrets enable mongodbatlas
...
```shell-session
$ bao write mongodbatlas/roles/test \
...
```shell-session
$ bao read mongodbatlas/roles/test
...
```shell-session
$ bao read mongodbatlas/creds/test
