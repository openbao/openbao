...
description: |-
  The Okta auth method allows users to authenticate with OpenBao using Okta
  credentials.
---
...
credentials. This allows OpenBao to be integrated into environments using Okta.

The mapping of groups in Okta to OpenBao policies is managed by using the
...
```shell-session
$ openbao login -method=okta username=my-username
```
...
```shell-session
$ openbao login -method=okta username=my-username totp=123456
```
...
```shell-session
$ openbao login -method=okta username=my-username totp=123456 provider=GOOGLE
```
...
1. Enable the Okta auth method:
  ...
  ```shell-session
  $ openbao auth enable okta
  ```

1. Configure OpenBao to communicate with your Okta account:

  ```shell-session
  $ openbao write auth/okta/config \
     base_url="okta.com" \
     org_name="dev-123456" \
     api_token="00abcxyz..."
  ```
...
1. Map an Okta group to a OpenBao policy:

  ```shell-session
  $ openbao write auth/okta/groups/scientists policies=nuclear-reactor
  ```

  In this example, anyone who successfully authenticates via Okta who is a
  member of the "scientists" group will receive a OpenBao token with the
  "nuclear-reactor" policy attached.

1. It is also possible to add users directly:

  ```shell-session
  $ openbao write auth/okta/groups/engineers policies=autopilot
  $ openbao write auth/okta/users/tesla groups=engineers
  ```

  This adds the Okta user "tesla" to the "engineers" group, which maps to
  the "autopilot" OpenBao policy.
