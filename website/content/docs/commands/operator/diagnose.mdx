...
description: |-
  "openbao operator diagnose" is a new operator-centric command, focused on providing a clear description 
...
`Check Consul Direct Storage Access` is a consul-specific check that ensures OpenBao is not accessing
...
`Check For Autoloaded License` is an enterprise diagnose check, which verifies that OpenBao 
