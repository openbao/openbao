...
If you plan to support authentication via `openbao login -method=oidc`, a localhost redirect URI
...
```shell-session
$ openbao login -method=oidc port=8400 role=test
...
```text
openbao write auth/oidc/role/demo -<<EOF
{
  "user_claim": "sub",
...
```shell-session
$ openbao write auth/jwt/login role=demo jwt=...
```
...
1. Create a named role:
   ...
   ```text
   openbao write auth/jwt/role/demo \
