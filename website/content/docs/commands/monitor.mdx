...
description: |-
  The "monitor" command displays the server logs of a OpenBao server in real
  time.
---

# monitor

The `monitor` command shows a real time display of the server logs of a OpenBao
...
```shell-session
$ openbao monitor -log-level=debug
```
