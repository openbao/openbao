...
| `namespace` _string_ | Namespace to get the secret from in Vault |
| `mount` _string_ | Mount for the secret in Vault |
| `path` _string_ | Path of the secret in Vault, corresponds to the `path` parameter for, [kv-v1](../../../api/secret/kv/kv-v1.mdx#read-secret) [kv-v2](../../../api/secret/kv/kv-v2.mdx#read-secret-version) |
| `version` _integer_ | Version of the secret to fetch. Only valid for type kv-v2. Corresponds to version query parameter: [version](../../../api/secret/kv/kv-v2.mdx#version) |
