...
introduce a OpenBao token into a Kubernetes Pod.
...
can check with `openbao read -field disable_iss_validation auth/kubernetes/config`.
...
```shell-session
$ openbao write auth/kubernetes/login role=demo jwt=...
```
...
1. Use the `/config` endpoint to configure OpenBao to talk to Kubernetes. Use
  ...
  OpenBao](/vault/docs/internals/security), this is allowable because OpenBao is
...
1. Create a named role:
  ...
  ```text
  openbao write auth/kubernetes/role/demo \
...
import (
	...
	auth "github.com/lf-edge/openbao/api/auth/kubernetes"
)
