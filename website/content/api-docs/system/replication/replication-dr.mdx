...
- `primary_api_addr` `(string: "")` – Set this to the API address (normal OpenBao
...
~> It is not safe to replicate from a newer version of OpenBao to an older version.
When upgrading replicated clusters, ensure that upstream clusters are always
on older versions of OpenBao than downstream clusters. See
[Upgrading OpenBao](/vault/docs/upgrading#replication-installations) for an example.
...
- `primary_api_addr` `(string: )` – Specifies the API address (normal OpenBao
  address) to override the value embedded in the token. This can be useful if
  the primary's redirect address is not accessible directly from this cluster.
...
OpenBao will complete the generation and issue the new token. Otherwise,
