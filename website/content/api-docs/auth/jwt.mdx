...
[OpenBao JWT/OIDC method documentation](/vault/docs/auth/jwt).
...
against any bound claims, and if valid a OpenBao token will be returned.
...
- `nonce` `(string: <required>)` - Opaque nonce that is part of the Authorization URL and will
  be included in the the redirect following successful authentication on the provider.
- `code` `(string: <required>)` - Provider-generated authorization code that OpenBao will exchange for
  an ID token.
