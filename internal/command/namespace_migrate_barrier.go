...
func (c *NamespaceMigrateBarrierCommand) Synopsis() string {
	return "Migrate a normal namespace to a sealable namespace or vice-versa"
}

func (c *NamespaceMigrateBarrierCommand) Help() string {
	helpText := `
Usage: bao namespace migrate-barrier [options] PATH

  Migrate a sealable namespace to a normal namespace:

      $ bao namespace migrate-barrier ns1
      
  Migrate a normal namespace to a sealable namespace:

      $ bao namespace migrate-barrier -seal=./seal.hcl ns1
...
func (c *NamespaceMigrateBarrierCommand) Flags() *FlagSets {
...
func (c *NamespaceMigrateBarrierCommand) AutocompleteArgs() complete.Predictor {
	return complete.PredictNothing
}

func (c *NamespaceMigrateBarrierCommand) AutocompleteFlags() complete.Flags {
	return c.Flags().Completions()
}

func (c *NamespaceMigrateBarrierCommand) Run(args []string) int {
	f := c.Flags()
	...
	input := &api.MigrateNamespaceBarrierInput{
	...
	resp, err := client.Sys().MigrateNamespaceBarrier(namespacePath, input)
	if err != nil {
		c.UI.Error(fmt.Sprintf("Error migrating namespace: %s", err))
		return 2
	}
	...
	if isTable && resp != nil && len(resp.KeyShares) != 0 {
		...
		c.UI.Output(wrapAtLength(fmt.Sprintf(
			"Namespace migrated with %d key shares and a key threshold of %d. Please "+
