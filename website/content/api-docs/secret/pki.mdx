...
### Lightweight OCSP Requests

Some OCSP clients only work with OCSP servers that support lightweight OCSP
([RFC 5019](https://datatracker.ietf.org/doc/html/rfc5019)), which requires the server to handle and
serve OCSP messages over plaintext HTTP. OpenBao supports hosting OCSP endpoints over plaintext HTTP
with the following configuration and steps:

1. The OpenBao server must be configured with a plaintext HTTP listener that only handles requests for
   configured and enabled public paths. The following is an example of such a configuration:

```hcl
   listener "tcp" {
     address            = "0.0.0.0:8280"
     tls_disable        = true
     only_public_routes = true // Dedicates the listener to only handle public routes
   }
```

2. Tune the PKI mount to expose its public paths (e.g., `ocsp` and `ocsp/*`) on the public listener.

```bash
   bao write sys/mounts/pki/tune expose_public_paths=true
```

Once the steps above have been performed, the mounted PKI engine can handle lightweight OCSP requests
...
- `cel_program` `(CelProgram: <required>)` - Specifies the CEL Variable list and main
  CEL expression. Refer to [Quick Start - CEL in PKI](/docs/secrets/pki/quick-start-cel) for more
